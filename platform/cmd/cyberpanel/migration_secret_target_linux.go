...
func newMigrationSecretTarget(ctx context.Context, db *sql.DB, scopes *migration.RuntimeScopeStore) (*migrationSecretTarget, error) {
	...
	if _, err := db.ExecContext(ctx, migrationTargetSecretSchema); err != nil {
		return nil, err
	}
...
func (target *migrationSecretTarget) databaseAudience(ctx context.Context, manifest migration.Manifest, plan migration.Plan, scope migration.RuntimeScope, envelope migration.SecretEnvelope) (secrets.ID, secrets.Purpose, secrets.AudienceBinding, error) {
	...
	if envelope.Purpose != "database-principal" {
	...
	for _, value := range manifest.Databases {
		for _, principal := range value.Principals {
			if principal.SecretID != envelope.SecretID {
				continue
			}
			if principalID != "" || principal.CredentialDisposition != migration.CredentialPreserved {
				return "", "", secrets.AudienceBinding{}, migration.ErrBlocked
			}
			var targetID migration.ID
			for _, mapping := range plan.Mappings {
				if mapping.SourceKind == string(migration.ImportDatabase) && mapping.SourceID == value.SourceID {
					if targetID != "" || mapping.Disposition != migration.DispositionCreate || !mapping.TargetID.Valid() {
						return "", "", secrets.AudienceBinding{}, migration.ErrBlocked
					}
...
func (target *migrationSecretTarget) approved(ctx context.Context, id migration.ID, envelope migration.SecretEnvelope) (migration.Manifest, migration.RuntimeScope, migrationSecretAuthority, error) {
	...
	if target.audienceResolver == nil {
		return migration.Manifest{}, migration.RuntimeScope{}, authority, migration.ErrBlocked
	}
	value, err := target.repository.Migration(ctx, id)
	if err != nil {
		return migration.Manifest{}, migration.RuntimeScope{}, authority, err
	}
	if value.Phase == migration.PhaseCanceled || value.Phase == migration.PhaseRolledBack || value.Phase == migration.PhaseRollingBack || value.Phase == migration.PhaseFailedTerminal || value.ManifestRoot == "" || value.PlanDigest == "" {
		return migration.Manifest{}, migration.RuntimeScope{}, authority, migration.ErrBlocked
	}
	scope, err := target.scopes.LoadByMigration(ctx, id)
	if err != nil {
		return migration.Manifest{}, scope, authority, err
	}
	...
	for _, candidate := range manifest.Secrets {
		if candidate.SecretID != envelope.SecretID {
			continue
		}
		candidateDigest, _, err := migrationSecretDigest(candidate)
		if err != nil || candidateDigest != digest || matched {
			return manifest, scope, authority, migration.ErrConflict
		}
		matched = true
	}
	if !matched {
	...
	if err != nil || plan.MigrationID != id || plan.ApprovedAt == nil {
		return manifest, scope, authority, errors.Join(migration.ErrBlocked, err)
	}
	...
	if scope.TenantID == "" || (owner != scopedSecretID("tenant", scope.TenantID) && owner != secrets.ID("tenant_"+hex.EncodeToString(tenantHash[:])[:48])) || audience.Validate() != nil {
		return manifest, scope, authority, migration.ErrBlocked
	}
...
func (target *migrationSecretTarget) ImportMigrationSecret(ctx context.Context, id migration.ID, envelope migration.SecretEnvelope) error {
	if target == nil || ctx == nil || !id.Valid() {
	...
	defer tx.Rollback()
	var canceled int
	if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM panel_migration_secret_cancellations WHERE migration_id=?`, id.String()).Scan(&canceled); err != nil {
		return err
	}
	if canceled != 0 {
	...
	if row.digest != digest || row.tenant != scope.TenantID || row.source != string(manifest.Source) || row.sourceInstallation != manifest.SourceInstallationID || row.targetInstallation != manifest.TargetInstallationID || !bytes.Equal(row.authority, encodedAuthority) {
		return migration.ErrConflict
	}
	if row.state != "pending" && row.state != "enrolled" {
		return migration.ErrBlocked
	}
	...
	if row.state == "enrolled" && !bytes.Equal(row.metadata, encodedMetadata) {
		return migration.ErrConflict
	}
	...
	if count != 1 {
		...
		defer cancel()
		if _, saveErr := target.db.ExecContext(cleanup, `UPDATE panel_migration_secret_bindings SET metadata_json=? WHERE migration_id=? AND envelope_id=? AND envelope_digest=? AND state!='revoked'`, encodedMetadata, id.String(), envelope.SecretID, digest); saveErr != nil {
			return errors.Join(migration.ErrAmbiguous, saveErr)
		}
...
func (target *migrationSecretTarget) enroll(ctx context.Context, id migration.ID, row migrationSecretRow, envelope migration.SecretEnvelope, authority migrationSecretAuthority) (secrets.Metadata, error) {
	...
	if preparedAuthority.Audience.ResourceKind != authority.Audience.ResourceKind {
		return secrets.Metadata{}, migration.ErrConflict
	}
	...
	if metadata.Validate() != nil || metadata.ID != authority.ID || metadata.OwnerTenantID != authority.Owner || metadata.Purpose != authority.Purpose || metadata.Version != 1 || metadata.State != secrets.StateActive {
		return secrets.Metadata{}, migration.ErrConflict
	}
	_, actual, err := migrationSecretDigest(metadata.Audience)
	_, expected, expectedErr := migrationSecretDigest(authority.Audience)
	if err != nil || expectedErr != nil || !bytes.Equal(actual, expected) {
		return secrets.Metadata{}, migration.ErrConflict
	}
	return metadata, nil
}
...
func (target *migrationSecretTarget) load(ctx context.Context, id migration.ID, envelopeID string) (migrationSecretRow, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return row, migration.ErrNotFound
	}
	return row, err
}
...
func (target *migrationSecretTarget) Resolve(ctx context.Context, id migration.ID, envelopeID string) (secrets.Metadata, error) {
	if target == nil || ctx == nil || !id.Valid() {
		return secrets.Metadata{}, migration.ErrInvalid
	}
	target.mu.Lock()
	defer target.mu.Unlock()
	...
	if row.state != "enrolled" {
		return secrets.Metadata{}, migration.ErrBlocked
	}
	var metadata secrets.Metadata
	if json.Unmarshal(row.metadata, &metadata) != nil || metadata.Validate() != nil {
		return secrets.Metadata{}, migration.ErrConflict
	}
	...
	digest, _, err := migrationSecretDigest(envelope)
	if err != nil || digest != row.digest || envelope.SecretID != envelopeID {
		return secrets.Metadata{}, migration.ErrConflict
	}
	manifest, scope, authority, err := target.approved(ctx, id, envelope)
	if err != nil {
	...
	if metadata.Audience.ResourceKind == "database_principal_native_hash" {
		if envelope.Purpose != "database-principal" || authority.Audience.ResourceKind != "database_principal" {
			return secrets.Metadata{}, migration.ErrBlocked
		}
		authority.Audience.ResourceKind = "database_principal_native_hash"
	}
	_, encodedAuthority, err := migrationSecretDigest(authority)
	if err != nil || row.tenant != scope.TenantID || row.source != string(manifest.Source) || row.sourceInstallation != manifest.SourceInstallationID || row.targetInstallation != manifest.TargetInstallationID || !bytes.Equal(row.authority, encodedAuthority) {
		return secrets.Metadata{}, migration.ErrConflict
	}
	var canceled int
	if err := target.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM panel_migration_secret_cancellations WHERE migration_id=?`, id.String()).Scan(&canceled); err != nil {
		return secrets.Metadata{}, err
	}
	if canceled != 0 {
		return secrets.Metadata{}, migration.ErrBlocked
	}
	if metadata.ID != authority.ID || metadata.OwnerTenantID != authority.Owner || metadata.Purpose != authority.Purpose || metadata.State != secrets.StateActive || metadata.Version != 1 {
		return secrets.Metadata{}, migration.ErrConflict
	}
	...
	if err != nil || expectedErr != nil || !bytes.Equal(actualAudience, expectedAudience) {
		return secrets.Metadata{}, migration.ErrConflict
	}
	return metadata, nil
}
...
func migrationPasswordIsHash(value []byte) bool {
	...
	if value[0] == '*' || value[0] == '$' || bytes.HasPrefix(value, []byte("{SHA")) {
		return true
	}
...
func migrationSecretMaterial(source, purpose string, value []byte, authority migrationSecretAuthority) (migrationSecretAuthority, []byte, error) {
	...
	if purpose == "mailbox-credential" {
		if authority.Purpose != secrets.PurposeAuthentication || authority.Audience.AdapterID != mail.MailboxCredentialAdapterID || authority.Audience.ResourceKind != "mailbox_crypt_bcrypt" || mail.ValidateMailboxCredentialHash(value) != nil {
			return authority, nil, migration.ErrBlocked
		}
		return authority, append([]byte(nil), value...), nil
	}
	if purpose == "mail-dkim-private-key" {
		if authority.Purpose != secrets.PurposeDKIMKey || authority.Audience.AdapterID != mail.OpenDKIMMaterialAdapterID || authority.Audience.ResourceKind != "mail_domain_dkim" {
			return authority, nil, migration.ErrBlocked
		}
		if _, err := mail.DKIMPublicBindingFromPrivateKey(value); err != nil {
			return authority, nil, migration.ErrBlocked
		}
		return authority, append([]byte(nil), value...), nil
	}
	if purpose != "database-principal" {
		return authority, append([]byte(nil), value...), nil
	}
...
func (target *migrationSecretTarget) RevokeMigrationSecrets(ctx context.Context, id migration.ID) error {
	...
	if _, err := target.db.ExecContext(ctx, `INSERT INTO panel_migration_secret_cancellations(migration_id,canceled_at) VALUES(?,?) ON CONFLICT(migration_id) DO NOTHING`, id.String(), time.Now().UTC().Format(time.RFC3339Nano)); err != nil {
		return err
	}
	...
	for _, identifier := range identifiers {
		...
		if row.tenant != scope.TenantID {
			return migration.ErrConflict
		}
		if _, err := target.db.ExecContext(ctx, `UPDATE panel_migration_secret_bindings SET state='revoking' WHERE migration_id=? AND envelope_id=? AND state!='revoked'`, id.String(), identifier); err != nil {
			return err
		}
		var metadata secrets.Metadata
		if len(row.metadata) == 0 {
			...
			var envelope migration.SecretEnvelope
			var authority migrationSecretAuthority
			if json.Unmarshal(row.envelope, &envelope) != nil || json.Unmarshal(row.authority, &authority) != nil {
				return migration.ErrInvalid
			}
			digest, _, err := migrationSecretDigest(envelope)
			...
			if _, err := target.db.ExecContext(ctx, `UPDATE panel_migration_secret_bindings SET metadata_json=? WHERE migration_id=? AND envelope_id=? AND state='revoking'`, encoded, id.String(), identifier); err != nil {
				return err
			}
		} else if json.Unmarshal(row.metadata, &metadata) != nil || metadata.Validate() != nil {
			return migration.ErrInvalid
		}
		if _, err := target.management.Revoke(ctx, migrationSecretRevokeRequest(metadata)); err != nil {
			return err
		}
		if _, err := target.db.ExecContext(ctx, `UPDATE panel_migration_secret_bindings SET state='revoked',envelope_json=?,metadata_json=? WHERE migration_id=? AND envelope_id=? AND state='revoking'`, []byte{}, []byte{}, id.String(), identifier); err != nil {
			return err
		}
	}
	return nil
...
func (target *migrationSecretTarget) decrypt(ctx context.Context, id migration.ID, installation string, envelope migration.SecretEnvelope) ([]byte, error) {
	if installation == "" || envelope.KeyID == "" || envelope.Algorithm != "X25519-HKDF-SHA256-AES-256-GCM" || envelope.Version != 1 || len(envelope.EncapsulatedKey) != 32 || len(envelope.Ciphertext) < 29 || len(envelope.Ciphertext) > (900<<10)+28 || len(envelope.AudienceDigest) != 64 {
		return nil, migration.ErrInvalid
	}
	...
	if err != nil || len(plaintext) == 0 || len(plaintext) > 900<<10 {
		wipeBytes(plaintext)
...
func migrationTargetSecretVerifier() (*migration.SignedManifestVerifier, error) {
	...
	if !ok || !before.Mode().IsRegular() || before.Mode().Perm()&0o022 != 0 || (stat.Uid != 0 && int(stat.Uid) != os.Geteuid()) || stat.Nlink != 1 || before.Size() < 1 || before.Size() > 1<<20 {
		return nil, migration.ErrBlocked
	}
	...
	if err != nil || pathErr != nil || !os.SameFile(before, after) || !os.SameFile(before, current) || before.Size() != after.Size() || before.Size() != int64(len(raw)) || !before.ModTime().Equal(after.ModTime()) {
	...
	for _, hash := range document.SchemaHashes {
		policy.SchemaHashes[strings.ToLower(strings.TrimSpace(hash))] = struct{}{}
	}
