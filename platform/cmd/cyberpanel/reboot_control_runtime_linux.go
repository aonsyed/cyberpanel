...
func assembleRebootControlLinuxEdge(ctx context.Context,db *sql.DB,executor operations.HostExecutor,haRepository ha.SQLRepository,now func()time.Time)(*rebootControlLinuxEdge,error){
	...
	if _,err=db.ExecContext(ctx,`CREATE TRIGGER IF NOT EXISTS reboot_quiesce_operations BEFORE INSERT ON panel_operation_receipts WHEN EXISTS(SELECT 1 FROM reboot_states WHERE node_id=NEW.node_id AND phase IN ('draining','checkpointed','armed','reboot_dispatched','reconciling')) BEGIN SELECT RAISE(ABORT,'node reboot quiesced'); END`);err!=nil{return nil,err}
...
func(runtime *rebootControlLocalRuntime)DrainListeners(ctx context.Context,operation rebootcontrol.DrainOperation)(rebootcontrol.DrainResult,error){if operation.NodeID!="local"||operation.OperationDigest==""{return rebootcontrol.DrainResult{},rebootcontrol.ErrInvalid};safe,writer,err:=runtime.writerSafe(ctx,operation.NodeID,runtime.now().UTC());if err!=nil||!safe{if err==nil{err=rebootcontrol.ErrConflict};return rebootcontrol.DrainResult{},err};node,err:=runtime.ha.LoadNode(ctx,ha.NodeID(operation.NodeID));if err==nil&&node.State!=ha.NodeDraining{if node.State!=ha.NodeReady{return rebootcontrol.DrainResult{},rebootcontrol.ErrConflict};_,err=(ha.GroupService{Store:runtime.ha,Now:runtime.now}).BeginDrain(ctx,node.ID,node.Generation,operation.Deadline,false)}else if errors.Is(err,ha.ErrNotFound){err=nil};if err!=nil{return rebootcontrol.DrainResult{},err};remaining,err:=runtime.acceptedOperations(ctx,operation.NodeID);if err!=nil{return rebootcontrol.DrainResult{},err};return rebootcontrol.DrainResult{OperationID:operation.ID,Fence:operation.Fence,Complete:remaining==0,RemainingWork:remaining,EvidenceDigest:rebootRuntimeDigest(operation.OperationDigest,writer,strconv.FormatUint(remaining,10))},nil}
func(runtime *rebootControlLocalRuntime)CheckpointOrRecordResumable(ctx context.Context,operation rebootcontrol.OperationCheckpoint)(rebootcontrol.OperationCheckpointResult,error){remaining,err:=runtime.acceptedOperations(ctx,operation.NodeID);if err!=nil{return rebootcontrol.OperationCheckpointResult{},err};nonresumable:=uint32(remaining);return rebootcontrol.OperationCheckpointResult{OperationID:operation.ID,Fence:operation.Fence,Complete:remaining==0,NonResumable:nonresumable,EvidenceDigest:rebootRuntimeDigest(operation.OperationDigest,strconv.FormatUint(remaining,10))},nil}
...
func(runtime *rebootControlLocalRuntime)ReconcileAfterBoot(ctx context.Context,operation rebootcontrol.ReconcileOperation)(rebootcontrol.ReconcileResult,error){remaining,err:=runtime.acceptedOperations(ctx,operation.NodeID);if err!=nil||remaining!=0{if err==nil{err=rebootcontrol.ErrConflict};return rebootcontrol.ReconcileResult{},err};safe,writer,err:=runtime.writerSafe(ctx,operation.NodeID,runtime.now().UTC());if err!=nil||!safe{if err==nil{err=rebootcontrol.ErrConflict};return rebootcontrol.ReconcileResult{},err};var integrity string;if err=runtime.db.QueryRowContext(ctx,"PRAGMA integrity_check(1)").Scan(&integrity);err!=nil||integrity!="ok"{if err==nil{err=rebootcontrol.ErrIntegrity};return rebootcontrol.ReconcileResult{},err};plan,err:=runtime.reboots.LoadPlan(ctx,operation.PlanID);if err!=nil{return rebootcontrol.ReconcileResult{},err};occurrence,err:=runtime.windows.LoadOccurrence(ctx,plan.Maintenance.OccurrenceID);if err!=nil||occurrence.Digest!=plan.Maintenance.OccurrenceDigest{if err==nil{err=rebootcontrol.ErrIntegrity};return rebootcontrol.ReconcileResult{},err};services,err:=runtime.qualifyRebootServices(ctx,operation);if err!=nil{return rebootcontrol.ReconcileResult{},err};actual:=rebootcontrol.BootIdentity{BootID:operation.ActualBootID,KernelRelease:operation.KernelRelease,KernelDigest:operation.KernelDigest,BootSlot:operation.BootSlot,ObservedAt:runtime.now().UTC(),EvidenceDigest:operation.BootEvidenceDigest};if err=runtime.reboots.ClearRequirement(ctx,plan.Packages.TransactionID,actual);err!=nil&&!errors.Is(err,rebootcontrol.ErrNotFound){return rebootcontrol.ReconcileResult{},err};configuration:=rebootRuntimeDigest(operation.OperationDigest,integrity);locks:=rebootRuntimeDigest(operation.OperationDigest,writer);schedules:=rebootRuntimeDigest(operation.OperationDigest,occurrence.Digest,plan.Packages.TransactionID,"cleared");result:=rebootcontrol.ReconcileResult{OperationID:operation.ID,Fence:operation.Fence,Services:rebootcontrol.ComponentResult{Complete:true,EvidenceDigest:services},Configuration:rebootcontrol.ComponentResult{Complete:true,EvidenceDigest:configuration},Locks:rebootcontrol.ComponentResult{Complete:true,EvidenceDigest:locks},Schedules:rebootcontrol.ComponentResult{Complete:true,EvidenceDigest:schedules}};result.EvidenceDigest=rebootRuntimeDigest(services,configuration,locks,schedules);return result,nil}
...
func(runtime *rebootControlLocalRuntime)releaseDrain(ctx context.Context,nodeID string)error{node,err:=runtime.ha.LoadNode(ctx,ha.NodeID(nodeID));if errors.Is(err,ha.ErrNotFound){return nil};if err!=nil{return err};if node.State==ha.NodeReady{return nil};if node.State!=ha.NodeDraining{return rebootcontrol.ErrConflict};_,err=(ha.GroupService{Store:runtime.ha,Now:runtime.now}).SetNodeState(ctx,node.ID,node.Generation,ha.NodeReady);return err}
