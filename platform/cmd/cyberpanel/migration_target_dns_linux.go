...
func migrationDNSIntent(scope migration.RuntimeScope, intent migration.ImportIntent) (dns.ZoneSpec, []dns.RecordSet, error) {
	var source migration.DNSZone
	if json.Unmarshal(intent.Payload, &source) != nil || source.SourceID != intent.SourceID || source.Mode != "native" || source.DNSSEC || len(intent.Chunks) != 0 || len(intent.SecretIDs) != 0 {
		return dns.ZoneSpec{}, nil, migration.ErrBlocked
	}
	name, err := dns.ParseName(source.Name)
	if err != nil {
		return dns.ZoneSpec{}, nil, err
	}
	zone := dns.ZoneSpec{ID: dns.ZoneID(intent.TargetID), TenantID: scope.TenantID, Name: name, Mode: dns.ZoneNative, Account: scope.TenantID, Generation: 1}
	sets := []dns.RecordSet{}
	soa := false
	keys := map[string]bool{}
	for _, record := range source.RecordSets {
		owner, parseErr := dns.ParseName(record.Name)
		if parseErr != nil {
			return zone, nil, parseErr
		}
		if strings.EqualFold(record.Type, "SOA") {
			if soa || len(record.Values) != 1 || (owner.String() != "@" && owner.String() != name.String()) {
				return zone, nil, migration.ErrInvalid
			}
			fields := strings.Fields(record.Values[0])
			if len(fields) != 7 {
				return zone, nil, migration.ErrInvalid
			}
			primary, parseErr := dns.ParseName(fields[0])
			if parseErr != nil {
				return zone, nil, parseErr
			}
			hostmaster, parseErr := dns.ParseName(fields[1])
			if parseErr != nil {
				return zone, nil, parseErr
			}
			values := make([]uint32, 4)
			for index := range values {
				parsed, parseErr := strconv.ParseUint(fields[index+3], 10, 32)
				if parseErr != nil {
					return zone, nil, parseErr
				}
				values[index] = uint32(parsed)
			}
			if values[0] < 300 || values[1] < 60 || values[2] < 86400 || values[3] < 30 || record.TTL < 30 {
				return zone, nil, migration.ErrInvalid
			}
			zone.SOA = dns.SOAConfig{Primary: primary, Hostmaster: hostmaster, Refresh: values[0], Retry: values[1], Expire: values[2], Minimum: values[3], DefaultTTL: record.TTL}
			soa = true
			continue
		}
		set, canonicalErr := (dns.RecordSet{ZoneID: zone.ID, Owner: owner, Kind: dns.RRKind(strings.ToUpper(record.Type)), TTL: record.TTL, Records: append([]string(nil), record.Values...)}).Canonical(zone.Name)
		if canonicalErr != nil {
			return zone, nil, canonicalErr
		}
		key := set.Owner.String() + "|" + string(set.Kind)
		if keys[key] {
			return zone, nil, migration.ErrInvalid
		}
		keys[key] = true
	...
	if !soa {
		return zone, nil, migration.ErrBlocked
	}
	sort.Slice(sets, func(i, j int) bool {
		return sets[i].Owner.String()+string(sets[i].Kind) < sets[j].Owner.String()+string(sets[j].Kind)
	})
...
func (target *migrationHostTarget) stageDNS(ctx context.Context, scope migration.RuntimeScope, intent migration.ImportIntent) (migration.ImportEffect, error) {
	zone, sets, err := migrationDNSIntent(scope, intent)
	if err != nil {
	...
	effect := migrationHostEffect(intent, struct {
		...
		Sets    []dns.RecordSet
		Absence string
	}{"dns-staged-unpublished", zone, sets, proof}, 1, 0, uint64(len(sets)))
	effect.ErrorCode = "DNS_STAGED_NOT_PUBLISHED"
	return effect, nil
}
...
func (target *migrationHostTarget) observeDNS(ctx context.Context, scope migration.RuntimeScope, intent migration.ImportIntent, active bool) (string, error) {
	wanted, sets, err := migrationDNSIntent(scope, intent)
	if err != nil {
		return "", err
	}
	if !active {
		cursor := ""
		for {
			zones, next, listErr := target.dns.ListZones(ctx, scope.TenantID, 500, cursor)
			if listErr != nil {
				return "", listErr
			}
			for _, candidate := range zones {
				if candidate.ID == wanted.ID || candidate.Name == wanted.Name {
		...
		return migrationHostDigest(struct{ Tenant, ID, State string }{scope.TenantID, string(wanted.ID), "not-published"}), nil
	}
	zone, err := target.dns.Zone(ctx, scope.TenantID, wanted.ID)
	if err != nil {
		return "", err
	}
	if zone.Name != wanted.Name || zone.TenantID != wanted.TenantID || zone.Mode != wanted.Mode || zone.Generation != 1 || zone.SOA != wanted.SOA {
		return "", migration.ErrConflict
	}
	...
	canonical := func(values []dns.RecordSet) (string, error) {
		for index := range values {
			value, err := values[index].Canonical(zone.Name)
			if err != nil {
				return "", err
			}
			sort.Strings(value.Records)
			values[index] = value
		}
		sort.Slice(values, func(i, j int) bool {
			return values[i].Owner.String()+string(values[i].Kind) < values[j].Owner.String()+string(values[j].Kind)
		})
		return migrationHostDigest(values), nil
	}
	left, err := canonical(sets)
	if err != nil {
		return "", err
	}
	right, err := canonical(observed)
	...
	if err == nil {
		...
		if !hasContainer && target.mail != nil {
			...
			for _, intent := range entries {
				...
				if _, activateErr := target.mail.Activate(ctx, origin); activateErr != nil {
...
func (target *migrationHostTarget) VerifyActive(ctx context.Context, value migration.Migration, plan migration.Plan, receipt migration.ActivationReceipt) (migration.Verification, error) {
	...
	proofs = append(proofs, auxiliary...)
	certificates, err := target.certificateProofs(ctx, entries)
	if err != nil {
		return migration.Verification{}, err
	}
	proofs = append(proofs, certificates...)
	mailProofs, err := target.mailProofs(ctx, entries, true)
	if err != nil {
		return migration.Verification{}, err
	}
	...
	application, err := target.applicationProbe.VerifyActive(ctx, value, plan, receipt)
