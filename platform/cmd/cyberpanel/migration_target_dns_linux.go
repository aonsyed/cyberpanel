...
func (target *migrationHostTarget) activateEntries(ctx context.Context,value migration.Migration,plan migration.Plan,scope migration.RuntimeScope,entries []migration.ImportIntent)(migration.ActivationReceipt,error){
	...
	for _,intent:=range entries{switch intent.Kind{case migration.ImportSite:if _,err=target.probeSite(ctx,scope,intent,false);err!=nil{return migration.ActivationReceipt{},err};case migration.ImportDNSZone:if _,err=target.observeDNS(ctx,scope,intent,false);err!=nil{return migration.ActivationReceipt{},err};case migration.ImportDatabase:effect,observeErr:=target.observeDatabase(ctx,intent);if observeErr!=nil||effect.Status!=migration.ImportEffectApplied{return migration.ActivationReceipt{},errors.Join(migration.ErrBlocked,observeErr)};default:if !migrationAuxiliaryKind(intent.Kind)&&intent.Kind!=migration.ImportCertificate{return migration.ActivationReceipt{},migration.ErrBlocked}}}
	if _,err=target.auxiliaryProofs(ctx,entries,false);err!=nil{return migration.ActivationReceipt{},err}
	if _,err=target.certificateProofs(ctx,entries);err!=nil{return migration.ActivationReceipt{},err}
	...
	for _,intent:=range entries{if intent.Kind!=migration.ImportCertificate{continue};origin,loadErr:=target.auxiliaryOrigin(ctx,intent);if loadErr!=nil{return migration.ActivationReceipt{},errors.Join(migration.ErrAmbiguous,loadErr)};effect,applyErr:=target.certificate.Activate(ctx,origin);if applyErr!=nil||effect.Status!=migration.ImportEffectApplied||effect.EvidenceDigest==""{return migration.ActivationReceipt{},errors.Join(migration.ErrAmbiguous,applyErr)};routing=append(routing,effect.EvidenceDigest)}
	for _,intent:=range entries{if !migrationAuxiliaryKind(intent.Kind){continue};origin,loadErr:=target.auxiliaryOrigin(ctx,intent);if loadErr!=nil{return migration.ActivationReceipt{},errors.Join(migration.ErrAmbiguous,loadErr)};effect,applyErr:=target.auxiliary.Activate(ctx,origin);if applyErr!=nil||effect.Status!=migration.ImportEffectApplied||effect.EvidenceDigest==""{return migration.ActivationReceipt{},errors.Join(migration.ErrAmbiguous,applyErr)};routing=append(routing,effect.EvidenceDigest)}
...
func (target *migrationHostTarget) VerifyActive(ctx context.Context,value migration.Migration,plan migration.Plan,receipt migration.ActivationReceipt)(migration.Verification,error){
	...
	auxiliary,err:=target.auxiliaryProofs(ctx,entries,true);if err!=nil{return migration.Verification{},err};proofs=append(proofs,auxiliary...);certificates,err:=target.certificateProofs(ctx,entries);if err!=nil{return migration.Verification{},err};proofs=append(proofs,certificates...);proofs=append(proofs,migrationHostDigest(struct{Root,AbsentDomains string}{value.ManifestRoot,"mail,access_credentials,containers"}));verification:=migration.Verification{HTTP:true,Files:true,Database:true,DNS:true,Mail:true,Cron:true,Containers:true,Backups:true,EvidenceDigest:migrationHostDigest(proofs),ObservedAt:time.Now().UTC()}
