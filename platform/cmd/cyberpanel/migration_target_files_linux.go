...
func (target *migrationHostTarget) sitePayload(intent migration.ImportIntent)(migration.Site,error){
	var source migration.Site;if json.Unmarshal(intent.Payload,&source)!=nil||source.SourceID!=intent.SourceID||source.DocumentRootRelative!="public_html"||source.RuntimeKind!="php_lsapi"||len(source.Content)!=1||len(intent.Chunks)!=1||len(source.Children)!=0||len(source.Redirects)!=0||len(source.ResourceProfile)!=0||len(source.ContainerApplicationIDs)!=0||source.Content[0]!=intent.Chunks[0]{return source,migration.ErrBlocked}
