...
func assembleDomainServices(ctx context.Context, repositories controlRepositories, identityService *identity.Service, identityStore *identity.Store, auditService *audit.Service, mailHostname, panelRegistrableDomain, previewRegistrableDomain string) (apiserver.DomainServices, error) {
	...
	identityConsoleEdge, err := newIdentityEdge(identityService, identityStore)
	if err != nil {
	...
	if err = redisRepository.Init(ctx); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("bootstrap managed Redis repository: %w", err)
	}
	...
	mailConsoleEdge, err := newMailEdge(repositories.MailControl, mailClient)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize mail console edge: %w", err)
	}
	mailTelemetryReady := true
	if telemetryErr := mailConsoleEdge.enableTelemetry(ctx, repositories.Operations, operationsCoordinator, auditService); telemetryErr != nil {
		mailConsoleEdge.setTelemetryUnavailable("telemetry_initialization_failed")
		mailTelemetryReady = false
	}
	webmailKey, err := mail.LoadMailSessionCredential(mail.MailSessionCredentialPath)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("load webmail session authority: %w", err)
	}
	defer func() {
		for index := range webmailKey {
			webmailKey[index] = 0
		}
	}()
	webmailService, mailSessions, err := mail.NewLocalWebmailService(webmailKey, "cyberpanel-webmail", mailHostname, repositories.Webmail, repositories.MailControl)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize webmail runtime: %w", err)
	}
	legacyWebmailEdge, err := newWebmailEdge(webmailService, repositories.Webmail)
	if err != nil {
	...
	webmailBackend, err := securewebmail.NewDovecotBackend(securewebmail.Endpoint{TLSAddress: "127.0.0.1:993", TLSServerName: mailHostname, DialTimeout: 5 * time.Second, CommandTimeout: 20 * time.Second})
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize Dovecot OAuth backend: %w", err)
	}
	secureWebmailService, err := securewebmail.NewService(webmailRepository, secureWebmailDirectory{store: repositories.MailControl}, secureWebmailAudit{service: auditService}, webmailBackend, "cyberpanel-webmail", 90*time.Second)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize secure webmail service: %w", err)
	}
	const webmailBlobRoot = "/var/lib/cyberpanel/webmail/blobs"
	if err = os.MkdirAll(webmailBlobRoot, 0700); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("create webmail blob store: %w", err)
	}
	webmailBlobs, err := securewebmail.NewLocalBlobStore(webmailBlobRoot)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("open webmail blob store: %w", err)
	}
	webmailImages, err := securewebmail.NewHTTPRemoteImageProxy(net.DefaultResolver, &net.Dialer{Timeout: 5 * time.Second}, 64<<20)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize webmail image proxy: %w", err)
	}
	if err = secureWebmailService.ConfigureContent(securewebmail.ContentDependencies{Blobs: webmailBlobs, Scanner: localClamScanner{socket: "/run/clamd/cyberpanel.sock"}, Images: webmailImages, Submitter: localWebmailSubmitter{address: "127.0.0.1:25"}, Spam: failClosedSpamReporter{}}); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("configure secure webmail content: %w", err)
	}
	if err = secureWebmailService.Bootstrap(ctx); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("bootstrap secure webmail: %w", err)
	}
	...
	if err = activateWebmailDovecotOAuth(ctx, mailProjector, mailClient); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("activate Dovecot OAuth passdb: %w", err)
	}
	...
	unsubscribeKey, err := mail.LoadCampaignUnsubscribeCredential(mail.CampaignUnsubscribeCredentialPath)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("load campaign unsubscribe authority: %w", err)
	}
	defer func() {
		for index := range unsubscribeKey {
			unsubscribeKey[index] = 0
		}
	}()
	campaignSender, err := mail.NewLocalCampaignSender(repositories.Marketing, repositories.MailControl, repositories.MailDeliveryPolicy, mailHostname, unsubscribeKey, "https://"+panelRegistrableDomain+"/unsubscribe")
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize campaign delivery runtime: %w", err)
	}
	...
	emailMarketingOperations, err := apiserver.NewEmailMarketingOperations(ctx, repositories.ControlDB, identityService, auditService, unsubscribeKey, "/var/lib/cyberpanel/control/emailmarketing-archives")
	if err != nil {
	...
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize DNS console edge: %w", err)
	}
	certificateRuntime, err := certificates.NewLocalLinuxClientRuntimeForCurrentExecutable(nil, false, dnsAuthority)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize certificate runtime: %w", err)
	}
	var certificateMaterials *certificates.MaterialService
	if auditService != nil && auditService.Writer != nil {
		materialRepository := certificates.MaterialRepository{DB: repositories.ControlDB}
		if err = materialRepository.Bootstrap(ctx); err != nil {
			return apiserver.DomainServices{}, fmt.Errorf("bootstrap certificate material catalog: %w", err)
		}
		trustRoots, rootErr := x509.SystemCertPool()
		if rootErr != nil {
			return apiserver.DomainServices{}, fmt.Errorf("load certificate material trust roots: %w", rootErr)
		}
		if trustRoots == nil {
			return apiserver.DomainServices{}, fmt.Errorf("certificate material trust roots unavailable")
		}
	...
	if err = certificateRenewal.Bootstrap(ctx); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("bootstrap certificate renewal: %w", err)
	}
	certificateConsoleEdge, err := newCertificateEdge(certificateRuntime, certificateIssuance, certificateDeployment)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize certificate console edge: %w", err)
	}
	accessClient, err := access.NewLocalAccessClient()
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("connect access executor: %w", err)
	}
	...
	if err = emailMarketingOperations.ConfigureContentRuntime(fileService, localMarketingDelivery, unsubscribeKey); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize email marketing content runtime: %w", err)
	}
	...
	accessConsoleEdge, err := newAccessEdge(credentialService, fileService, repositories.Access, repositories.Hosting)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize access console edge: %w", err)
	}
	...
	desiredWebTuning, err := management.DesiredTuning(defaultWebTuning)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("construct default web-engine tuning: %w", err)
	}
	...
	webManagementRuntime, err := management.NewRuntime(catalog, activationClient, ols.New(), enterprise.New())
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize web-engine management runtime: %w", err)
	}
	webManagement, err := management.New(repositories.WebEngine, webManagementRuntime, webManagementRuntime, webManagementRuntime)
	if err != nil {
	...
	if _, err = repositories.WebEngine.EnsureInstallation(ctx, observedInstallation); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("reconcile web-engine installation projection: %w", err)
	}
	webEngineConsoleEdge, err := newWebEngineEdge(webManagement, webengine.Edition(installedEdition))
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize web-engine console edge: %w", err)
	}
	webAccessCredentials, err := accesspolicy.NewLocalCredentialSource()
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("connect web access credential broker: %w", err)
	}
	webAccessPolicies, err := accesspolicy.New(catalog, activationClient, webAccessCredentials, ols.New(), enterprise.New())
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize web access policy authority: %w", err)
	}
	...
	containerBroker, err := containers.NewLocalContainerBrokerClient()
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("connect container broker: %w", err)
	}
	containerPolicy, err := containers.NewDefaultPolicy()
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize container policy: %w", err)
	}
	containerRoutes, err := containerproxy.New(catalog, activationClient, ols.New(), enterprise.New())
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize container route authority: %w", err)
	}
	containerBackups, err := containers.NewBrokerVolumeBackupCoordinator(repositories.Containers, containerBroker)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize container volume backup coordinator: %w", err)
	}
	containerService, err := containers.NewService(repositories.Containers, containerBroker, containerPolicy, containerRoutes, containerBackups)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize container service: %w", err)
	}
	recipeVerifier, err := containers.LoadDefaultLinuxRecipeVerifier()
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("load container recipe trust: %w", err)
	}
	containerIDs, err := containers.NewDeterministicIDAllocator("local")
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize container id allocator: %w", err)
	}
	containerApplications, err := containers.NewApplicationService(containerService, repositories.Containers, recipeVerifier, containerIDs)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize container applications: %w", err)
	}
	packagedN8N, err := containers.ReadPackagedN8NRecipe(ctx, recipeVerifier)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("load packaged n8n recipe: %w", err)
	}
	if err = integrations.ValidateN8NApplicationRecipe(packagedN8N); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("validate packaged n8n contract: %w", err)
	}
	...
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("connect mail-delivery material broker: %w", err)
	}
	mailDeliveryConsumerDigest, err := certificates.CurrentExecutableDigest()
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("digest mail-delivery consumer: %w", err)
	}
	mailDeliveryRuntime, err := maildelivery.NewCyberMailLocalRuntimeV1(maildelivery.CyberMailLocalRuntimeV1Options{Repository: repositories.MailDelivery, Material: mailDeliveryMaterial, Management: secretEnrollment.client, HelloName: mailHostname, ConsumerReleaseDigest: mailDeliveryConsumerDigest, Now: runtimeClock{}.Now})
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize CyberMail adapter: %w", err)
	}
	mailDeliveryAuthorizer, err := identity.NewAuthorizer(identityStore)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize mail-delivery authorization: %w", err)
	}
	mailDeliveryService, err := maildelivery.NewService(maildelivery.ServiceConfig{Repository: repositories.MailDelivery, Authorizer: cyberMailAuthorization{authorizer: mailDeliveryAuthorizer, now: runtimeClock{}.Now}, StepUp: cyberMailAuthorization{authorizer: mailDeliveryAuthorizer, now: runtimeClock{}.Now}, Consent: cyberMailConsent{}, Audit: cyberMailAudit{writer: auditService.Writer}, Providers: mailDeliveryRuntime.Registry, ProviderConsumerReleaseDigest: mailDeliveryConsumerDigest, Now: runtimeClock{}.Now})
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize mail-delivery service: %w", err)
	}
	mailConsumerDigest, err := webEngineExecutableDigest("/usr/local/libexec/cyberpanel/panel-execd")
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("digest OpenDKIM material consumer: %w", err)
	}
	mailRotation, err := mail.NewDKIMRotationService(repositories.MailControl, mailProjector, cyberpanelDKIMRuntime{client: mailClient}, secretEnrollment.client, mail.SystemDKIMTXTObserver{}, mailConsumerDigest, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize DKIM rotation: %w", err)
	}
	mailCoordinator.DKIMRotation = mailRotation
	providerDigest, err := integrations.ProviderWorkerReleaseDigest("")
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("digest provider worker: %w", err)
	}
	providerClients := map[integrations.ProviderKind]integrations.Provider{}
	for _, kind := range []integrations.ProviderKind{integrations.ProviderCloudflare, integrations.ProviderAWSS3, integrations.ProviderWasabi, integrations.ProviderBackblaze} {
		client, clientErr := integrations.NewLocalProviderWorkerClient(kind)
		if clientErr != nil {
			return apiserver.DomainServices{}, fmt.Errorf("connect %s provider worker: %w", kind, clientErr)
		}
		providerClients[kind] = client
	}
	providerRegistrations, err := integrations.DefaultProviderWorkerRegistrations(providerDigest, providerClients)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("register provider workers: %w", err)
	}
	integrationRuntime, err := integrations.NewRuntime(repositories.Integrations, secretEnrollment.client, providerRegistrations, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize integration runtime: %w", err)
	}
	integrationProfiles := map[string]integrationEdgeProfile{}
	for _, registration := range providerRegistrations {
		integrationProfiles[registration.PublicName] = integrationEdgeProfile{Kind: registration.Kind, Purpose: registration.Purpose, Endpoint: registration.Endpoint}
	}
	integrationConsoleEdge, err := newIntegrationEdge(repositories.Integrations, integrationRuntime.Bindings, integrationProfiles)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize integration console edge: %w", err)
	}
	applicationClient, err := apps.NewLocalLinuxApplicationClient()
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("connect application executor: %w", err)
	}
	applicationCatalog, applicationCatalogAuthority, err := apps.NewLinuxPinnedApplicationCatalog("")
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize application catalog: %w", err)
	}
	releaseDigest, err := apps.LinuxApplicationExecutorDigest()
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("digest application consumer: %w", err)
	}
	applicationSecrets, err := apps.NewApplicationSecretIssuer(secretEnrollment.client, repositories.Applications, releaseDigest)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize application secret issuer: %w", err)
	}
	applicationDatabases, err := apps.NewApplicationDatabaseProvisioner(databaseCoordinator, repositories.Database, secretEnrollment.client, releaseDigest)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize application database provisioner: %w", err)
	}
	applicationRecovery := &apps.LinuxApplicationRecoveryProvider{Client: applicationClient, Store: repositories.Applications}
	securityConsoleEdge, err := newSecurityEdge(repositories.Applications, applicationClient, applicationRecovery, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize security console edge: %w", err)
	}
	...
	hostingConsoleEdge, err := newHostingEdge(hostingCoordinator, repositories.Hosting, webAccessPolicies, runtimeClock{}.Now)
	if err != nil {
	...
	previewRepository, err := preview.NewRepository(repositories.ControlDB, previewRegistrableDomain)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("open preview session authority: %w", err)
	}
	if err = previewRepository.Bootstrap(ctx); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("bootstrap preview session authority: %w", err)
	}
	previewSessions, err := preview.NewService(previewRepository, repositories.Hosting, hostingCoordinator, panelRegistrableDomain, previewRegistrableDomain)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize preview session authority: %w", err)
	}
	hostingPreviewConsoleEdge, err := newHostingPreviewEdge(previewSessions)
	if err != nil {
	...
	backupConsoleEdge, err := newBackupEdge(backupRuntime.Catalog, &restoreWorkflow, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize backup console edge: %w", err)
	}
	...
	migrationSecretFactory := func(ctx context.Context, db *sql.DB, scopes *migration.RuntimeScopeStore) (migration.MigrationSecretGateway, error) {
		value, createErr := newMigrationSecretTarget(ctx, db, scopes)
		if createErr != nil {
			return nil, createErr
		}
		migrationSecrets = value
		return value, nil
	}
	migrationDatabaseFactory := func(ctx context.Context, db *sql.DB, chunks *migration.ChunkStore, scopes *migration.RuntimeScopeStore) (migration.CanonicalImportHandler, error) {
		if migrationSecrets == nil {
			return nil, migration.ErrBlocked
		}
		resolveSecret := func(ctx context.Context, id migration.ID, envelope string) (migration.DatabaseImportCredential, error) {
			metadata, resolveErr := migrationSecrets.Resolve(ctx, id, envelope)
			if resolveErr != nil {
				return migration.DatabaseImportCredential{}, resolveErr
			}
			ref, resolveErr := database.NewSecretRef(metadata.ID.String())
			if resolveErr != nil {
				return migration.DatabaseImportCredential{}, resolveErr
			}
			credential := migration.DatabaseImportCredential{SecretRef: ref}
			switch metadata.Audience.ResourceKind {
			case "database_principal":
			case "database_principal_native_hash":
				credential.Format = database.CredentialFormatNativeHash
			default:
				return migration.DatabaseImportCredential{}, migration.ErrBlocked
			}
			return credential, nil
		}
		resolveSite := func(ctx context.Context, id, source migration.ID) (site.SiteID, error) {
			value, loadErr := repositories.Migrations.Migration(ctx, id)
			if loadErr != nil {
				return site.SiteID{}, loadErr
			}
			plan, loadErr := repositories.Migrations.Plan(ctx, value.PlanDigest)
			if loadErr != nil {
				return site.SiteID{}, loadErr
			}
			if plan.ApprovedAt == nil || plan.ApprovalDigest == "" || plan.MigrationID != id {
				return site.SiteID{}, migration.ErrBlocked
			}
			for _, mapping := range plan.Mappings {
				if mapping.SourceKind == string(migration.ImportSite) && mapping.SourceID == source && mapping.Disposition == migration.DispositionCreate {
					return site.NewSiteID(mapping.TargetID.String())
				}
			}
			return site.SiteID{}, migration.ErrBlocked
		}
	...
	migrationScopes, err := migration.NewRuntimeScopeStore(repositories.ControlDB)
	if err != nil {
		return apiserver.DomainServices{}, err
	}
	...
	migrationCertificateFactory := func(ctx context.Context, db *sql.DB, chunks *migration.ChunkStore, scopes *migration.RuntimeScopeStore) (*migrationCertificateTarget, error) {
		value, createErr := newMigrationCertificateTarget(ctx, db, chunks, scopes, certificateMaterials, certificateRuntime.Secrets, certificateDeployment, migrationSecrets, nil)
		if createErr != nil {
			return nil, createErr
		}
	...
	migrationFactory := migrationTargetFactory(hostingCoordinator, repositories.Hosting, fileService, accessClient, dnsAuthority, repositories.Migrations, migrationProbe, containerApplications, catalog, activationClient, migrationSecretFactory, migrationDatabaseFactory, migrationAuxiliaryServices{git: gitService, cron: cronService, backup: backupRuntime}, migrationCertificateFactory, repositories.MailControl, mailProjector, mailClient)
	...
	migrationConsoleEdge, err := newMigrationEdge(migrationRuntime, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize migration console edge: %w", err)
	}
	haApprovalAuthority, _ := newHAPromotionApprovalAuthority(&repositories.HA, identityStore, runtimeClock{}.Now)
	haRemoteSender, err := newHAFederatedSender(ctx, repositories.ControlDB, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize federated HA sender: %w", err)
	}
	localHAProviders, err := newLocalMariaDBHAProviders(ctx, &repositories.HA, databaseExecutor, databaseExecutor, catalog, activationClient, configuration.Engine.Listeners, haApprovalAuthority, haRemoteSender, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize local MariaDB HA providers: %w", err)
	}
	if err = startHAPeerVoting(ctx, repositories.ControlDB, localHAProviders, runtimeClock{}.Now); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize protected HA peer voting: %w", err)
	}
	fleetHAConsoleEdge, err := newFleetHAEdge(&repositories.HA, localHAProviders, haApprovalAuthority, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize fleet and HA console edge: %w", err)
	}
	federationConsoleEdge, err := newFederationEdge(ctx, repositories.ControlDB, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize federation console edge: %w", err)
	}
	maintenanceRepository, err := maintenance.NewRepository(repositories.ControlDB)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("open maintenance-window repository: %w", err)
	}
	if err = maintenanceRepository.Bootstrap(ctx); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("bootstrap maintenance-window repository: %w", err)
	}
	maintenanceEvaluator, err := maintenance.NewEvaluator(maintenanceRepository, maintenance.EvaluatorConfig{})
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize maintenance-window evaluator: %w", err)
	}
	maintenanceWindows, err := maintenance.NewWindowService(maintenanceRepository, maintenanceEvaluator, maintenance.WindowServiceConfig{Now: runtimeClock{}.Now})
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize maintenance-window service: %w", err)
	}
	productUpdateEdge, err := assembleProductUpdateEdge(ctx, repositories.ControlDB, auditService, runtimeClock{})
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize product-update catalog: %w", err)
	}
	packageMaintenanceEdge, err := assemblePackageMaintenanceEdge(ctx, repositories.ControlDB, runtimeClock{}.Now)
	if err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize package-maintenance runtime: %w", err)
	}
	if err = migrationRuntime.StartChunkMaintenance(ctx, migrationChunkMaintenanceAudit{service: auditService}); err != nil {
	...
	if err = startFederationRuntime(ctx, repositories.ControlDB, localHAProviders); err != nil {
		return apiserver.DomainServices{}, fmt.Errorf("initialize federation runtime: %w", err)
	}
...
func (policy certificateMaterialPolicy) ResolveMaterialImportPolicy(ctx context.Context, principal certificates.MaterialPrincipal, scope certificates.MaterialScope) (certificates.MaterialImportPolicy, error) {
	if ctx == nil || policy.trustRoots == nil || principal.TenantID == "" || principal.SubjectID == "" || principal.TenantID != scope.TenantID || scope.ResourceID == "" {
		return certificates.MaterialImportPolicy{}, certificates.ErrMaterialUnauthorized
	}
...
func (policy certificateMaterialPolicy) ResolveSelfSignedMaterialPolicy(ctx context.Context, principal certificates.MaterialPrincipal, scope certificates.MaterialScope) (certificates.SelfSignedMaterialPolicy, error) {
	if ctx == nil || policy.trustRoots == nil || principal.TenantID == "" || principal.SubjectID == "" || principal.TenantID != scope.TenantID || scope.ResourceID == "" {
		return certificates.SelfSignedMaterialPolicy{}, certificates.ErrMaterialUnauthorized
	}
...
func (sink certificateMaterialAuditSink) RecordMaterialAudit(ctx context.Context, source certificates.MaterialAuditEvent) error {
	if ctx == nil || sink.service == nil || sink.service.Writer == nil || source.EventID == "" || source.TenantID == "" || source.SubjectID == "" || source.Action == "" || source.ResourceID == "" || source.OccurredAt.IsZero() {
		return audit.ErrInvalid
	}
	class, outcome := audit.ClassMutation, audit.OutcomeFailed
	switch source.Outcome {
	case "admitted":
		class, outcome = audit.ClassAuthorization, audit.OutcomeAllowed
	case "denied":
		class, outcome = audit.ClassAuthorization, audit.OutcomeDenied
	case "succeeded":
		outcome = audit.OutcomeApplied
		if source.Action == certificates.MaterialActionInspect || source.Action == certificates.MaterialActionList {
	...
	targetID := source.MaterialID
	if targetID == "" {
		targetID = source.ResourceID
	}
	attributes := map[string]string{}
	if source.ReasonCode != "" {
		attributes["reason_code"] = source.ReasonCode
	}
	if source.CertificateFingerprint != "" {
		attributes["certificate_fingerprint_sha256"] = source.CertificateFingerprint
	}
...
func (runtime cyberpanelDKIMRuntime) ApplyDKIMGeneration(ctx context.Context, effect mail.EffectRequest, generation mail.ConfigGeneration) (mail.DKIMRuntimeReceipt, error) {
	receipt := mail.DKIMRuntimeReceipt{}
	if runtime.client == nil {
		return receipt, mail.ErrInvalidCommand
	}
	applied, activation, err := runtime.client.ApplyGeneration(ctx, effect, generation)
	receipt.Effect = applied
	receipt.GenerationDigest = activation.GenerationDigest
	receipt.RolledBack = activation.RolledBack
	if err != nil {
		return receipt, err
	}
	reload, reloadErr := runtime.client.ControlService(ctx, mail.ServiceOpenDKIM, mail.ServiceReload)
	receipt.OpenDKIMReloadDigest = reload.EvidenceDigest
	probe, probeErr := runtime.client.ControlService(ctx, mail.ServiceOpenDKIM, mail.ServiceProbe)
	receipt.OpenDKIMProbeDigest = probe.EvidenceDigest
	receipt.OpenDKIMActive = probe.Active
	if reloadErr != nil {
		return receipt, reloadErr
	}
	if probeErr != nil {
		return receipt, probeErr
	}
	if !probe.Active {
		return receipt, mail.ErrInvalidReceipt
	}
	return receipt, nil
}

type integratedWebmailEdge struct {
	secure *securewebmail.Service
	legacy apiserver.WebmailEdgeService
}

func (edge integratedWebmailEdge) SecureWebmail() *securewebmail.Service { return edge.secure }
func (edge integratedWebmailEdge) ListContacts(ctx context.Context, call apiserver.EdgeCall, session mail.MailSession, page apiserver.EdgePagePayload) (apiserver.EdgePage[mail.Contact], error) {
	if edge.legacy == nil {
		return apiserver.EdgePage[mail.Contact]{}, mail.ErrUnauthorized
	}
	return edge.legacy.ListContacts(ctx, call, session, page)
}
func (edge integratedWebmailEdge) ListSieveRules(ctx context.Context, call apiserver.EdgeCall, session mail.MailSession, page apiserver.EdgePagePayload) (apiserver.EdgePage[mail.SieveRule], error) {
	if edge.legacy == nil {
		return apiserver.EdgePage[mail.SieveRule]{}, mail.ErrUnauthorized
	}
	return edge.legacy.ListSieveRules(ctx, call, session, page)
}
...
func (directory secureWebmailDirectory) ListMailAccounts(ctx context.Context, _ securewebmail.Principal, tenant string) ([]securewebmail.AuthorizedMailAccount, error) {
	mailboxes, next, err := directory.store.List(ctx, tenant, mail.ResourceMailbox, 500, "")
	if err != nil {
		return nil, err
	}
	if next != "" {
		return nil, securewebmail.ErrLimit
	}
	domains, next, err := directory.store.List(ctx, tenant, mail.ResourceDomain, 500, "")
	if err != nil {
		return nil, err
	}
	if next != "" {
	...
	domainNames := make(map[mail.DomainID]string, len(domains))
	for _, resource := range domains {
		if resource.State != mail.StateActive {
			continue
		}
		var domain mail.Domain
		if json.Unmarshal(resource.Spec, &domain) != nil || domain.ID == "" || domain.Tenant != tenant || domain.Name == "" {
			return nil, securewebmail.ErrProtocol
		}
		domainNames[domain.ID] = domain.Name
	}
	accounts := make([]securewebmail.AuthorizedMailAccount, 0, len(mailboxes))
	for _, resource := range mailboxes {
		if resource.State != mail.StateActive {
			continue
		}
		var mailbox mail.Mailbox
		if json.Unmarshal(resource.Spec, &mailbox) != nil || string(mailbox.ID) != resource.ID || !mailbox.Enabled || resource.Generation == 0 {
			continue
		}
		domain := domainNames[mailbox.Domain]
		if domain == "" {
			continue
		}
		address := strings.ToLower(mailbox.Local + "@" + domain)
		accounts = append(accounts, securewebmail.AuthorizedMailAccount{TenantID: tenant, MailboxID: resource.ID, DisplayLabel: address, AddressLabel: address, AuthorizationEpoch: resource.Generation, Enabled: true})
	}
	return accounts, nil
}
...
func (directory secureWebmailDirectory) AuthorizeMailbox(ctx context.Context, _ securewebmail.Principal, tenant, mailboxID string) (securewebmail.AuthorizedMailAccount, error) {
	resource, found, err := directory.store.Load(ctx, tenant, mail.ResourceMailbox, mailboxID)
	if err != nil {
		return securewebmail.AuthorizedMailAccount{}, err
	}
	if !found || resource.State != mail.StateActive || resource.Generation == 0 {
		return securewebmail.AuthorizedMailAccount{}, securewebmail.ErrNotFound
	}
	var mailbox mail.Mailbox
	if json.Unmarshal(resource.Spec, &mailbox) != nil || string(mailbox.ID) != mailboxID || !mailbox.Enabled {
		return securewebmail.AuthorizedMailAccount{}, securewebmail.ErrNotFound
	}
	domainResource, found, err := directory.store.Load(ctx, tenant, mail.ResourceDomain, string(mailbox.Domain))
	if err != nil {
		return securewebmail.AuthorizedMailAccount{}, err
	}
	if !found || domainResource.State != mail.StateActive {
		return securewebmail.AuthorizedMailAccount{}, securewebmail.ErrNotFound
	}
	var domain mail.Domain
	if json.Unmarshal(domainResource.Spec, &domain) != nil || domain.ID != mailbox.Domain || domain.Tenant != tenant || domain.Name == "" {
		return securewebmail.AuthorizedMailAccount{}, securewebmail.ErrProtocol
	}
	address := strings.ToLower(mailbox.Local + "@" + domain.Name)
	return securewebmail.AuthorizedMailAccount{TenantID: tenant, MailboxID: mailboxID, DisplayLabel: address, AddressLabel: address, AuthorizationEpoch: resource.Generation, Enabled: true}, nil
}
...
func (sink secureWebmailAudit) RecordWebmailSecurity(ctx context.Context, event securewebmail.AuditEvent) error {
	if sink.service == nil {
		return securewebmail.ErrUnavailable
	}
	sum := sha256.Sum256([]byte(event.RequestID + "\x00" + event.Operation + "\x00" + event.MailboxDigest + "\x00" + event.OccurredAt.UTC().Format(time.RFC3339Nano)))
	request := sha256.Sum256([]byte(event.RequestID))
...
func (submitter localWebmailSubmitter) Submit(ctx context.Context, envelope securewebmail.SubmissionEnvelope, source io.Reader, maximum uint64) (string, error) {
	if ctx == nil || source == nil || submitter.address != "127.0.0.1:25" || maximum == 0 || maximum > securewebmail.MaximumComposeBytes {
		return "", securewebmail.ErrInvalid
	}
	connection, err := (&net.Dialer{Timeout: 5 * time.Second}).DialContext(ctx, "tcp", submitter.address)
	if err != nil {
		return "", errors.Join(securewebmail.ErrUnavailable, err)
	}
	...
	if err = webmailSMTPCommand(protocol, 250, "EHLO localhost"); err != nil {
		return "", errors.Join(securewebmail.ErrUnavailable, err)
	}
	if err = webmailSMTPCommand(protocol, 250, "MAIL FROM:<"+envelope.From+">"); err != nil {
		return "", errors.Join(securewebmail.ErrUnavailable, err)
	}
	for _, recipient := range envelope.Recipients {
		if err = webmailSMTPCommand(protocol, 250, "RCPT TO:<"+recipient+">"); err != nil {
			return "", errors.Join(securewebmail.ErrUnavailable, err)
		}
	}
	if err = webmailSMTPCommand(protocol, 354, "DATA"); err != nil {
		return "", errors.Join(securewebmail.ErrUnavailable, err)
	}
	writer := protocol.DotWriter()
	written, copyErr := io.Copy(writer, io.LimitReader(source, int64(maximum)+1))
	if copyErr != nil || uint64(written) > maximum {
		if uint64(written) > maximum {
			return "", securewebmail.ErrLimit
		}
		return "", errors.Join(securewebmail.ErrUnavailable, copyErr)
	}
	if err = writer.Close(); err != nil {
		return "", errors.Join(securewebmail.ErrPartial, err)
	}
	_, reply, err := protocol.ReadResponse(250)
	if err != nil {
	...
	_, _ = protocol.Cmd("QUIT")
	marker := "queued as "
	index := strings.LastIndex(strings.ToLower(reply), marker)
	if index < 0 {
		return "", errors.Join(securewebmail.ErrPartial, securewebmail.ErrProtocol)
	}
	fields := strings.Fields(reply[index+len(marker):])
	if len(fields) == 0 {
		return "", errors.Join(securewebmail.ErrPartial, securewebmail.ErrProtocol)
	}
	queue := strings.Trim(fields[0], ".[]()")
	if !postfixQueuePattern.MatchString(queue) {
		return "", errors.Join(securewebmail.ErrPartial, securewebmail.ErrProtocol)
	}
	return queue, nil
}

func webmailSMTPCommand(connection *textproto.Conn, code int, command string) error {
	id, err := connection.Cmd("%s", command)
	if err != nil {
		return err
	}
	connection.StartResponse(id)
	defer connection.EndResponse(id)
	_, _, err = connection.ReadResponse(code)
	return err
}
...
func (scanner localClamScanner) Scan(ctx context.Context, source io.Reader, maximum uint64) (string, error) {
	if ctx == nil || source == nil || scanner.socket != "/run/clamd/cyberpanel.sock" || maximum == 0 || maximum > securewebmail.MaximumAttachmentBytes {
		return "", securewebmail.ErrInvalid
	}
	connection, err := (&net.Dialer{Timeout: 5 * time.Second}).DialContext(ctx, "unix", scanner.socket)
	if err != nil {
		return "", errors.Join(securewebmail.ErrUnavailable, err)
	}
	defer connection.Close()
	...
	limited := io.LimitReader(source, int64(maximum)+1)
	var total uint64
	for {
	...
	reply, err := bufio.NewReaderSize(connection, 4096).ReadString(0)
...
func (failClosedSpamReporter) Report(context.Context, securewebmail.BlobOwner, []securewebmail.MessageIdentity, bool) error {
	return securewebmail.ErrUnavailable
}

type webmailTokenInfo struct {
	database  *sql.DB
...
func activateWebmailDovecotOAuth(ctx context.Context, projector mail.RepositorySnapshotProjector, client *mail.MailDaemonClient) error {
	if ctx == nil || client == nil {
		return mail.ErrInvalidCommand
	}
	request := mail.EffectRequest{CommandID: "startup-webmail-dovecot-oauth", TenantID: "system", Kind: mail.ResourcePolicy, ResourceID: "webmail-dovecot-oauth", Generation: 1, Action: mail.ActionUpdate}
	snapshot, err := projector.ProjectMail(ctx, request)
	if err != nil {
		return err
	}
	generation, err := (mail.ConfigRenderer{}).Render(snapshot)
	if err != nil {
		return err
	}
	binding := sha256.Sum256([]byte("webmail-dovecot-oauth-activation-v1\x00" + generation.Digest))
	request.EffectID = "mailfx_" + hex.EncodeToString(binding[:])[:48]
	request.CommandDigest = generation.Digest
	request.DesiredDigest = generation.Digest
	effect, activation, err := client.ApplyGeneration(ctx, request, generation)
	if err != nil {
		return err
	}
	if effect.Outcome != mail.EffectConfirmed || effect.AppliedGeneration != activation.GenerationDigest || effect.ProbeDigest == "" {
		return mail.ErrInvalidReceipt
	}
	return nil
}

func bootstrapWebmailDovecotTokens(ctx context.Context, database *sql.DB) error {
	if ctx == nil || database == nil {
		return securewebmail.ErrInvalid
	}
	_, err := database.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS webmail_dovecot_tokens_v1(token_digest TEXT PRIMARY KEY CHECK(length(token_digest)=64),tenant_id TEXT NOT NULL,mailbox_id TEXT NOT NULL,authz_epoch INTEGER NOT NULL,expires_at INTEGER NOT NULL,consumed_at INTEGER NOT NULL);
CREATE TRIGGER IF NOT EXISTS webmail_dovecot_grant_consumed_v1 AFTER UPDATE OF consumed_at ON webmail_grants_v1 WHEN NEW.consumed_at IS NOT NULL AND OLD.consumed_at IS NULL BEGIN INSERT OR REPLACE INTO webmail_dovecot_tokens_v1(token_digest,tenant_id,mailbox_id,authz_epoch,expires_at,consumed_at) VALUES(NEW.token_digest,NEW.tenant_id,NEW.mailbox_id,NEW.authz_epoch,NEW.expires_at,NEW.consumed_at); END;`)
	...
	_, err = database.ExecContext(ctx, `DELETE FROM webmail_dovecot_tokens_v1 WHERE expires_at<=?`, time.Now().UTC().UnixNano())
	return err
}

func serveWebmailTokenInfo(ctx context.Context, database *sql.DB, store mail.SQLControlRepository) error {
	if ctx == nil || database == nil || store.DB == nil {
		return securewebmail.ErrInvalid
	}
	listener, err := net.Listen("tcp", "127.0.0.1:18090")
	if err != nil {
		return err
	}
	server := &http.Server{Handler: webmailTokenInfo{database: database, directory: secureWebmailDirectory{store: store}}, ReadHeaderTimeout: 3 * time.Second, ReadTimeout: 5 * time.Second, WriteTimeout: 5 * time.Second, IdleTimeout: 5 * time.Second, MaxHeaderBytes: 8 << 10}
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				...
				return
			case now := <-ticker.C:
				_, _ = database.ExecContext(ctx, `DELETE FROM webmail_dovecot_tokens_v1 WHERE expires_at<=?`, now.UTC().UnixNano())
			}
		}
	}()
...
func (handler webmailTokenInfo) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	active := func() { writer.WriteHeader(http.StatusUnauthorized); _, _ = writer.Write([]byte(`{"active":false}`)) }
	...
	if token == "" && request.Method == http.MethodPost {
	...
	err := handler.database.QueryRowContext(request.Context(), `SELECT tenant_id,mailbox_id,authz_epoch,expires_at,consumed_at FROM webmail_dovecot_tokens_v1 WHERE token_digest=? AND expires_at>?`, digestString, now.UnixNano()).Scan(&tenant, &mailbox, &epoch, &expires, &consumed)
	if err != nil || consumed <= 0 || expires <= now.UnixNano() {
		active()
		return
	}
	account, err := handler.directory.AuthorizeMailbox(request.Context(), securewebmail.Principal{UserID: "dovecot", SessionID: "introspection"}, tenant, mailbox)
	if err != nil || account.AuthorizationEpoch != epoch {
		active()
		return
	}
	result, err := handler.database.ExecContext(request.Context(), `DELETE FROM webmail_dovecot_tokens_v1 WHERE token_digest=? AND consumed_at=? AND expires_at>?`, digestString, consumed, now.UnixNano())
	if err != nil {
		active()
		return
	}
	...
	_, _ = handler.database.ExecContext(request.Context(), `UPDATE webmail_grants_v1 SET revoked_at=? WHERE token_digest=? AND consumed_at=? AND revoked_at IS NULL`, now.UnixNano(), digestString, consumed)
	writer.WriteHeader(http.StatusOK)
	_ = json.NewEncoder(writer).Encode(map[string]any{"active": true, "email": account.AddressLabel, "username": account.AddressLabel, "scope": "imap"})
}
