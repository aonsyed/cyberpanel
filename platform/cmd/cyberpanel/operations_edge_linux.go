...
func(edge *operationsEdge)recordRedisObservation(ctx context.Context,spec redisservice.InstanceSpec,executionErr error)error{
	...
	if executionErr==nil&&spec.DesiredLifecycle==redisservice.LifecycleRemoved{observation.Lifecycle=redisservice.LifecycleStopped;observation.Health=redisservice.HealthDegraded;observation.Drift=redisservice.DriftPresent}
	if executionErr!=nil{
...
func(edge *operationsEdge)RecordRedisBackup(ctx context.Context,call apiserver.EdgeCall,artifact redisservice.ArtifactDescriptor)(apiserver.RedisBackupResult,error){
	edge.redisMu.Lock();defer edge.redisMu.Unlock()
	record,err:=edge.redisRecord(ctx,call);if err!=nil{return apiserver.RedisBackupResult{},err};now:=edge.now().UTC();if call.ExpectedGeneration==0||call.ExpectedGeneration!=record.Spec.Generation||record.Observed==nil||record.Observed.Lifecycle!=redisservice.LifecycleRunning||record.Observed.Health!=redisservice.HealthHealthy||record.Observed.Drift!=redisservice.DriftNone||record.Observed.ConfigGeneration!=record.Spec.ConfigGeneration{return apiserver.RedisBackupResult{},redisservice.ErrStale}
	if artifact.InstanceID!=record.Spec.ID||artifact.RedisVersion!=record.Spec.Support.RedisVersion||artifact.ConfigGeneration!=record.Spec.ConfigGeneration||artifact.CreatedAt.After(now){return apiserver.RedisBackupResult{},redisservice.ErrInvalid}
	if record.Spec.Backup.Frequency==redisservice.BackupDisabled||artifact.Kind==redisservice.ArtifactRDB&&record.Spec.Persistence.RDB==redisservice.RDBDisabled||artifact.Kind==redisservice.ArtifactAOF&&(!record.Spec.Backup.IncludeAOF||record.Spec.Persistence.AOF==redisservice.AOFDisabled){return apiserver.RedisBackupResult{},redisservice.ErrUnsupported}
	...
	if err=edge.redis.PutArtifact(ctx,artifact);err!=nil{return apiserver.RedisBackupResult{},err};return apiserver.RedisBackupResult{State:"recorded",Artifact:artifact,Generation:record.Spec.Generation},nil
}
...
func(edge *operationsEdge)PlanRedisRestore(ctx context.Context,call apiserver.EdgeCall,payload apiserver.RedisRestorePayload)(apiserver.RedisRestorePlan,error){
	...
	expectedKind:=redisservice.ArtifactRDB;if record.Spec.Persistence.AOF!=redisservice.AOFDisabled{expectedKind=redisservice.ArtifactAOF};if artifact.InstanceID!=record.Spec.ID||recovery.InstanceID!=record.Spec.ID||artifact.RedisVersion!=record.Spec.Support.RedisVersion||recovery.RedisVersion!=record.Spec.Support.RedisVersion||artifact.Kind!=expectedKind||recovery.Kind!=expectedKind||recovery.ConfigGeneration!=record.Spec.ConfigGeneration{return apiserver.RedisRestorePlan{},redisservice.ErrUnsupported}
	snapshot,err:=edge.redis.ConsumerSnapshot(ctx,record.Spec.ID);if err!=nil{return apiserver.RedisRestorePlan{},err};return apiserver.RedisRestorePlan{State:"preflight_ready",InstanceID:record.Spec.ID,Artifact:artifact,RecoveryArtifact:recovery,ExpectedGeneration:record.Spec.Generation,ConsumerGeneration:snapshot.Generation},nil
}
...
func(edge *operationsEdge)DeleteRedis(ctx context.Context,call apiserver.EdgeCall,recoveryArtifactRef string)(apiserver.RedisMutation,error){
	edge.redisMu.Lock();defer edge.redisMu.Unlock()
	record,err:=edge.redisRecord(ctx,call);if err!=nil{return apiserver.RedisMutation{},err};if call.ExpectedGeneration==0||call.ExpectedGeneration!=record.Spec.Generation||recoveryArtifactRef==""{return apiserver.RedisMutation{},redisservice.ErrConflict};if err=edge.guardRedisConsumers(ctx,record.Spec.ID,true);err!=nil{return apiserver.RedisMutation{},err}
	recovery,err:=edge.redis.Artifact(ctx,recoveryArtifactRef);if err!=nil{return apiserver.RedisMutation{},err};expectedKind:=redisservice.ArtifactRDB;if record.Spec.Persistence.AOF!=redisservice.AOFDisabled{expectedKind=redisservice.ArtifactAOF};if recovery.InstanceID!=record.Spec.ID||recovery.RedisVersion!=record.Spec.Support.RedisVersion||recovery.Kind!=expectedKind||recovery.ConfigGeneration!=record.Spec.ConfigGeneration{return apiserver.RedisMutation{},redisservice.ErrStale}
	if record.Spec.DesiredLifecycle==redisservice.LifecycleRemoved{return edge.finishRedisTarget(ctx,call,record.Spec,"-redis-delete-reconcile")}
	target:=record.Spec;target.DesiredLifecycle=redisservice.LifecycleRemoved;target.Generation++;target.UpdatedAt=edge.now().UTC();target.Digest="";target,err=redisservice.SealInstanceSpec(target);if err!=nil{return apiserver.RedisMutation{},err};if err=edge.redis.PutSpec(ctx,target,record.Spec.Generation);err!=nil{return apiserver.RedisMutation{},err};return edge.finishRedisTarget(ctx,call,target,"-redis-delete")
}
