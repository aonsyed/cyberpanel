...
(async()=>{
 ...
 try {
  ...
  assert.equal(denial.status,404,'trusted site ownership must reject a mismatched tenant, including for the installation owner');
  console.log('PASS installation-owner correct tenant/site list200 and mismatched tenant/site404; tenant-limited actor coverage is in the HTTP regression');
  if(process.argv.includes('--scope-only'))return;
