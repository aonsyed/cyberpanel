...
func (c *SQLCollector) Collect(ctx context.Context, request CollectRequest) (Snapshot, error) {
	...
	if request.Selection.Sites || request.Selection.Databases || request.Selection.Mail || request.Selection.Credentials || request.Selection.Schedules || request.Selection.Repositories || request.Selection.Containers || request.Selection.BackupPolicies || request.Selection.Certificates || request.Selection.DNS && len(request.SiteSourceIDs) != 0 {
		if err := validateCyberPanelSiteSelection(request.SiteSourceIDs); err != nil {
			return Snapshot{}, err
		}
...
func validateCyberPanelSiteSelection(values []string) error {
	for _, value := range values {
		if !strings.HasPrefix(value, "website:") {
			return ErrDenied
		}
		number := strings.TrimPrefix(value, "website:")
...
func collectSites(ctx context.Context, tx *sql.Tx, selected []string) ([]SiteRecord, error) {
	...
	for rows.Next() {
		...
		var state int
		if err := rows.Scan(&id, &ownerID, &packageID, &domain, &phpVersion, &state, &externalApp, &diskMB, &transferMB, &memoryMB, &cpuCores, &ioMBPS, &maxConnections); err != nil {
			return nil, err
		}
	...
	for childRows.Next() {
		...
		if err := childRows.Scan(&id, &masterID, &domain, &path, &sslEnabled, &phpVersion, &alias); err != nil {
			return nil, err
		}
		sourceID := "child:" + strconv.FormatInt(id, 10)
		_, explicitlySelected := allowed[sourceID]
		if len(allowed) != 0 && !explicitlySelected && !mainIncluded[masterID] {
			continue
		}
		parentID := "website:" + strconv.FormatInt(masterID, 10)
		hostname := normalizeHostname(domain)
		if alias != 0 {
			legacyChildAliases[parentID] = append(legacyChildAliases[parentID], hostname)
			continue
		}
	...
	for aliasRows.Next() {
		var id, masterID int64
		var hostname string
		if err := aliasRows.Scan(&id, &masterID, &hostname); err != nil {
			return nil, err
		}
	...
	for parent, values := range legacyChildAliases {
		aliases[parent] = append(aliases[parent], values...)
	}
...
func collectDatabases(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]DatabaseRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &websiteID, &name, &username, &charset, &collation); err != nil {
			return nil, err
		}
		...
		for principalRows.Next() {
			...
			if scanErr := principalRows.Scan(&additional); scanErr != nil {
				principalRows.Close()
				return nil, scanErr
			}
			key := strings.ToLower(additional)
			if _, duplicate := seenPrincipals[key]; duplicate {
				continue
			}
			seenPrincipals[key] = struct{}{}
			principals = append(principals, DatabasePrincipalRecord{Name: additional, GrantSets: []string{"ALL PRIVILEGES"}, Password: lookupSecretRef("database-password", additional)})
		}
		if principalErr := principalRows.Err(); principalErr != nil {
			principalRows.Close()
...
func collectDNS(ctx context.Context, tx *sql.Tx, sites []SiteRecord, restricted bool) ([]DNSZoneRecord, error) {
	...
	for rows.Next() {
		...
		var dnssec int
		if err := rows.Scan(&id, &name, &mode, &dnssec); err != nil {
			return nil, err
		}
		name = normalizeHostname(name)
		if restricted && !zoneMatchesSites(name, hostnames) {
			continue
		}
	...
	for index := range zones {
		...
		for recordRows.Next() {
			...
			var ttl uint32
			var priority int
			if scanErr := recordRows.Scan(&name, &recordType, &content, &ttl, &priority); scanErr != nil {
				recordRows.Close()
				return nil, scanErr
			}
			if ttl == 0 {
			...
			set := sets[key]
			if set == nil {
				set = &DNSRecordSet{Name: normalizeDNSName(name), Type: strings.ToUpper(recordType), TTL: ttl}
				sets[key] = set
			} else if ttl > 0 && ttl < set.TTL {
				set.TTL = ttl
			}
			if priority > 0 && (set.Type == "MX" || set.Type == "SRV") {
				content = strconv.Itoa(priority) + " " + content
			}
			set.Values = append(set.Values, strings.TrimSpace(content))
		}
		if recordErr := recordRows.Err(); recordErr != nil {
			recordRows.Close()
		...
		for _, set := range sets {
			...
			zones[index].value.RecordSets = append(zones[index].value.RecordSets, *set)
		}
		sort.Slice(zones[index].value.RecordSets, func(i, j int) bool {
			left, right := zones[index].value.RecordSets[i], zones[index].value.RecordSets[j]
			return left.Name+left.Type+strconv.FormatUint(uint64(left.TTL), 10) < right.Name+right.Type+strconv.FormatUint(uint64(right.TTL), 10)
		})
	}
	values := make([]DNSZoneRecord, 0, len(zones))
	for _, zone := range zones {
		values = append(values, zone.value)
	}
	return values, nil
}
...
func collectMail(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]MailDomainRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&domain, &mainID, &childID); err != nil {
			return nil, err
		}
		siteID := ""
		if mainID.Valid {
			siteID = mainSites[mainID.Int64]
		}
		if siteID == "" && childID.Valid {
			siteID = childSites[childID.Int64]
		}
		if siteID == "" {
			continue
		}
		...
		for mailboxRows.Next() {
			...
			if scanErr := mailboxRows.Scan(&address, &mailLocation); scanErr != nil {
				mailboxRows.Close()
				return nil, scanErr
			}
		...
		if mailboxErr := mailboxRows.Err(); mailboxErr != nil {
			mailboxRows.Close()
	...
	byDomain := map[string]*MailDomainRecord{}
	for index := range values {
		byDomain[values[index].Name] = &values[index]
	}
	for forwardRows.Next() {
		var source, destination string
		if err := forwardRows.Scan(&source, &destination); err != nil {
			return nil, err
		}
		...
		if domain := byDomain[parts[1]]; domain != nil {
			entry := source + " -> " + destination
			domain.Forwarders = append(domain.Forwarders, entry)
		}
	}
	if err := forwardRows.Err(); err != nil {
		return nil, err
	}
	if present, err := legacyTablePresent(ctx, tx, "e_catchall"); err != nil {
		return nil, err
	} else if present {
		catchRows, err := tx.QueryContext(ctx, `SELECT domain_id,destination,enabled FROM e_catchall ORDER BY domain_id`)
		if err != nil {
			return nil, err
		}
		for catchRows.Next() {
			var domainName, destination string
			var enabled bool
			if err := catchRows.Scan(&domainName, &destination, &enabled); err != nil {
				catchRows.Close()
				return nil, err
			}
			if domain := byDomain[normalizeHostname(domainName)]; domain != nil && enabled {
				domain.CatchAll = append(domain.CatchAll, destination)
			}
		}
		if err := catchRows.Err(); err != nil {
			catchRows.Close()
			return nil, err
		}
		catchRows.Close()
	}
	if present, err := legacyTablePresent(ctx, tx, "e_pattern_forwarding"); err != nil {
		return nil, err
	} else if present {
		patternRows, err := tx.QueryContext(ctx, `SELECT domain_id,pattern,destination,pattern_type,priority,enabled FROM e_pattern_forwarding ORDER BY domain_id,priority,id`)
		if err != nil {
			return nil, err
		}
		for patternRows.Next() {
			var domainName, pattern, destination, patternType string
			var priority int
			var enabled bool
			if err := patternRows.Scan(&domainName, &pattern, &destination, &patternType, &priority, &enabled); err != nil {
				patternRows.Close()
				return nil, err
			}
			if domain := byDomain[normalizeHostname(domainName)]; domain != nil && enabled {
				domain.Forwarders = append(domain.Forwarders, patternType+":"+pattern+" -> "+destination)
			}
		}
	...
	if present, err := legacyTablePresent(ctx, tx, "e_pipeprograms"); err != nil {
		return nil, err
	} else if present {
		pipeRows, err := tx.QueryContext(ctx, `SELECT source,destination FROM e_pipeprograms ORDER BY source`)
		if err != nil {
			return nil, err
		}
		for pipeRows.Next() {
			var source, destination string
			if err := pipeRows.Scan(&source, &destination); err != nil {
				pipeRows.Close()
				return nil, err
			}
			parts := strings.Split(strings.ToLower(source), "@")
			if len(parts) == 2 {
				if domain := byDomain[parts[1]]; domain != nil {
					domain.Forwarders = append(domain.Forwarders, "pipe:"+source+" -> "+destination)
				}
			}
		}
...
func legacyTablePresent(ctx context.Context, tx *sql.Tx, name string) (bool, error) {
	return tablePresent(ctx, tx, name)
}
...
func collectFTP(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]CredentialRecord, error) {
	...
	rows, err := tx.QueryContext(ctx, `SELECT ID,domain_id,User,Dir,Status,Uid,Gid FROM users ORDER BY ID`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var id, websiteID int64
		var username, directory, status string
		var uid, gid uint32
		if err := rows.Scan(&id, &websiteID, &username, &directory, &status, &uid, &gid); err != nil {
			return nil, err
		}
		siteID := mainSites[websiteID]
		if siteID == "" {
			continue
		}
		relative, ok := relativeHomePath(siteHostname(sites, siteID), directory)
		if !ok {
			return nil, ErrInvalid
		}
		owner := ""
		for _, site := range sites {
			if site.SourceID == siteID {
				owner = site.OwnerSourceID
				break
			}
		}
		enabled, ok := legacyAccessEnabled(status)
		if !ok {
			return nil, ErrInvalid
		}
		sourceID := "ftp:" + strconv.FormatInt(id, 10)
		values = append(values, CredentialRecord{SourceID: sourceID, SiteSourceID: siteID, TenantSourceID: owner, PrincipalSourceID: "ftps-principal:" + strconv.FormatInt(id, 10), Kind: "ftps", Policy: "ftps_read_write", Username: username, RootRelative: relative, UID: uid, GID: gid, CredentialFormat: migration.AccessCredentialUnixCryptHash, Enabled: enabled, Secret: lookupSecretRef("ftp-password", strconv.FormatInt(id, 10))})
	}
	return values, rows.Err()
}

func legacyAccessEnabled(value string) (bool, bool) {
	switch strings.ToLower(strings.TrimSpace(value)) {
	case "1", "active", "enabled", "on":
		return true, true
	case "0", "disabled", "off", "suspended":
		return false, true
	default:
		return false, false
	}
...
func collectContainers(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]ContainerRecord, error) {
	mainSites, _ := selectedSiteDatabaseIDs(sites)
	rows, err := tx.QueryContext(ctx, `SELECT id,admin_id,SiteType,SiteName FROM websiteFunctions_dockersites ORDER BY id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var siteType int
		var name string
		if err := rows.Scan(&id, &websiteID, &siteType, &name); err != nil {
			return nil, err
		}
		siteID := mainSites[websiteID]
		if siteID == "" {
		...
		recipe := "cyberpanel-wordpress"
		if siteType != 0 {
			recipe = "cyberpanel-legacy-application"
		}
		values = append(values, ContainerRecord{SourceID: "container:" + strconv.FormatInt(id, 10), SiteSourceID: siteID, RecipeID: recipe, RecipeVersion: "legacy-v1", DescriptorArtifact: ArtifactID("container-descriptor:" + strconv.FormatInt(id, 10)), VolumeArtifacts: []ArtifactID{ArtifactID("container-volumes:" + strconv.FormatInt(id, 10))}})
		_ = name
	}
	return values, rows.Err()
}
...
func collectBackupPolicies(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]BackupPolicyRecord, error) {
	...
	collectors := []struct {
		table   string
		collect func(context.Context, *sql.Tx, []SiteRecord) ([]BackupPolicyRecord, error)
	}{
...
func collectLegacyBackupPolicies(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]BackupPolicyRecord, error) {
	...
	type schedule struct {
		id                     int64
		destination, frequency string
	}
	schedules := []schedule{}
	for rows.Next() {
		var value schedule
		if err := rows.Scan(&value.id, &value.destination, &value.frequency); err != nil {
			return nil, err
		}
...
func collectNormalBackupPolicies(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]BackupPolicyRecord, error) {
	...
	type job struct {
		...
		name, config, destination, destinationConfig string
	}
	jobs := []job{}
	for rows.Next() {
		var value job
		if err := rows.Scan(&value.id, &value.name, &value.config, &value.destinationID, &value.destination, &value.destinationConfig); err != nil {
			return nil, err
		}
	...
	if present, err := tablePresent(ctx, tx, "websiteFunctions_normalbackupsites"); err != nil {
		return nil, err
	} else if present {
		...
		for siteRows.Next() {
			var jobID, websiteID int64
			if err := siteRows.Scan(&jobID, &websiteID); err != nil {
				siteRows.Close()
				return nil, err
			}
			if assigned[jobID] == nil {
				assigned[jobID] = map[int64]struct{}{}
			}
			assigned[jobID][websiteID] = struct{}{}
		}
	...
	for _, job := range jobs {
		...
		if backupConfigHasSecret(destinationConfig) {
			credential = SecretRef("backup-normal-destination:" + strconv.FormatInt(job.destinationID, 10))
		}
		allSites := strings.EqualFold(backupConfigString(config, "allSites", "all_sites"), "all")
		for websiteID, siteID := range mainSites {
			if _, selected := assigned[job.id][websiteID]; !allSites && !selected {
				continue
			}
			values = append(values, BackupPolicyRecord{SourceID: "backup-policy:normal:" + strconv.FormatInt(job.id, 10) + ":" + safeOpaque(siteID), SiteSourceID: siteID, Schedule: schedule, Retention: retention, Provider: provider, Repository: repository, Credential: credential})
		}
	}
	return values, nil
}
...
func collectIncrementalBackupPolicies(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]BackupPolicyRecord, error) {
	...
	for rows.Next() {
		var id, retention int64
		var destination, frequency, hostname string
		if err := rows.Scan(&id, &destination, &frequency, &retention, &hostname); err != nil {
			return nil, err
		}
		siteID := byHostname[normalizeHostname(hostname)]
		if siteID == "" {
			continue
		}
		identity := strconv.FormatInt(id, 10) + "\x00" + siteID
		...
		if retention > 0 {
			retentionValue = strconv.FormatInt(retention, 10) + " days"
		}
		values = append(values, BackupPolicyRecord{SourceID: "backup-policy:incremental:" + strconv.FormatInt(id, 10) + ":" + safeOpaque(siteID), SiteSourceID: siteID, Schedule: legacyBackupSchedule(frequency), Retention: retentionValue, Provider: legacyBackupProvider(destination), Repository: safeBackupRepository(destination)})
	}
	return values, rows.Err()
}
...
func collectGoogleDriveBackupPolicies(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]BackupPolicyRecord, error) {
	...
	for rows.Next() {
		var id int64
		var name, auth, frequency, hostname string
		if err := rows.Scan(&id, &name, &auth, &frequency, &hostname); err != nil {
			return nil, err
		}
		siteID := byHostname[normalizeHostname(hostname)]
		if siteID == "" {
		...
		retention := "source-default"
		credential := SecretRef("")
		if strings.TrimSpace(auth) != "" && !strings.EqualFold(strings.TrimSpace(auth), "inactive") {
			credential = SecretRef("backup-google-drive-auth:" + strconv.FormatInt(id, 10))
			if config, decodeErr := decodeBackupConfig(auth); decodeErr == nil {
				if value := backupConfigString(config, "FileRetentiontime", "retention"); value != "" {
					retention = value
				}
			}
		}
		values = append(values, BackupPolicyRecord{SourceID: "backup-policy:google-drive:" + strconv.FormatInt(id, 10) + ":" + safeOpaque(siteID), SiteSourceID: siteID, Schedule: legacyBackupSchedule(frequency), Retention: retention, Provider: "google_drive", Repository: safeBackupRepository(name), Credential: credential})
	}
	return values, rows.Err()
}
...
func collectRemoteBackupPolicies(ctx context.Context, tx *sql.Tx, sites []SiteRecord) ([]BackupPolicyRecord, error) {
	...
	seen := map[string]struct{}{}
	for rows.Next() {
		var scheduleID, configID, websiteID int64
		var name, frequency, retention, scheduleConfigRaw, configType, configRaw string
		if err := rows.Scan(&scheduleID, &name, &frequency, &retention, &scheduleConfigRaw, &configID, &configType, &configRaw, &websiteID); err != nil {
			return nil, err
		}
		siteID := mainSites[websiteID]
		if siteID == "" {
		...
		identity := strconv.FormatInt(scheduleID, 10) + "\x00" + siteID
		if _, duplicate := seen[identity]; duplicate {
			continue
		}
		seen[identity] = struct{}{}
		scheduleConfig, err := decodeBackupConfig(scheduleConfigRaw)
		if err != nil {
			return nil, err
		}
		destinationConfig, err := decodeBackupConfig(configRaw)
		if err != nil {
			return nil, err
		}
		credential := SecretRef("")
		if backupConfigHasSecret(destinationConfig) {
			credential = SecretRef("backup-remote-config:" + strconv.FormatInt(configID, 10))
		}
...
func selectedMainDatabaseIDs(sites []SiteRecord) map[int64]string {
	main, _ := selectedSiteDatabaseIDs(sites)
	return main
}
func selectedSiteDatabaseIDs(sites []SiteRecord) (map[int64]string, map[int64]string) {
	main := map[int64]string{}
	child := map[int64]string{}
	for _, site := range sites {
		parts := strings.Split(site.SourceID, ":")
		if len(parts) != 2 {
			continue
		}
		...
		if parts[0] == "website" {
			main[id] = site.SourceID
		} else if parts[0] == "child" {
			child[id] = site.SourceID
		}
	}
	return main, child
}
func siteHostnames(sites []SiteRecord) []string {
	values := []string{}
	for _, site := range sites {
		values = append(values, site.PrimaryHostname)
		values = append(values, site.Aliases...)
	}
	return normalizedHostnames(values)
}
func zoneMatchesSites(zone string, sites []string) bool {
	for _, site := range sites {
...
func siteHostname(sites []SiteRecord, id string) string {
	for _, site := range sites {
		if site.SourceID == id {
...
func siteIDsByHostname(sites []SiteRecord) map[string]string {
	values := map[string]string{}
	for _, site := range sites {
		values[normalizeHostname(site.PrimaryHostname)] = site.SourceID
		for _, alias := range site.Aliases {
			values[normalizeHostname(alias)] = site.SourceID
		}
	}
	return values
}
func normalizePHPVersion(value string) string {
	value = strings.TrimSpace(strings.TrimPrefix(strings.ToUpper(value), "PHP"))
	...
	if len(digits) >= 2 {
		return string(digits[0]) + "." + string(digits[1:])
	}
	return value
}
func relativeDocumentRoot(masterDomain, path, child string) string {
	relative, ok := relativeHomePath(masterDomain, path)
	if ok {
		return relative
	}
	return "public_html/" + safeOpaque(child)
}
func relativeHomePath(domain, path string) (string, bool) {
	prefix := "/home/" + domain + "/"
...
func megabytes(value int64) uint64 {
	if value <= 0 {
		return 0
	}
...
func legacyBackupSchedule(value string) string {
	value = strings.TrimSpace(value)
...
func legacyBackupProvider(value string) string {
	value = strings.ToLower(value)
	if strings.Contains(value, "sftp") || strings.Contains(value, "ssh") {
		return "sftp"
	}
	if strings.Contains(value, "drive") {
		return "google_drive"
	}
	if strings.Contains(value, "s3") || strings.Contains(value, "amazon") || strings.Contains(value, "wasabi") || strings.Contains(value, "backblaze") {
		return "s3"
	}
	return "local"
}
func tablePresent(ctx context.Context, tx *sql.Tx, name string) (bool, error) {
	var count int
	err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND BINARY TABLE_NAME=BINARY ?`, name).Scan(&count)
	return count > 0, err
}
func decodeBackupConfig(raw string) (map[string]any, error) {
	raw = strings.TrimSpace(raw)
	if raw == "" || strings.EqualFold(raw, "inactive") {
...
func backupConfigString(config map[string]any, keys ...string) string {
...
func backupConfigHasSecret(config map[string]any) bool {
	for key, value := range config {
		normalized := strings.ToLower(strings.ReplaceAll(strings.ReplaceAll(key, "_", ""), "-", ""))
		switch normalized {
		case "password", "passwd", "secretkey", "secertkey", "accesskey", "token", "refreshtoken", "clientsecret", "privatekey":
			if strings.TrimSpace(backupAnyString(value)) != "" {
				return true
			}
		}
	}
...
func backupAnyString(value any) string {
	switch typed := value.(type) {
	case string:
		return typed
	case json.Number:
...
func mergeBackupConfig(left, right map[string]any) map[string]any {
...
func backupRepositoryDescriptor(name string, config map[string]any) string {
	parts := []string{safeBackupRepository(name)}
	for _, keys := range [][]string{{"Provider", "provider", "type"}, {"Hostname", "host", "ip"}, {"Path", "path"}, {"S3keyname", "bucket", "BucketName"}, {"EndUrl", "endpoint"}, {"username", "Username", "user"}, {"port"}} {
		if value := backupConfigString(config, keys...); value != "" {
			parts = append(parts, safeBackupRepository(value))
		}
	}
	return strings.Join(parts, "|")
}
func safeBackupRepository(value string) string {
	value = strings.TrimSpace(value)
	if value == "" {
...
func normalizeDNSMode(value string) string {
	switch strings.ToLower(strings.TrimSpace(value)) {
	case "master", "primary":
		return "primary"
...
func mailboxFormat(value string) string {
	if strings.HasPrefix(strings.ToLower(strings.TrimSpace(value)), "mdbox:") {
		return "mdbox"
	}
	return "maildir"
}
...
func NewSQLSecretSource(database *sql.DB) (*SQLSecretSource, error) {
	if database == nil {
		return nil, ErrInvalid
	}
	return &SQLSecretSource{database: database}, nil
}
func (s *SQLSecretSource) ReadSecret(ctx context.Context, ref SecretRef) ([]byte, error) {
	if s == nil || s.database == nil || ctx == nil || !ref.Valid() {
		return nil, ErrInvalid
	}
	value := string(ref)
	...
	case strings.HasPrefix(value, "ftp-password:"):
		...
		if id, parseErr := strconv.ParseInt(argument, 10, 64); parseErr != nil || id < 1 {
			err = ErrInvalid
		}
		accessHash = true
	...
	if accessHash {
		encoded, encodeErr := access.EncodeUnixCryptHashCredential(secret)
		if encodeErr != nil {
			return nil, ErrDenied
		}
		return encoded, nil
...
func lookupSecretRef(kind, value string) SecretRef {
	return SecretRef(kind + ":" + hex.EncodeToString([]byte(strings.TrimSpace(value))))
}
func decodeLookupSecretRef(value, kind string) (string, error) {
	encoded := strings.TrimPrefix(value, kind+":")
	if encoded == value || encoded == "" || len(encoded)%2 != 0 {
		return "", ErrInvalid
	}
	raw, err := hex.DecodeString(encoded)
	if err != nil || len(raw) == 0 || strings.ContainsRune(string(raw), '\x00') {
		return "", ErrInvalid
	}
