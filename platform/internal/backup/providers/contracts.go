...
func manifestBytes(manifest backup.RecoveryPointManifest)([]byte,error){if backup.ValidateRecoveryPointManifest(manifest)!=nil{return nil,ErrIntegrity};return canonicalJSON(manifest)}
type commitMarker struct{Version uint8 `json:"version"`;ObjectFormat string `json:"object_format,omitempty"`;EncryptionDomain string `json:"encryption_domain,omitempty"`;KeyVersion uint64 `json:"key_version,omitempty"`;RecoveryPointID backup.RecoveryPointID `json:"recovery_point_id"`;RepositoryID backup.RepositoryID `json:"repository_id"`;ManifestDigest string `json:"manifest_digest"`;Objects uint64 `json:"objects"`;Bytes uint64 `json:"bytes"`;EffectID string `json:"effect_id"`;CommittedAt time.Time `json:"committed_at"`}
func commitBytes(manifest backup.RecoveryPointManifest,repository backup.RepositoryID,effect string,now time.Time)([]byte,string,error){_ = now;if backup.ValidateRecoveryPointManifest(manifest)!=nil||repository==""||effect==""||len(effect)>1024{return nil,"",ErrInvalid};objects,bytes:=manifestTotals(manifest);marker:=commitMarker{Version:1,RecoveryPointID:manifest.RecoveryPointID,RepositoryID:repository,ManifestDigest:manifest.ManifestDigest,Objects:objects,Bytes:bytes,EffectID:effect,CommittedAt:manifest.CreatedAt.UTC()};payload,err:=canonicalJSON(marker);if err!=nil{return nil,"",err};return payload,hashBytes(payload),nil}
func validateCommitBytes(payload []byte,spec backup.RepositorySpec,manifest backup.RecoveryPointManifest,receipt backup.CopyReceipt)(commitMarker,error){var marker commitMarker;decoder:=json.NewDecoder(strings.NewReader(string(payload)));decoder.DisallowUnknownFields();if decoder.Decode(&marker)!=nil||decoder.Decode(&struct{}{})!=io.EOF{return commitMarker{},ErrIntegrity};objects,bytes:=manifestTotals(manifest);if !validCommitStorageFormat(marker,spec)||marker.RecoveryPointID!=manifest.RecoveryPointID||marker.RepositoryID!=spec.Repository.ID||marker.ManifestDigest!=manifest.ManifestDigest||marker.Objects!=objects||marker.Bytes!=bytes||marker.EffectID==""||len(marker.EffectID)>1024||!marker.CommittedAt.Equal(manifest.CreatedAt.UTC())||receipt.CommitMarker!=""&&hashBytes(payload)!=receipt.CommitMarker{return commitMarker{},ErrIntegrity};return marker,nil}
...
func validateManifest(manifest backup.RecoveryPointManifest)error{if backup.ValidateRecoveryPointManifest(manifest)!=nil{return ErrIntegrity};return nil}
func validateObject(object backup.ObjectDescriptor)error{if backup.ValidateObjectDescriptor(object)!=nil{return ErrInvalid};return nil}
