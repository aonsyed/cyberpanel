...
func validateManagementReachability(policy FirewallPolicy) error {
	rules := append([]FirewallRule(nil), policy.Rules...)
	sort.Slice(rules, func(i, j int) bool { return rules[i].Priority < rules[j].Priority })
	for _, management := range policy.ManagementProbe.SourceCIDRs {
		decided := false
		for _, rule := range rules {
			if rule.Protocol != ProtocolTCP || (rule.Family == FamilyIPv4) != management.Addr().Is4() || !portRangesContain(rule.DestinationPorts, policy.ManagementProbe.Port) || !sourcesCover(rule.Sources, management) {
				continue
			}
			if rule.Action != FirewallAccept {
				return ErrInvalidEffect
			}
			decided = true
			break
...
func portRangesContain(ranges []PortRange, port uint16) bool {
	for _, candidate := range ranges {
		if candidate.From <= port && candidate.To >= port {
			return true
		}
	}
	return false
}
func sourcesCover(sources []netip.Prefix, target netip.Prefix) bool {
	if len(sources) == 0 {
		return true
	}
	for _, source := range sources {
		if source.Bits() <= target.Bits() && source.Contains(target.Addr()) {
			return true
		}
	}
	return false
...
func renderNFTables(policy FirewallPolicy) []byte {
	...
	renderNFTChain := func(name string, hook string, priority int, defaultAction FirewallAction) {
		basePolicy := defaultAction
		if basePolicy == FirewallReject {
			basePolicy = FirewallDrop
		}
		fmt.Fprintf(&buffer, " chain %s { type filter hook %s priority %d; policy %s;\n", name, hook, priority, basePolicy)
		if name == "input" {
			buffer.WriteString("  ct state established,related accept\n  iifname \"lo\" accept\n")
		}
		if name == "input" {
			rules := append([]FirewallRule(nil), policy.Rules...)
			sort.Slice(rules, func(i, j int) bool { return rules[i].Priority < rules[j].Priority })
			for _, rule := range rules {
				buffer.WriteString(renderNFTRule(rule))
			}
		}
		if defaultAction == FirewallReject {
			buffer.WriteString("  reject\n")
		}
		buffer.WriteString(" }\n")
	}
...
func renderNFTRule(rule FirewallRule) string {
	var parts []string
	familyExpression := "meta nfproto ipv4"
	addressExpression := "ip saddr"
	protocol := string(rule.Protocol)
	if rule.Family == FamilyIPv6 {
		familyExpression = "meta nfproto ipv6"
		addressExpression = "ip6 saddr"
	}
	if rule.Protocol == ProtocolICMP {
		protocol = "meta l4proto icmp"
		if rule.Family == FamilyIPv6 {
			protocol = "meta l4proto ipv6-icmp"
		}
	}
	parts = append(parts, familyExpression)
	if len(rule.Sources) > 0 {
		values := make([]string, len(rule.Sources))
		for index, prefix := range rule.Sources {
			values[index] = prefix.String()
		}
		parts = append(parts, addressExpression+" { "+strings.Join(values, ", ")+" }")
	}
	parts = append(parts, protocol)
	if len(rule.DestinationPorts) > 0 {
		values := make([]string, len(rule.DestinationPorts))
		for index, ports := range rule.DestinationPorts {
			if ports.From == ports.To {
				values[index] = strconv.Itoa(int(ports.From))
			} else {
				values[index] = fmt.Sprintf("%d-%d", ports.From, ports.To)
			}
		}
		parts = append(parts, "dport { "+strings.Join(values, ", ")+" }")
	}
	if rule.RatePerMinute > 0 {
		parts = append(parts, fmt.Sprintf("limit rate %d/minute", rule.RatePerMinute))
	}
	if rule.Log {
		parts = append(parts, "log prefix \"cyberpanel-fw \"")
	}
...
func renderFirewalld(policy FirewallPolicy) []byte {
	...
	buffer.WriteString(strings.ToUpper(string(policy.DefaultInbound)))
	...
	for _, rule := range rules {
		...
		if len(sources) == 0 {
			if rule.Family == FamilyIPv4 {
				...
				sources = []netip.Prefix{netip.MustParsePrefix("::/0")}
			}
		}
		for _, source := range sources {
			fmt.Fprintf(&buffer, " <rule family=\"%s\" priority=\"%d\"><source address=\"%s\"/>", families[rule.Family], rule.Priority, source.String())
			if rule.Protocol != ProtocolICMP {
				for _, ports := range rule.DestinationPorts {
					value := strconv.Itoa(int(ports.From))
					if ports.To != ports.From {
						value = fmt.Sprintf("%d-%d", ports.From, ports.To)
					}
					fmt.Fprintf(&buffer, "<port protocol=\"%s\" port=\"%s\"/>", rule.Protocol, value)
				}
			} else {
				protocol := "icmp"
				if rule.Family == FamilyIPv6 {
				...
				fmt.Fprintf(&buffer, "<protocol value=\"%s\"/>", protocol)
			}
			if rule.Log {
				buffer.WriteString("<log prefix=\"cyberpanel-fw\" level=\"info\"")
				if rule.RatePerMinute > 0 {
					fmt.Fprintf(&buffer, "><limit value=\"%d/m\"/></log>", rule.RatePerMinute)
				} else {
					buffer.WriteString("/>")
				}
			}
			if rule.RatePerMinute > 0 {
				fmt.Fprintf(&buffer, "<%s><limit value=\"%d/m\"/></%s>", rule.Action, rule.RatePerMinute, rule.Action)
			} else {
				fmt.Fprintf(&buffer, "<%s/>", rule.Action)
			}
...
func renderFirewalldPolicies(policy FirewallPolicy) map[string][]byte {
	zone := string(renderFirewalld(policy))
	zoneStart := strings.Index(zone, "<zone target=")
	body := ""
	if zoneStart >= 0 {
		openingEnd := strings.Index(zone[zoneStart:], ">\n")
		if openingEnd >= 0 {
			body = zone[zoneStart+openingEnd+2:]
			body = strings.TrimSuffix(body, "</zone>\n")
		}
	}
	inbound := fmt.Sprintf("<?xml version=\"1.0\" encoding=\"utf-8\"?>\n<policy target=\"%s\"><ingress-zone name=\"ANY\"/><egress-zone name=\"HOST\"/>\n%s</policy>\n", strings.ToUpper(string(policy.DefaultInbound)), body)
	...
	for _, zoneName := range []string{"public", "external", "dmz", "work", "home", "internal", "trusted"} {
		path := "/etc/firewalld/policies/cyberpanel-forward-" + zoneName + ".xml"
		result[path] = []byte(fmt.Sprintf("<?xml version=\"1.0\" encoding=\"utf-8\"?>\n<policy target=\"%s\"><ingress-zone name=\"%s\"/><egress-zone name=\"ANY\"/></policy>\n", strings.ToUpper(string(policy.DefaultForward)), zoneName))
	}
	return result
}
...
func renderSSHPolicyPorts(policy SSHPolicy, ports []uint16) ([]byte, error) {
	...
	if policy.Authentication == SSHKeysAndMFA {
		buffer.WriteString("AuthenticationMethods publickey,keyboard-interactive:pam\n")
	} else {
		buffer.WriteString("AuthenticationMethods publickey\n")
	}
	...
	if len(policy.AllowedGroups) > 0 {
		groups := make([]string, len(policy.AllowedGroups))
		for index, group := range policy.AllowedGroups {
			name := group.String()
			if !managedSSHGroup(name) {
				return nil, ErrInvalidEffect
			}
		...
		buffer.WriteString("AllowGroups " + strings.Join(groups, " ") + "\n")
	}
...
func (executor *LinuxOperationsExecutor) putSSHKey(effect PutSSHKeyEffect) (linuxEffectResult, error) {
	key := effect.Key
	path := "/etc/ssh/authorized_keys/cyberpanel-" + key.PrincipalID.String()
	if err := executor.guardGeneration(KindSSHKey, key.ID, key.Generation, key); err != nil {
		return linuxEffectResult{}, err
	}
	if !managedSSHPrincipal(key.PrincipalID.String()) {
		return linuxEffectResult{}, ErrInvalidEffect
	}
	account, lookupErr := user.Lookup(key.PrincipalID.String())
	if lookupErr != nil {
		return linuxEffectResult{}, ErrInvalidEffect
	}
	...
	if err := validateSSHKeyMaterial(key); err != nil {
		return linuxEffectResult{}, err
	}
	...
	for index, line := range lines {
		if strings.HasSuffix(line, marker) {
			lines[index] = strings.TrimSpace(string(content))
			replaced = true
		}
	}
	if !replaced {
	...
	if err = executor.storeGeneration(KindSSHKey, key.ID, key.Generation, mustActivationDigest(key)); err != nil {
...
func validateSSHKeyMaterial(key SSHKey) error {
	if strings.ContainsAny(key.PublicBlob, " \t") {
		return ErrInvalidEffect
	}
	decoded, err := base64.StdEncoding.DecodeString(key.PublicBlob)
	if err != nil || len(decoded) < 8 {
		return ErrInvalidEffect
	}
	length := binary.BigEndian.Uint32(decoded[:4])
	if length == 0 || int(length) > len(decoded)-4 || string(decoded[4:4+length]) != key.Algorithm {
		return ErrInvalidEffect
	}
	digest := sha256.Sum256(decoded)
	if hex.EncodeToString(digest[:]) != key.FingerprintSHA256 {
		return ErrInvalidEffect
	}
	return nil
}
...
func (executor *LinuxOperationsExecutor) deleteSSHKey(effect DeleteSSHKeyEffect) (linuxEffectResult, error) {
	...
	for _, line := range strings.Split(string(snapshot.Content), "\n") {
		if line != "" && !strings.HasSuffix(line, marker) {
	...
	if len(kept) == 0 {
		...
		err = atomicOperationsFile(path, []byte(strings.Join(kept, "\n")+"\n"), 0o600)
	}
	if err != nil {
		return linuxEffectResult{}, err
	}
...
func renderSSHKey(key SSHKey) []byte {
	...
	if len(key.Restrictions.SourceCIDRs) > 0 {
		values := make([]string, len(key.Restrictions.SourceCIDRs))
		for index, prefix := range key.Restrictions.SourceCIDRs {
			values[index] = prefix.String()
		}
		restrictions = append(restrictions, "from=\""+strings.Join(values, ",")+"\"")
	}
	if !key.Restrictions.PermitPTY {
		restrictions = append(restrictions, "no-pty")
	}
	if !key.Restrictions.PermitPortForwarding {
		restrictions = append(restrictions, "no-port-forwarding")
	}
	if !key.Restrictions.PermitAgentForwarding {
...
func managedSSHPrincipal(value string) bool {
	return strings.HasPrefix(value, "cp_") && len(value) <= 32
}
func managedSSHGroup(value string) bool {
	return managedSSHPrincipal(value) || value == "cyberpanel-users"
}
...
func (executor *LinuxOperationsExecutor) applyWAF(ctx context.Context, request EffectRequest, effect WAFPolicyEffect) (linuxEffectResult, error) {
	...
	if err := validateWAFPreviousGeneration(currentIndex, previousGeneration, snapshot); err != nil {
		return linuxEffectResult{}, err
	}
	policyDigest, _ := activationDigest(policy)
...
func (executor *LinuxOperationsExecutor) renderWAFGeneration(policies []WAFPolicy, now time.Time) ([]byte, []wafSourceEvidence, []wafExclusionEvidence, error) {
	...
	installedAssets, err := installedWAFAssets("/")
	if err != nil {
	...
	sources = append(sources, installedAssets)
	if node.CRS == nil {
	...
	for _, scopedPolicy := range policies {
		...
		for exclusionIndex, exclusion := range scopedPolicy.Exclusions {
			for _, ruleID := range exclusion.RuleIDs {
				if ruleID >= 200000 && ruleID <= 200007 {
					return nil, nil, nil, ErrInvalidEffect
				}
			}
			...
			digest, _ := activationDigest(struct {
				Scope      string       `json:"scope"`
				Policy     ResourceID   `json:"policy"`
				Generation uint64       `json:"generation"`
				Ordinal    int          `json:"ordinal"`
				Exclusion  WAFExclusion `json:"exclusion"`
			}{scope, scopedPolicy.ID, scopedPolicy.Generation, exclusionIndex, exclusion})
...
func (executor *LinuxOperationsExecutor) validateWebConfiguration(ctx context.Context) ([]byte, error) {
	if info, err := os.Stat("/usr/local/lsws/bin/openlitespeed"); err == nil && info.Mode().IsRegular() {
		output, runErr := executor.runner.Run(ctx, "/usr/local/lsws/bin/openlitespeed", "-t")
...
func (executor *LinuxOperationsExecutor) guardGeneration(kind ResourceKind, id ResourceID, generation uint64, value any) error {
	...
	digest, _ := activationDigest(value)
	path := executor.generationPath(kind, id)
	...
	if json.Unmarshal(content, &current) != nil || current.Generation > generation || current.Generation == generation && current.Digest != digest {
		return ErrConflict
	}
	return nil
}
...
func (executor *LinuxOperationsExecutor) requireGeneration(kind ResourceKind, id ResourceID, generation uint64) error {
	content, err := os.ReadFile(executor.generationPath(kind, id))
	if err != nil {
		return err
	}
	var current struct {
	...
	if json.Unmarshal(content, &current) != nil || current.Generation != generation || !validSHA256(current.Digest) {
		return ErrConflict
	}
	return nil
}

func (executor *LinuxOperationsExecutor) generationPath(kind ResourceKind, id ResourceID) string {
	return filepath.Join(executor.stateRoot, "generations", string(kind)+"-"+id.String()+".json")
}
func activationDigest(value any) (string, error) {
...
func activationEvidence(strategy ActivationStrategy, candidate, previous, stage, commit string) *ActivationEvidence {
	return &ActivationEvidence{Strategy: strategy, CandidateDigest: candidate, PreviousDigest: previous, StageReceiptDigest: stage, ProbeReceiptDigest: digestBytes([]byte("probe-confirmed:" + candidate)), CommitReceiptDigest: commit}
}
func digestBytes(content []byte) string {
	digest := sha256.Sum256(content)
	return hex.EncodeToString(digest[:])
}
func snapshotDigest(snapshot operationsFileSnapshot) string {
	if !snapshot.Existed {
		return ""
	}
	return digestBytes(snapshot.Content)
}
...
func boundedText(content []byte, maximum int) string {
	if len(content) > maximum {
		content = content[:maximum]
	}
	return strings.TrimSpace(string(content))
}
func probeTCP(ctx context.Context, port uint16, successes uint8) error {
	dialer := net.Dialer{Timeout: 2 * time.Second}
	for count := uint8(0); count < successes; count++ {
		connection, err := dialer.DialContext(ctx, "tcp", net.JoinHostPort("127.0.0.1", strconv.Itoa(int(port))))
		if err != nil {
...
func allowedWAFPackPath(path string) bool {
	clean := filepath.Clean(path)
	return strings.HasPrefix(clean, "/usr/share/modsecurity-crs/") || strings.HasPrefix(clean, "/usr/local/lsws/conf/modsec/packs/")
}
