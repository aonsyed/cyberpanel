...
func probeManagedRedisSocket(ctx context.Context,path string,credential []byte)error{if len(credential)<32||len(credential)>256{return ErrInvalidEffect};connection,err:=(&net.Dialer{Timeout:2*time.Second}).DialContext(ctx,"unix",path);if err!=nil{return err};defer connection.Close();_ = connection.SetDeadline(time.Now().Add(2*time.Second));var request bytes.Buffer;fmt.Fprintf(&request,"*3\r\n$4\r\nAUTH\r\n$10\r\ncyberpanel\r\n$%d\r\n",len(credential));request.Write(credential);request.WriteString("\r\n*1\r\n$4\r\nPING\r\n");payload:=request.Bytes();if count,writeErr:=connection.Write(payload);writeErr!=nil||count!=len(payload){wipeOperationsBytes(payload);if writeErr!=nil{return writeErr};return io.ErrShortWrite};wipeOperationsBytes(payload);reader:=bufio.NewReaderSize(connection,256);for _,expected:=range [][]byte{[]byte("+OK\r\n"),[]byte("+PONG\r\n")}{response,readErr:=reader.ReadSlice('\n');if readErr!=nil{return readErr};if !bytes.Equal(response,expected){return errors.New("invalid authenticated Redis protocol response")}};return nil}
