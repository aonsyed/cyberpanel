...
func verifyClientData(raw []byte,kind string,challenge []byte,rpID string,origins []string)error{var value clientData;if decodeCollectedClientData(raw,&value)!=nil||value.Type!=kind||value.CrossOrigin||value.TopOrigin!=""{return ErrInvalid};decoded,err:=decodeBase64URL(value.Challenge,32,32);if err!=nil||subtle.ConstantTimeCompare(decoded,challenge)!=1{return ErrInvalid};parsed,err:=url.Parse(value.Origin);if err!=nil||parsed.Scheme!="https"||parsed.User!=nil||parsed.Path!=""||parsed.RawQuery!=""||parsed.Fragment!=""{return ErrInvalid};allowed:=false;for _,origin:=range origins{if value.Origin==origin{allowed=true;break}};if !allowed||!strings.EqualFold(parsed.Hostname(),rpID){return ErrInvalid};return nil}

// CollectedClientData is extensible browser-owned JSON, unlike our strict API
// envelopes. Its original bytes, including unknown members, remain covered by
// the assertion signature. Reject ambiguous duplicate keys before decoding.
func decodeCollectedClientData(raw []byte, value *clientData) error {
