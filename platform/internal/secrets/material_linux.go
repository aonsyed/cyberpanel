...
type LinuxMaterialPeerAuthorizer struct{allowed map[uint32]struct{}; inspected bool}
func NewInspectedMaterialPeerAuthorizer(allowedUIDs ...uint32)(*LinuxMaterialPeerAuthorizer,error){policy,err:=NewLinuxMaterialPeerAuthorizer(allowedUIDs...);if err==nil{policy.inspected=true};return policy,err}
...
func(policy *LinuxMaterialPeerAuthorizer)Authorize(connection net.Conn)(VerifiedPeer,error){
	...
	if policy.inspected { peer,err:=inspectMaterialConnection(context.Background(),connection);if err!=nil||peer.UID!=credential.Uid||peer.PID!=uint32(credential.Pid){return VerifiedPeer{},ErrForbidden};return peer,nil }
...
func linuxExecutableDigest(pid uint32)(string,error){
	path:="/proc/"+strconv.FormatUint(uint64(pid),10)+"/exe"
	file,err:=os.Open(path);if err!=nil{return "",err};defer file.Close()
	before,err:=file.Stat();if err!=nil||!before.Mode().IsRegular(){return "",ErrForbidden}
	hasher:=sha256.New();copied,err:=io.Copy(hasher,io.LimitReader(file,1<<30))
	if err!=nil||copied<=0||copied>=1<<30{return "",fmt.Errorf("executable digest unavailable")}
	after,err:=os.Stat(path);if err!=nil||!os.SameFile(before,after)||before.Size()!=after.Size()||!before.ModTime().Equal(after.ModTime()){return "",ErrForbidden}
