...
` + enrollmentSchema + lifecycleSchema
...
func(s *Store)Grant(ctx context.Context,id federation.ID)(federation.MutationGrant,error){var grant federation.MutationGrant;var raw []byte;var state string;err:=s.db.QueryRowContext(ctx,`SELECT state,grant_json FROM fleet_grants WHERE id=?`,id).Scan(&state,&raw);if errors.Is(err,sql.ErrNoRows){return grant,ErrNotFound};if err!=nil{return grant,err};if state!="active"{return grant,ErrForbidden};if err=json.Unmarshal(raw,&grant);err!=nil{return grant,err};return grant,nil}
func(s *Store)QueueIntent(ctx context.Context,owner federation.ID,intent federation.Intent,priority uint8,requestDigest string)(federation.Intent,bool,error){if s==nil||ctx==nil||!owner.Valid()||!intent.ID.Valid()||!intent.NodeID.Valid()||intent.IdempotencyKey==""||intent.EffectID==""||!validSHA256(requestDigest){return federation.Intent{},false,ErrInvalid};raw,err:=json.Marshal(intent);if err!=nil{return federation.Intent{},false,err};tx,err:=s.db.BeginTx(ctx,&sql.TxOptions{Isolation:sql.LevelSerializable});if err!=nil{return federation.Intent{},false,err};defer tx.Rollback();currentNode,err:=loadNodeTx(ctx,tx,intent.NodeID);if err!=nil{return federation.Intent{},false,err};if currentNode.OwnerTenantID!=owner||currentNode.AuthorityEpoch!=intent.AuthorityEpoch||currentNode.State==NodeRevoking||currentNode.State==NodeRevoked{return federation.Intent{},false,ErrStale};result,err:=tx.ExecContext(ctx,`INSERT INTO fleet_command_admissions(node_id,idempotency_key,request_digest,intent_id,effect_id,created_at) VALUES(?,?,?,?,?,?) ON CONFLICT DO NOTHING`,intent.NodeID,intent.IdempotencyKey,requestDigest,intent.ID,intent.EffectID,s.clock().UTC());if err!=nil{return federation.Intent{},false,err};inserted,err:=result.RowsAffected();if err!=nil{return federation.Intent{},false,err};if inserted==0{var storedDigest string;var storedIntentID federation.ID;if err=tx.QueryRowContext(ctx,`SELECT request_digest,intent_id FROM fleet_command_admissions WHERE node_id=? AND idempotency_key=?`,intent.NodeID,intent.IdempotencyKey).Scan(&storedDigest,&storedIntentID);errors.Is(err,sql.ErrNoRows){return federation.Intent{},false,ErrConflict};if err!=nil{return federation.Intent{},false,err};if storedDigest!=requestDigest{return federation.Intent{},false,ErrConflict};var storedRaw []byte;if err=tx.QueryRowContext(ctx,`SELECT intent_json FROM fleet_intents WHERE id=? AND node_id=?`,storedIntentID,intent.NodeID).Scan(&storedRaw);errors.Is(err,sql.ErrNoRows){return federation.Intent{},false,ErrConflict};if err!=nil{return federation.Intent{},false,err};var stored federation.Intent;if err=json.Unmarshal(storedRaw,&stored);err!=nil{return federation.Intent{},false,err};return stored,false,tx.Commit()};now:=s.clock().UTC();_,err=tx.ExecContext(ctx,`INSERT INTO fleet_intents VALUES(?,?,?,?,?,?,?,?,?,?)`,intent.ID,intent.NodeID,owner,"queued",priority,raw,[]byte(`{}`),now,now,intent.ExpiresAt);if err!=nil{return federation.Intent{},false,err};if err=tx.Commit();err!=nil{return federation.Intent{},false,err};return intent,true,nil}
...
func(s *Store)Receipt(ctx context.Context,receipt federation.Receipt)(bool,error){if s==nil||ctx==nil{return false,ErrInvalid};raw,err:=json.Marshal(receipt);if err!=nil{return false,err};receiptDigest:=digest(raw);tx,err:=s.db.BeginTx(ctx,&sql.TxOptions{Isolation:sql.LevelSerializable});if err!=nil{return false,err};defer tx.Rollback();var rawIntent,currentRaw []byte;var current string;if err=tx.QueryRowContext(ctx,`SELECT intent_json,status,receipt_json FROM fleet_intents WHERE id=?`,receipt.IntentID).Scan(&rawIntent,&current,&currentRaw);errors.Is(err,sql.ErrNoRows){return false,ErrNotFound};if err!=nil{return false,err};var intent federation.Intent;if err=json.Unmarshal(rawIntent,&intent);err!=nil{return false,err};if receipt.NodeID!=intent.NodeID||receipt.EffectID!=intent.EffectID||receipt.IdempotencyKey!=intent.IdempotencyKey{return false,ErrConflict};result,err:=tx.ExecContext(ctx,`INSERT INTO fleet_receipt_replays(node_id,receipt_digest,intent_id,signature_key_id,received_at) VALUES(?,?,?,?,?) ON CONFLICT DO NOTHING`,receipt.NodeID,receiptDigest,receipt.IntentID,receipt.SignatureKeyID,s.clock().UTC());if err!=nil{return false,err};inserted,err:=result.RowsAffected();if err!=nil{return false,err};if inserted==0{var storedIntent federation.ID;var storedKey string;if err=tx.QueryRowContext(ctx,`SELECT intent_id,signature_key_id FROM fleet_receipt_replays WHERE node_id=? AND receipt_digest=?`,receipt.NodeID,receiptDigest).Scan(&storedIntent,&storedKey);err!=nil{return false,err};if storedIntent!=receipt.IntentID||storedKey!=receipt.SignatureKeyID{return false,ErrConflict};return true,tx.Commit()};if len(currentRaw)>0&&!bytes.Equal(currentRaw,[]byte(`{}`)){var previous federation.Receipt;if err=json.Unmarshal(currentRaw,&previous);err!=nil{return false,err};if receipt.UpdatedAt.Before(previous.UpdatedAt)||receipt.UpdatedAt.Equal(previous.UpdatedAt)&&sameReceiptEvidence(previous,receipt){return true,tx.Commit()};if !validReceiptTransition(previous.Status,receipt.Status){return false,ErrConflict}};if current==string(federation.IntentApplied)||current==string(federation.IntentRejected)||current==string(federation.IntentExpired){return false,ErrConflict};result,err=tx.ExecContext(ctx,`UPDATE fleet_intents SET status=?,receipt_json=?,updated_at=? WHERE id=? AND status IN ('queued','sent','accepted','running','ambiguous')`,receipt.Status,raw,s.clock().UTC(),receipt.IntentID);if err!=nil{return false,err};rows,err:=result.RowsAffected();if err!=nil||rows!=1{return false,ErrConflict};if err=tx.Commit();err!=nil{return false,err};return false,nil}
...
func (s *Store) ProvisionEnrollment(ctx context.Context, enrollment ProvisionedEnrollment) error {
	...
	lookupErr := tx.QueryRowContext(ctx, `SELECT owner_tenant_id,COALESCE((SELECT generation FROM fleet_node_lifecycle WHERE node_id=fleet_nodes.id),1) FROM fleet_nodes WHERE id=?`, enrollment.Node.ID).Scan(&existingOwner, &lifecycleGeneration)
	if lookupErr != nil && !errors.Is(lookupErr, sql.ErrNoRows) {
		return lookupErr
	}
	// Startup bundles cannot restore authority or certificates superseded by a
	// durable operator lifecycle operation.
	if lookupErr == nil && lifecycleGeneration > 1 {
		if existingOwner != enrollment.Node.OwnerTenantID {
			return ErrForbidden
		}
		return tx.Commit()
	}
