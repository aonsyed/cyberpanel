...
const Schema = `
CREATE TABLE IF NOT EXISTS identity_installation_claim (
 singleton INTEGER PRIMARY KEY CHECK(singleton=1), principal_id TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS identity_principal_session_policies (
 principal_id TEXT PRIMARY KEY, mode TEXT NOT NULL, ipv4_prefix_bits INTEGER NOT NULL,
 ipv6_prefix_bits INTEGER NOT NULL, generation BIGINT NOT NULL, updated_at TIMESTAMP NOT NULL
);
CREATE TABLE IF NOT EXISTS identity_session_binding_policies (
 session_id TEXT PRIMARY KEY, mode TEXT NOT NULL, ipv4_prefix_bits INTEGER NOT NULL,
 ipv6_prefix_bits INTEGER NOT NULL
);
...
const sessionSelect=`SELECT s.id,s.principal_id,s.credential_id,s.authz_epoch,s.credential_epoch,s.assurance,COALESCE(b.mode,'exact_address'),COALESCE(b.ipv4_prefix_bits,32),COALESCE(b.ipv6_prefix_bits,128),s.source_prefix,s.user_agent_digest,s.csrf_secret_digest,s.created_at,s.last_seen_at,s.expires_at,s.absolute_expires_at,s.revoked_at FROM identity_sessions s LEFT JOIN identity_session_binding_policies b ON b.session_id=s.id`

type sessionScanner interface{Scan(...any)error}

func scanSession(row sessionScanner)(Session,error){var value Session;var prefix,mode string;var revoked sql.NullTime;err:=row.Scan(&value.ID,&value.PrincipalID,&value.CredentialID,&value.AuthzEpoch,&value.CredentialEpoch,&value.Assurance,&mode,&value.BindingPolicy.IPv4PrefixBits,&value.BindingPolicy.IPv6PrefixBits,&prefix,&value.UserAgentDigest,&value.CSRFSecretDigest,&value.CreatedAt,&value.LastSeenAt,&value.ExpiresAt,&value.AbsoluteExpiresAt,&revoked);if errors.Is(err,sql.ErrNoRows){return Session{},ErrNotFound};if err!=nil{return Session{},err};value.BindingPolicy.Mode=SessionBindingMode(mode);value.SourcePrefix,err=netip.ParsePrefix(prefix);if err!=nil{return Session{},err};if revoked.Valid{value.RevokedAt=&revoked.Time};return value,value.Validate()}

func(s *Store)Session(ctx context.Context,id ID)(Session,error){if s==nil||s.db==nil||ctx==nil||!id.Valid(){return Session{},ErrInvalid};return scanSession(s.db.QueryRowContext(ctx,sessionSelect+` WHERE s.id=?`,id))}

func(s *Store)PrincipalSessionPolicy(ctx context.Context,principalID ID)(PrincipalSessionPolicy,error){if s==nil||s.db==nil||ctx==nil||!principalID.Valid(){return PrincipalSessionPolicy{},ErrInvalid};principal,err:=s.Principal(ctx,principalID);if err!=nil{return PrincipalSessionPolicy{},err};value:=PrincipalSessionPolicy{PrincipalID:principal.ID,Binding:DefaultSessionBindingPolicy(),Generation:principal.Generation,UpdatedAt:principal.UpdatedAt};var mode string;var storedGeneration uint64;err=s.db.QueryRowContext(ctx,`SELECT mode,ipv4_prefix_bits,ipv6_prefix_bits,generation,updated_at FROM identity_principal_session_policies WHERE principal_id=?`,principalID).Scan(&mode,&value.Binding.IPv4PrefixBits,&value.Binding.IPv6PrefixBits,&storedGeneration,&value.UpdatedAt);if errors.Is(err,sql.ErrNoRows){return value,value.Validate()};if err!=nil{return PrincipalSessionPolicy{},err};if storedGeneration==0||storedGeneration>principal.Generation{return PrincipalSessionPolicy{},ErrConflict};value.Binding.Mode=SessionBindingMode(mode);return value,value.Validate()}

func(s *Store)ListPrincipalSessions(ctx context.Context,principalID ID,limit int,cursor ID)([]Session,string,uint64,error){if s==nil||s.db==nil||ctx==nil||!principalID.Valid()||limit<1||limit>200||(cursor!=""&&!cursor.Valid()){return nil,"",0,ErrInvalid};var total uint64;if err:=s.db.QueryRowContext(ctx,`SELECT COUNT(*) FROM identity_sessions WHERE principal_id=?`,principalID).Scan(&total);err!=nil{return nil,"",0,err};rows,err:=s.db.QueryContext(ctx,sessionSelect+` WHERE s.principal_id=? AND s.id>? ORDER BY s.id LIMIT ?`,principalID,cursor,limit+1);if err!=nil{return nil,"",0,err};defer rows.Close();values:=make([]Session,0,limit);for rows.Next(){value,scanErr:=scanSession(rows);if scanErr!=nil{return nil,"",0,scanErr};values=append(values,value)};if err=rows.Err();err!=nil{return nil,"",0,err};next:="";if len(values)>limit{next=values[limit-1].ID.String();values=values[:limit]};return values,next,total,nil}
