...
func(s *Service)ValidateSession(ctx context.Context,sessionID ID,rawToken,csrf []byte,source netip.Addr,userAgentDigests ...string)(Session,Principal,error){if len(userAgentDigests)>1{return Session{},Principal{},ErrInvalid};userAgentDigest:="";if len(userAgentDigests)==1{userAgentDigest=userAgentDigests[0]};session,err:=s.store.Session(ctx,sessionID);if err!=nil{return Session{},Principal{},ErrUnauthenticated};now:=s.clock().UTC();if session.RevokedAt!=nil||!now.Before(session.ExpiresAt)||!now.Before(session.AbsoluteExpiresAt){return Session{},Principal{},ErrUnauthenticated};if digest(rawToken)[:16]!=session.ID.StringSuffix()||(len(csrf)>0&&digest(csrf)!=session.CSRFSecretDigest){return Session{},Principal{},ErrUnauthenticated};p,err:=s.store.Principal(ctx,session.PrincipalID);if err!=nil||p.State!=PrincipalActive||p.AuthzEpoch!=session.AuthzEpoch||p.CredentialEpoch!=session.CredentialEpoch{return Session{},Principal{},ErrUnauthenticated};source=source.Unmap();if !source.IsValid(){return Session{},Principal{},ErrUnauthenticated};sourceMatches:=session.SourcePrefix.Contains(source);switch session.BindingPolicy.Mode{case SessionBindingNone:case SessionBindingExactAddress,SessionBindingPrefix:if !sourceMatches{s.record(ctx,session.PrincipalID,"","session.binding.reject","session",session.ID,"rejected",sessionBindingAuditRequest(session,now,"source"));return Session{},Principal{},ErrUnauthenticated};case SessionBindingRiskBasedStepUp:if !sourceMatches||userAgentDigest!=session.UserAgentDigest{reason:="source";if sourceMatches{reason="user_agent"};s.record(ctx,session.PrincipalID,"","session.binding.step_up","session",session.ID,"step_up_required",sessionBindingAuditRequest(session,now,reason));return Session{},Principal{},ErrSessionBindingStepUp};default:return Session{},Principal{},ErrUnauthenticated};session.LastSeenAt=now;if extended:=now.Add(30*time.Minute);extended.Before(session.AbsoluteExpiresAt){session.ExpiresAt=extended};if err=s.store.Apply(ctx,Mutation{Session:&session});err!=nil{return Session{},Principal{},err};return session,p,nil}

func sessionBindingAuditRequest(session Session,at time.Time,reason string)string{return session.ID.String()+"\x00"+at.Format(time.RFC3339Nano)+"\x00"+reason}

func(s *Service)CurrentSessionPolicy(ctx context.Context,actor ActorContext)(PrincipalSessionPolicy,error){if s==nil||s.store==nil||actor.SessionID==""{return PrincipalSessionPolicy{},ErrInvalid};if err:=s.validateActor(ctx,actor,AssurancePassword);err!=nil{return PrincipalSessionPolicy{},err};return s.store.PrincipalSessionPolicy(ctx,actor.PrincipalID)}

func(s *Service)UpdateSessionPolicy(ctx context.Context,actor ActorContext,expected uint64,binding SessionBindingPolicy)(PrincipalSessionPolicy,error){if s==nil||s.store==nil||actor.SessionID==""||expected==0||binding.Validate()!=nil{return PrincipalSessionPolicy{},ErrInvalid};if err:=s.validateActor(ctx,actor,AssurancePassword);err!=nil{return PrincipalSessionPolicy{},err};tx,err:=s.store.db.BeginTx(ctx,&sql.TxOptions{Isolation:sql.LevelSerializable});if err!=nil{return PrincipalSessionPolicy{},err};defer tx.Rollback();principal,err:=scanPrincipal(tx.QueryRowContext(ctx,`SELECT id,kind,username,email,display_name,state,locale,theme,authz_epoch,credential_epoch,generation,created_at,updated_at,deleted_at FROM identity_principals WHERE id=?`,actor.PrincipalID));if err!=nil{return PrincipalSessionPolicy{},err};if principal.AuthzEpoch!=actor.AuthzEpoch{return PrincipalSessionPolicy{},ErrUnauthenticated};if principal.Generation!=expected{return PrincipalSessionPolicy{},ErrStaleGeneration};current:=DefaultSessionBindingPolicy();var mode string;var storedGeneration uint64;var storedUpdated time.Time;err=tx.QueryRowContext(ctx,`SELECT mode,ipv4_prefix_bits,ipv6_prefix_bits,generation,updated_at FROM identity_principal_session_policies WHERE principal_id=?`,principal.ID).Scan(&mode,&current.IPv4PrefixBits,&current.IPv6PrefixBits,&storedGeneration,&storedUpdated);if errors.Is(err,sql.ErrNoRows){err=nil}else if err==nil{if storedGeneration==0||storedGeneration>principal.Generation{return PrincipalSessionPolicy{},ErrConflict};current.Mode=SessionBindingMode(mode)};if err!=nil{return PrincipalSessionPolicy{},err};if current.Validate()!=nil{return PrincipalSessionPolicy{},ErrConflict};if current==binding{return PrincipalSessionPolicy{},ErrConflict};weakening:=sessionPolicyWeakening(current,binding);if weakening&&actor.Assurance<AssuranceMFA{return PrincipalSessionPolicy{},ErrAssuranceRequired};currentSession,err:=scanSession(tx.QueryRowContext(ctx,sessionSelect+` WHERE s.id=?`,actor.SessionID));if err!=nil{return PrincipalSessionPolicy{},err};if currentSession.PrincipalID!=principal.ID||currentSession.AuthzEpoch!=actor.AuthzEpoch||currentSession.RevokedAt!=nil{return PrincipalSessionPolicy{},ErrForbidden};now:=s.clock().UTC();nextGeneration:=principal.Generation+1;nextEpoch:=principal.AuthzEpoch+1;result,err:=tx.ExecContext(ctx,`UPDATE identity_principals SET authz_epoch=?,generation=?,updated_at=? WHERE id=? AND generation=?`,nextEpoch,nextGeneration,now,principal.ID,principal.Generation);if err!=nil{return PrincipalSessionPolicy{},err};if rows,_:=result.RowsAffected();rows!=1{return PrincipalSessionPolicy{},ErrStaleGeneration};_,err=tx.ExecContext(ctx,`INSERT INTO identity_principal_session_policies(principal_id,mode,ipv4_prefix_bits,ipv6_prefix_bits,generation,updated_at) VALUES(?,?,?,?,?,?) ON CONFLICT(principal_id) DO UPDATE SET mode=excluded.mode,ipv4_prefix_bits=excluded.ipv4_prefix_bits,ipv6_prefix_bits=excluded.ipv6_prefix_bits,generation=excluded.generation,updated_at=excluded.updated_at`,principal.ID,binding.Mode,binding.IPv4PrefixBits,binding.IPv6PrefixBits,nextGeneration,now);if err!=nil{return PrincipalSessionPolicy{},err};var revokedOthers int64;if weakening{result,err=tx.ExecContext(ctx,`UPDATE identity_sessions SET revoked_at=? WHERE principal_id=? AND id<>? AND revoked_at IS NULL`,now,principal.ID,currentSession.ID);if err==nil{revokedOthers,_=result.RowsAffected();_,err=tx.ExecContext(ctx,`UPDATE identity_sessions SET authz_epoch=? WHERE id=? AND revoked_at IS NULL`,nextEpoch,currentSession.ID)}}else{_,err=tx.ExecContext(ctx,`UPDATE identity_sessions SET authz_epoch=? WHERE principal_id=? AND revoked_at IS NULL`,nextEpoch,principal.ID)};if err!=nil{return PrincipalSessionPolicy{},err};_,err=tx.ExecContext(ctx,`UPDATE identity_credentials SET authz_epoch=? WHERE principal_id=? AND kind<>'api_key' AND state='active'`,nextEpoch,principal.ID);if err!=nil{return PrincipalSessionPolicy{},err};if err=tx.Commit();err!=nil{return PrincipalSessionPolicy{},err};policy:=PrincipalSessionPolicy{PrincipalID:principal.ID,Binding:binding,Generation:nextGeneration,UpdatedAt:now};s.record(ctx,principal.ID,"","session.policy.update","principal",principal.ID,"applied",principal.ID.String()+"\x00"+fmt.Sprint(nextGeneration));if weakening{s.record(ctx,principal.ID,"","session.revoke_others","session",currentSession.ID,"applied",currentSession.ID.String()+"\x00policy\x00"+fmt.Sprint(nextGeneration)+"\x00"+fmt.Sprint(revokedOthers))};return policy,nil}

func sessionPolicyWeakening(current,next SessionBindingPolicy)bool{if current==next{return false};if next.Mode==SessionBindingExactAddress{return false};if current.Mode==SessionBindingNone{return false};if current.Mode!=next.Mode{return true};return next.IPv4PrefixBits<current.IPv4PrefixBits||next.IPv6PrefixBits<current.IPv6PrefixBits}

func(s *Service)ListSessions(ctx context.Context,actor ActorContext,limit int,cursor ID)([]SessionMetadata,string,uint64,error){if s==nil||s.store==nil||actor.SessionID==""{return nil,"",0,ErrInvalid};if err:=s.validateActor(ctx,actor,AssurancePassword);err!=nil{return nil,"",0,err};values,next,total,err:=s.store.ListPrincipalSessions(ctx,actor.PrincipalID,limit,cursor);if err!=nil{return nil,"",0,err};metadata:=make([]SessionMetadata,0,len(values));for _,value:=range values{metadata=append(metadata,sessionMetadata(value))};return metadata,next,total,nil}

func(s *Service)RevokeSession(ctx context.Context,actor ActorContext,sessionID ID)(SessionMetadata,error){if s==nil||s.store==nil||actor.SessionID==""||!sessionID.Valid(){return SessionMetadata{},ErrInvalid};if err:=s.validateActor(ctx,actor,AssurancePassword);err!=nil{return SessionMetadata{},err};session,err:=s.store.Session(ctx,sessionID);if err!=nil{return SessionMetadata{},err};if session.PrincipalID!=actor.PrincipalID{return SessionMetadata{},ErrNotFound};if session.RevokedAt!=nil{return sessionMetadata(session),nil};now:=s.clock().UTC();result,err:=s.store.db.ExecContext(ctx,`UPDATE identity_sessions SET revoked_at=? WHERE id=? AND principal_id=? AND revoked_at IS NULL`,now,session.ID,actor.PrincipalID);if err!=nil{return SessionMetadata{},err};if rows,_:=result.RowsAffected();rows!=1{return SessionMetadata{},ErrConflict};session.RevokedAt=&now;s.record(ctx,actor.PrincipalID,"","session.revoke","session",session.ID,"applied",session.ID.String()+"\x00"+now.Format(time.RFC3339Nano));return sessionMetadata(session),nil}

func(s *Service)RevokeOtherSessions(ctx context.Context,actor ActorContext)(uint64,error){if s==nil||s.store==nil||actor.SessionID==""{return 0,ErrInvalid};if err:=s.validateActor(ctx,actor,AssurancePassword);err!=nil{return 0,err};now:=s.clock().UTC();tx,err:=s.store.db.BeginTx(ctx,&sql.TxOptions{Isolation:sql.LevelSerializable});if err!=nil{return 0,err};defer tx.Rollback();var owner ID;var revoked sql.NullTime;err=tx.QueryRowContext(ctx,`SELECT principal_id,revoked_at FROM identity_sessions WHERE id=?`,actor.SessionID).Scan(&owner,&revoked);if errors.Is(err,sql.ErrNoRows){return 0,ErrNotFound};if err!=nil{return 0,err};if owner!=actor.PrincipalID||revoked.Valid{return 0,ErrForbidden};result,err:=tx.ExecContext(ctx,`UPDATE identity_sessions SET revoked_at=? WHERE principal_id=? AND id<>? AND revoked_at IS NULL`,now,actor.PrincipalID,actor.SessionID);if err!=nil{return 0,err};rows,err:=result.RowsAffected();if err!=nil{return 0,err};if err=tx.Commit();err!=nil{return 0,err};s.record(ctx,actor.PrincipalID,"","session.revoke_others","session",actor.SessionID,"applied",actor.SessionID.String()+"\x00"+now.Format(time.RFC3339Nano)+"\x00"+fmt.Sprint(rows));return uint64(rows),nil}

func sessionMetadata(value Session)SessionMetadata{return SessionMetadata{ID:value.ID,Assurance:value.Assurance,BindingPolicy:value.BindingPolicy,SourcePrefix:value.SourcePrefix,UserAgentDigest:value.UserAgentDigest,CreatedAt:value.CreatedAt,LastSeenAt:value.LastSeenAt,ExpiresAt:value.ExpiresAt,AbsoluteExpiresAt:value.AbsoluteExpiresAt,RevokedAt:value.RevokedAt}}

func(s *Service)AuthenticateAPIKey(ctx context.Context,raw []byte)(ActorContext,Principal,error){if len(raw)<24||len(raw)>4096{clearBytes(raw);return ActorContext{},Principal{},ErrUnauthenticated};verifiedID,ok,err:=s.verifier.VerifyAPIKey(ctx,raw);clearBytes(raw);if err!=nil||!ok{return ActorContext{},Principal{},ErrUnauthenticated};credential,loadErr:=s.store.Credential(ctx,verifiedID);if loadErr!=nil{credential,loadErr=s.store.CredentialByVerifierRef(ctx,verifiedID)};if loadErr==nil{if _,serviceErr:=s.store.serviceAPICredentialByID(ctx,credential.ID);serviceErr==nil||!errors.Is(serviceErr,ErrNotFound){return ActorContext{},Principal{},ErrUnauthenticated}};if loadErr!=nil||credential.Kind!=CredentialAPIKey||credential.State!=CredentialActive||credential.RevokedAt!=nil||credential.CompromisedAt!=nil{return ActorContext{},Principal{},ErrUnauthenticated};now:=s.clock().UTC();if credential.ExpiresAt!=nil&&!now.Before(*credential.ExpiresAt){return ActorContext{},Principal{},ErrExpired};principal,err:=s.store.Principal(ctx,credential.PrincipalID);if err!=nil||principal.State!=PrincipalActive||principal.AuthzEpoch!=credential.AuthzEpoch{return ActorContext{},Principal{},ErrUnauthenticated};credential.LastUsedAt=&now;if err=s.store.Apply(ctx,Mutation{Credential:&credential});err!=nil{return ActorContext{},Principal{},err};return ActorContext{PrincipalID:principal.ID,CredentialID:credential.ID,AuthzEpoch:principal.AuthzEpoch,Assurance:AssurancePassword},principal,nil}
...
func(s *Service)issueSession(ctx context.Context,p Principal,c Credential,assurance AssuranceLevel,source netip.Addr,ua string,ttl,absolute time.Duration)(Session,[]byte,[]byte,error){if !source.IsValid(){return Session{},nil,nil,ErrInvalid};policy,err:=s.store.PrincipalSessionPolicy(ctx,p.ID);if err!=nil{return Session{},nil,nil,err};if policy.Generation!=p.Generation{return Session{},nil,nil,ErrStaleGeneration};prefix,err:=policy.Binding.PrefixFor(source);if err!=nil{return Session{},nil,nil,err};if ttl<=0{ttl=30*time.Minute};if absolute<ttl{absolute=12*time.Hour};raw:=randomBytes(32);csrf:=randomBytes(32);encoded:=base32.StdEncoding.WithPadding(base32.NoPadding).EncodeToString(randomBytes(20));id,_:=NewID("ses_"+strings.ToLower(encoded));now:=s.clock().UTC();session:=Session{ID:id,PrincipalID:p.ID,CredentialID:c.ID,AuthzEpoch:p.AuthzEpoch,CredentialEpoch:p.CredentialEpoch,Assurance:assurance,BindingPolicy:policy.Binding,SourcePrefix:prefix,UserAgentDigest:ua,CSRFSecretDigest:digest(csrf),CreatedAt:now,LastSeenAt:now,ExpiresAt:now.Add(ttl),AbsoluteExpiresAt:now.Add(absolute)};session.ID=ID(session.ID.String()+"_"+digest(raw)[:16]);if err:=session.Validate();err!=nil{return Session{},nil,nil,err};if err:=s.store.Apply(ctx,Mutation{Session:&session});err!=nil{return Session{},nil,nil,err};return session,raw,csrf,nil}
