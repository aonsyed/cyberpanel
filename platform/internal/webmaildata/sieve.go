...
func compileSieve(scope Scope, generation uint64, rules []SieveRule, createdAt time.Time) (SieveProgram, error) {
	...
	for _, rule := range rules { if rule.Enabled && rule.CanonicalVacation != nil { script.WriteString("require [\"include\"];\n"); break } }
	references := make([]CanonicalVacationReference, 0)
	for _, rule := range rules {
		if !rule.Enabled { continue }
		if rule.CanonicalVacation != nil {
			if !validDigest(rule.CanonicalVacation.ProgramDigest) { return SieveProgram{}, ErrInvalid }
			references = append(references, *rule.CanonicalVacation)
			script.WriteString("include :personal "+strconv.Quote(vacationScriptName(scope,*rule.CanonicalVacation))+";\n")
			continue
		}
