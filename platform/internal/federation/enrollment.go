...
type EnrollmentResponse struct{PeerID ID;PeerSigningKeys map[string]SigningKeyTrust;Grant MutationGrant;NodeCertificate []byte;NodeCertificateRef string;CertificateExpiresAt time.Time;AuthorityEpoch uint64}
...
func(s *EnrollmentService)Enroll(ctx context.Context,tokenID ID,rawToken []byte,capabilities CapabilitySet)(Peer,error){defer wipe(rawToken);token,err:=s.tokens.Consume(ctx,tokenID,digestBytes(rawToken),s.clock().UTC());if err!=nil{return Peer{},err};if capabilities.NodeID!=token.NodeID||capabilities.CanonicalDigest()!=capabilities.Digest{return Peer{},ErrConflict};signingPublic,signingRef,err:=s.keys.CreateSigningIdentity(ctx,token.NodeID);if err!=nil{return Peer{},err};hpkePublic,hpkeRef,err:=s.keys.CreateHPKEIdentity(ctx,token.NodeID);if err!=nil{_ = s.keys.Destroy(ctx,signingRef);return Peer{},err};response,err:=s.exchange.Enroll(ctx,EnrollmentRequest{TokenID:token.ID,Token:rawToken,NodeID:token.NodeID,SigningPublicKey:signingPublic,HPKEPublicKey:hpkePublic,CapabilityDigest:capabilities.Digest,CAFingerprint:token.CAFingerprint});if err!=nil{_ = s.keys.Destroy(ctx,signingRef);_ = s.keys.Destroy(ctx,hpkeRef);return Peer{},err};now:=s.clock().UTC();grant:=response.Grant;if response.PeerID!=token.PeerID||response.AuthorityEpoch==0||response.AuthorityEpoch!=capabilities.AuthorityEpoch||!response.CertificateExpiresAt.After(now)||len(response.PeerSigningKeys)==0||grant.Validate(now)!=nil||grant.PeerID!=response.PeerID||grant.NodeID!=token.NodeID||grant.AuthorityEpoch!=response.AuthorityEpoch||len(grant.SigningKeys)!=len(response.PeerSigningKeys){return Peer{},ErrForbidden};seen:=make(map[string]struct{},len(grant.SigningKeys));for _,trusted:=range grant.SigningKeys{peerKey,found:=response.PeerSigningKeys[trusted.ID];trustedJSON,trustedErr:=json.Marshal(trusted);peerJSON,peerErr:=json.Marshal(peerKey);if !found||trustedErr!=nil||peerErr!=nil||trusted.ID!=peerKey.ID||!bytes.Equal(trustedJSON,peerJSON){return Peer{},ErrForbidden};if _,duplicate:=seen[trusted.ID];duplicate{return Peer{},ErrForbidden};seen[trusted.ID]=struct{}{}};for id,key:=range response.PeerSigningKeys{if id!=key.ID||key.Validate()!=nil{return Peer{},ErrForbidden}};certificateRef,err:=s.certificates.StoreNodeCertificate(ctx,token.NodeID,response.NodeCertificate,response.CertificateExpiresAt);if err!=nil{return Peer{},err};peer:=Peer{ID:response.PeerID,State:"active",CAFingerprint:token.CAFingerprint,SigningKeys:response.PeerSigningKeys,NodeCertificateRef:certificateRef,HPKEKeyRef:hpkeRef,CreatedAt:now,UpdatedAt:now};if err=s.federation.ActivateEnrollment(ctx,token.NodeID,capabilities.AuthorityEpoch,peer,grant);err!=nil{return Peer{},err};return peer,nil}
