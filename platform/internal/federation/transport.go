...
func(c *TLSConnector)Connect(ctx context.Context)(Session,error){if c==nil||c.Address==""||c.ServerName==""||c.TLSConfig==nil||len(c.TLSConfig.Certificates)==0{return nil,ErrInvalid};config:=c.TLSConfig.Clone();config.ServerName=c.ServerName;config.MinVersion=tls.VersionTLS13;raw,err:=c.Dialer.DialContext(ctx,"tcp",c.Address);if err!=nil{return nil,err};connection:=tls.Client(raw,config);if err=connection.HandshakeContext(ctx);err!=nil{raw.Close();return nil,err};session,err:=NewFramedSession(connection,c.MaxFrame);if err!=nil{raw.Close();return nil,err};return session,nil}
type framedSession struct{connection net.Conn;reader *bufio.Reader;max uint32;write sync.Mutex}
func NewFramedSession(connection net.Conn,max uint32)(Session,error){if connection==nil{return nil,ErrInvalid};if max==0||max>MaxFrameBytes{max=MaxFrameBytes};return &framedSession{connection:connection,reader:bufio.NewReaderSize(connection,64<<10),max:max},nil}
