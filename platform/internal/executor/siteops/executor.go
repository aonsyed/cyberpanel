...
func NewExecutor(registry Registry, host Host, php PHPBinaryResolver, config Config) (*Executor, error) {
	if nilInterface(registry) || nilInterface(host) || nilInterface(php) || !config.Edition.valid() { return nil, errors.New("siteops registry, host, PHP resolver, and engine edition are required") }
	if config.Retention == 0 { config.Retention = DefaultRetention }
	if config.Retention < 24*time.Hour || config.Retention > 365*24*time.Hour { return nil, errors.New("siteops tombstone retention must be between one day and one year") }
	return &Executor{registry: registry, host: host, php: php, edition: config.Edition, retention: config.Retention}, nil
}

func (executor *Executor) WithClock(clock func() time.Time) *Executor { if executor != nil && clock != nil { executor.now = clock }; return executor }
func (executor *Executor) currentTime() time.Time { if executor.now != nil { return executor.now().UTC() }; return time.Now().UTC() }

func (executor *Executor) Execute(ctx context.Context, request Request) (Response, error) {
	if executor == nil || nilInterface(executor.registry) || nilInterface(executor.host) || nilInterface(ctx) { return Response{}, ErrInvalidRequest }
	now := executor.currentTime(); if err := request.Validate(now); err != nil { return Response{}, err }
	...
	if operationErr != nil {
		code := errorCode(operationErr); _ = executor.registry.Fail(request, binding, code, executor.currentTime())
...
func (executor *Executor) perform(ctx context.Context, request Request, binding RuntimeBinding) (string, RuntimeBinding, error) {
	if err := ctx.Err(); err != nil { return "", binding, err }
	identity, err := identityFromBinding(binding); if err != nil { return "", binding, err }
	switch request.Operation {
	case OperationEnsureIdentity:
		if binding.State == BindingTombstoned || binding.State == BindingDeleted { return "", binding, ErrRegistryConflict }
		if err = executor.host.EnsureIdentity(ctx, identity); err != nil { return "", binding, err }
		if binding.State == BindingAllocated || binding.State == BindingSuspended { binding.State = BindingActive }
	...
	case OperationEnsureDirectories:
		...
		if binding.State == BindingQuarantined {
			if binding.QuarantineToken == "" { return "", binding, ErrCorruptRegistry }
			if err = executor.host.RestoreQuarantine(ctx, identity, binding.QuarantineGeneration, binding.QuarantineToken); err != nil { return "", binding, err }
			binding.QuarantineToken, binding.QuarantineGeneration, binding.State = "", 0, BindingActive
		}
		layout := layoutFor(binding, request.Generation); if err = layout.Validate(); err != nil { return "", binding, err }
		if err = executor.host.EnsureDirectories(ctx, identity, layout); err != nil { return "", binding, err }
	...
	case OperationEnsureLSAPIPool:
		if binding.State != BindingActive && binding.State != BindingSuspended { return "", binding, ErrRegistryConflict }
		if binding.RootGeneration != request.Generation { return "", binding, ErrRegistryConflict }
		binary, resolveErr := executor.php.Resolve(request.PHPProfile); if resolveErr != nil { return "", binding, resolveErr }
		spec := poolSpec(binding, request, executor.edition, binary); unit, renderErr := spec.RenderSystemdUnit(); if renderErr != nil { return "", binding, renderErr }
		if err = executor.host.InstallLSAPI(ctx, spec, unit); err != nil { return "", binding, err }
		if err = executor.host.StartLSAPI(ctx, spec); err != nil { return "", binding, err }
	...
	case OperationWriteHealth:
		...
		if err = executor.host.WriteHealth(ctx, identity, request.Generation, body); err != nil { return "", binding, err }
	...
	case OperationQuarantine:
		if binding.State == BindingTombstoned || binding.State == BindingDeleted { return "", binding, ErrRegistryConflict }
		if binding.State == BindingQuarantined { return evidence("quarantine", binding.SiteKey, binding.QuarantineToken, decimal(request.Generation)), binding, nil }
		if err = executor.stopGenerationPool(ctx, request, binding, request.Generation); err != nil { return "", binding, err }
		token := quarantineToken(binding, request.Generation)
		if err = executor.host.Quarantine(ctx, identity, request.Generation, token); err != nil { return "", binding, err }
		if err = executor.host.DisableIdentity(ctx, identity); err != nil { return "", binding, err }
	...
	case OperationPurge:
		if binding.State == BindingDeleted { return "", binding, ErrRegistryConflict }
		if binding.State == BindingTombstoned { return evidence("purge", binding.SiteKey, binding.TombstoneToken, binding.TombstonePurgeAfter.UTC().Format(time.RFC3339Nano)), binding, nil }
		if err = executor.removeAllPools(ctx, request, binding); err != nil { return "", binding, err }
		token := tombstoneToken(binding, request.Generation)
		quarantineGeneration := binding.QuarantineGeneration; if quarantineGeneration == 0 { quarantineGeneration = request.Generation }
		if err = executor.host.Tombstone(ctx, identity, quarantineGeneration, binding.QuarantineToken, token); err != nil { return "", binding, err }
		if err = executor.host.DisableIdentity(ctx, identity); err != nil { return "", binding, err }
...
func (executor *Executor) CollectExpired(ctx context.Context, limit int) (int, error) {
	if executor == nil || nilInterface(executor.registry) || nilInterface(executor.host) { return 0, errors.New("siteops executor is not configured") }
	bindings, err := executor.registry.ExpiredTombstones(executor.currentTime(), limit); if err != nil { return 0, err }
	collected := 0
	for _, binding := range bindings {
		if err = ctx.Err(); err != nil { return collected, err }
		unlock := executor.locks.lock(string(binding.RuntimeKey))
		identity, identityErr := identityFromBinding(binding)
		if identityErr == nil { identityErr = executor.host.DeleteTombstone(ctx, identity, binding.TombstoneToken) }
		if identityErr == nil { identityErr = executor.host.DeleteIdentity(ctx, identity) }
		if identityErr == nil { identityErr = executor.registry.DeleteBinding(binding.RuntimeKey, binding.Fence, executor.currentTime()) }
		unlock()
		if identityErr != nil { return collected, identityErr }
		collected++
	}
...
func addGeneration(values []uint64, generation uint64) []uint64 {
	...
	result := make([]uint64, 0, len(set)); for value := range set { result = append(result, value) }; sort.Slice(result, func(left, right int) bool { return result[left] < result[right] }); return result
}

type keyedLocks struct { mu sync.Mutex; values map[string]*keyLock }
type keyLock struct { mu sync.Mutex; users int }
func (locks *keyedLocks) lock(key string) func() {
	locks.mu.Lock(); if locks.values == nil { locks.values = map[string]*keyLock{} }; value := locks.values[key]; if value == nil { value = &keyLock{}; locks.values[key] = value }; value.users++; locks.mu.Unlock(); value.mu.Lock()
	return func() { value.mu.Unlock(); locks.mu.Lock(); value.users--; if value.users == 0 { delete(locks.values, key) }; locks.mu.Unlock() }
}

func nilInterface(value any) bool { if value == nil { return true }; reflected := reflect.ValueOf(value); switch reflected.Kind() { case reflect.Chan, reflect.Func, reflect.Interface, reflect.Map, reflect.Pointer, reflect.Slice: return reflected.IsNil(); default: return false } }

func receiptFromResponse(request Request, response Response) (any, error) {
	if !response.Succeeded { return nil, joinError(response.ErrorCode) }
	switch request.Operation {
	case OperationEnsureIdentity:
	...
	default: return nil, fmt.Errorf("unsupported siteops receipt")
	}
}
