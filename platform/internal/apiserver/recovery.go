...
type HAWriterCommitStatus struct{ProofDigest string `json:"proof_digest"`;LeaseID ha.WriterLeaseID `json:"lease_id"`;FencingEpoch uint64 `json:"fencing_epoch"`;ExpiresAt time.Time `json:"expires_at"`;State string `json:"state"`}
...
type RecoveryController struct{Identity *identity.Service;Claims ClaimTokenStore;Core CoreTransport;TrustPaths TrustPaths;Clock func()time.Time;StaticHA *ha.StaticDeploymentService;PeerVotes *ha.PeerVoteService;PeerCommits *ha.PeerCommitService}
...
func(server *RecoveryServer)Handler()http.Handler{mux:=http.NewServeMux();mux.HandleFunc("GET /recovery/v1/health",server.health);mux.HandleFunc("POST /recovery/v1/claim",server.claim);mux.HandleFunc("POST /recovery/v1/trust/rotate",server.rotate);mux.HandleFunc("POST /recovery/v1/ha/deployment",server.admitStaticHA);mux.HandleFunc("GET /recovery/v1/ha/deployment",server.staticHAStatus);mux.HandleFunc("POST /recovery/v1/ha/replication/verify",server.verifyStaticReplication);mux.HandleFunc("POST /recovery/v1/ha/quorum/prepare",server.prepareHAPeerQuorum);mux.HandleFunc("POST /recovery/v1/ha/quorum/commit",server.commitHAPeerQuorum);mux.HandleFunc("POST /recovery/v1/ha/writer/source-authority/verify",server.verifyHAWriterSourceAuthority);mux.HandleFunc("POST /recovery/v1/ha/writer/candidate-authority/verify",server.verifyHAWriterCandidateAuthority);mux.HandleFunc("POST /recovery/v1/ha/writer/activation/load",server.loadHAWriterActivation);mux.HandleFunc("POST /recovery/v1/ha/writer/activation/admit",server.admitHAWriterActivation);mux.HandleFunc("POST /recovery/v1/ha/writer/activation/verify",server.verifyHAWriterActivation);return http.HandlerFunc(func(writer http.ResponseWriter,request *http.Request){writer.Header().Set("Cache-Control","no-store");writer.Header().Set("X-Content-Type-Options","nosniff");mux.ServeHTTP(writer,request)})}
...
func (server *RecoveryServer) prepareHAPeerQuorum(writer http.ResponseWriter,request *http.Request){
	if server.Controller==nil||server.Controller.PeerVotes==nil{writeProblem(writer,classifyError(ErrUnavailable,""),1<<20);return}
	var input struct{PromotionID ha.PromotionID `json:"promotion_id"`}
	if err:=readJSON(request,4096,&input);err!=nil{writeProblem(writer,classifyError(err,""),1<<20);return}
	ctx,cancel:=context.WithTimeout(request.Context(),28*time.Second);defer cancel()
	proof,err:=server.Controller.PeerVotes.Collect(ctx,input.PromotionID)
	if err!=nil{writeStaticHAProblem(writer,err);return}
	_ = writeJSON(writer,http.StatusOK,proof,32<<10)
}

func (client *RecoveryClient) CollectHAPeerVotes(ctx context.Context,promotionID ha.PromotionID)(ha.PeerVoteProjection,error){
	var result ha.PeerVoteProjection
	err:=client.request(ctx,http.MethodPost,"/recovery/v1/ha/quorum/prepare",struct{PromotionID ha.PromotionID `json:"promotion_id"`}{promotionID},&result)
	return result,err
}

func (server *RecoveryServer) commitHAPeerQuorum(writer http.ResponseWriter,request *http.Request){
	if server.Controller==nil||server.Controller.PeerCommits==nil{writeProblem(writer,classifyError(ErrUnavailable,""),1<<20);return}
	var input ha.PeerCommitInput
	if err:=readJSON(request,4096,&input);err!=nil{writeProblem(writer,classifyError(err,""),1<<20);return}
	ctx,cancel:=context.WithTimeout(request.Context(),28*time.Second);defer cancel()
	proof,err:=server.Controller.PeerCommits.Collect(ctx,input)
	if err!=nil{writeStaticHAProblem(writer,err);return}
	result:=HAWriterCommitStatus{ProofDigest:proof.ProofDigest,LeaseID:proof.LeaseID,FencingEpoch:proof.FencingEpoch,ExpiresAt:proof.ExpiresAt,State:proof.State}
	_ = writeJSON(writer,http.StatusOK,result,32<<10)
}

func (client *RecoveryClient) CollectHAPeerCommit(ctx context.Context,input ha.PeerCommitInput)(HAWriterCommitStatus,error){
	var result HAWriterCommitStatus
	err:=client.request(ctx,http.MethodPost,"/recovery/v1/ha/quorum/commit",input,&result)
	return result,err
}
