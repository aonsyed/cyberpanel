...
func bindFoundation(registry *Registry,services DomainServices)error{
	if services.DNSRepository!=nil&&services.DNSProvider!=nil{if err:=registry.Bind("dns.rrset.apply",func(ctx context.Context,inv Invocation,value any)(OperationResult,error){p:=value.(*DNSApplyPayload);if p.Zone.TenantID!=""&&p.Zone.TenantID!=inv.Request.TenantID{return OperationResult{},ErrNotFound};p.Zone.TenantID=inv.Request.TenantID;p.Change.Key=inv.IdempotencyKey;if err:=services.DNSRepository.ApplyForTenant(ctx,inv.Request.TenantID,services.DNSProvider,p.Zone,p.Change);err!=nil{return OperationResult{},err};return OperationResult{Status:http.StatusOK,Value:p.Change},nil});err!=nil{return err}}
