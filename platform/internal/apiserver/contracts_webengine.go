...
type WebEnginePHPInstallPayload struct{Profile management.PHPProfile `json:"profile"`;CommitAuthorizationDigest string `json:"commit_authorization_digest"`}
...
func registerWebEngineContracts(registry *Registry)error{
	operations:=[]Operation{
		...
		{Name:"webengine.php.install",Permission:identity.MustPermission("webengine:manage"),Assurance:identity.AssuranceMFA,Auth:AuthRequired,Mutating:true,NewPayload:func()any{return &WebEnginePHPInstallPayload{}},ValidatePayload:validateWebEnginePHPInstall,ResolveScope:installationScope},
...
func validateWebEnginePHPInstall(value any)error{payload:=value.(*WebEnginePHPInstallPayload);profile:=payload.Profile;if profile.ID==""||len(profile.ID)>96||profile.Version!="8.2"&&profile.Version!="8.3"&&profile.Version!="8.4"||profile.BinaryPathID!=""||profile.BinaryArtifactDigest!=""||profile.ConfigDigest!=""||profile.CatalogDigest!=""||profile.CatalogSequence!=0||profile.Generation!=0||profile.Detached||len(payload.CommitAuthorizationDigest)!=64||strings.Trim(payload.CommitAuthorizationDigest,"0123456789abcdef")!=""{return invalid("web engine PHP profile")};return nil}
...
func bindWebEngine(registry *Registry,services DomainServices)error{
	...
	if capabilities.InstallPHP{if err:=registry.Bind("webengine.php.install",func(ctx context.Context,inv Invocation,value any)(OperationResult,error){p:=value.(*WebEnginePHPInstallPayload);profile,err:=services.WebEngine.CreatePHPProfile(ctx,management.PHPProfileCommand{CommandID:effectID(inv),Profile:p.Profile,ExpectedGeneration:inv.Request.ExpectedGeneration,Fence:inv.Request.ExpectedGeneration+1,CommitAuthorizationDigest:p.CommitAuthorizationDigest});if err!=nil{return OperationResult{},mapDomainError(err)};return OperationResult{Status:http.StatusOK,Value:profile,Generation:profile.Generation},nil});err!=nil{return err}}
