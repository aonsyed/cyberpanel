...
func(catalog *SQLCatalog)FinalizeProxyRoute(ctx context.Context,prepared PreparedProxyRoute,activationDigest string)error{if prepared.Token==""||prepared.EffectID==""||activationDigest==""{return errors.New("invalid proxy route finalization")};tx,err:=catalog.db.BeginTx(ctx,&sql.TxOptions{Isolation:sql.LevelSerializable});if err!=nil{return err};defer tx.Rollback();var status string;var proposal []byte;var withdraw bool;err=tx.QueryRowContext(ctx,`SELECT status,proposed_json,withdraw FROM webengine_proxy_changes WHERE change_token=? AND effect_id=?`,prepared.Token,prepared.EffectID).Scan(&status,&proposal,&withdraw);if err!=nil{return err};if status=="finalized"{return tx.Commit()};if status!="pending"{return ErrChangeClosed};var route composer.ProxyRouteInput;if json.Unmarshal(proposal,&route)!=nil{return errors.New("invalid proxy route proposal")};if withdraw{_,err=tx.ExecContext(ctx,`DELETE FROM webengine_proxy_routes WHERE route_ref=?`,route.Ref)}else{_,err=tx.ExecContext(ctx,`INSERT INTO webengine_proxy_routes(route_ref,generation,input_json,updated_at) VALUES(?,?,?,?) ON CONFLICT(route_ref) DO UPDATE SET generation=excluded.generation,input_json=excluded.input_json,updated_at=excluded.updated_at`,route.Ref,route.Generation,proposal,catalog.clock().UTC())};if err!=nil{return err};_,err=tx.ExecContext(ctx,`UPDATE webengine_node_config SET snapshot_generation=?,applied_digest=?,updated_at=? WHERE singleton_id=1`,prepared.Plan.SnapshotGeneration,activationDigest,catalog.clock().UTC());if err!=nil{return err};_,err=tx.ExecContext(ctx,`UPDATE webengine_proxy_changes SET status='finalized',activation_digest=?,completed_at=? WHERE change_token=? AND status='pending'`,activationDigest,catalog.clock().UTC(),prepared.Token);if err!=nil{return err};return tx.Commit()}
...
func loadAllSiteInputsForProxy(ctx context.Context,tx *sql.Tx)([]composer.SiteInput,error){rows,err:=tx.QueryContext(ctx,`SELECT input_json FROM webengine_site_inputs ORDER BY tenant_id,site_id`);if err!=nil{return nil,err};defer rows.Close();values:=[]composer.SiteInput{};for rows.Next(){var raw []byte;if err=rows.Scan(&raw);err!=nil{return nil,err};var value composer.SiteInput;if json.Unmarshal(raw,&value)!=nil{return nil,errors.New("invalid stored site input")};values=append(values,value)};return values,rows.Err()}
