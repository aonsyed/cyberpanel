...
func(authority *Authority)Withdraw(ctx context.Context,exposure containers.Exposure)(uint64,error){route,err:=routeInput(exposure);if err!=nil{return 0,err};prepared,err:=authority.Catalog.PrepareProxyRoute(ctx,routeEffect(exposure),route,true);if err!=nil{return 0,err};return authority.Commit(ctx,exposure,prepared.Plan.SnapshotGeneration)}
