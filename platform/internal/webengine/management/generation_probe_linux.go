...
func prepareLifecycleChallenges(render native.RenderRequest) ([]lifecycleChallenge, func() error, error) {
	if err := recoverLifecycleChallenges(); err != nil { return nil, func() error { return nil }, err }
	var challenges []lifecycleChallenge
	type ownedFile struct { root *os.Root; name string; info os.FileInfo }
	var owned []ownedFile
	cleanup := func() error {
		var failures []error
		for _, item := range owned {
			current, err := item.root.Lstat(item.name)
			if err == nil && !os.SameFile(current, item.info) { err = ErrConflict }
			if err == nil { err = item.root.Remove(item.name) }
			if err != nil && !errors.Is(err, os.ErrNotExist) { failures = append(failures, err) }
			failures = append(failures, item.root.Close())
		}
		owned = nil
		err := errors.Join(failures...)
		if err == nil { err = atomicPHPFile(filepath.Join(lifecycleStateRoot, "candidate-challenges.json"), []byte("null")) }
		return err
	}
	fail := func(err error) ([]lifecycleChallenge, func() error, error) { return nil, func() error { return nil }, errors.Join(err, cleanup()) }
	sites := map[webengine.ResourceRef]native.SiteRuntime{}
	for _, site := range render.Snapshot.Sites { sites[site.SiteRef] = site }
	active := map[webengine.ResourceRef]bool{}
	for _, binding := range render.Desired.Bindings {
		if binding.RoutingState == webengine.RoutingServe && binding.Relationship == webengine.BindingPrimary { active[binding.ApplicationRef] = true }
	}
	for _, app := range render.Desired.Applications {
		if !active[app.Ref] { continue }
		if len(challenges) >= 32 || app.PHPProfileRef == "" || app.ReverseProxy != nil { return fail(ErrUnsupported) }
		site, exists := sites[app.SiteRef]
		if !exists { return fail(ErrInvalid) }
		...
		if err != nil || !before.IsDir() || before.Mode()&os.ModeSymlink != 0 { return fail(ErrInvalid) }
		owner, ok := before.Sys().(*syscall.Stat_t)
		if !ok || owner.Uid != 0 || owner.Gid < siteops.DefaultUIDMinimum || owner.Gid > siteops.DefaultUIDMaximum { return fail(ErrInvalid) }
		resolved, err := filepath.EvalSymlinks(rootPath)
		if err != nil || resolved != rootPath { return fail(ErrInvalid) }
		root, err := os.OpenRoot(rootPath)
		if err != nil { return fail(err) }
		after, err := root.Stat(".")
		if err != nil || !os.SameFile(before, after) { root.Close(); return fail(ErrConflict) }
		var secret [32]byte
		if _, err = rand.Read(secret[:]); err != nil { root.Close(); return fail(err) }
		...
		if err != nil { root.Close(); return fail(err) }
		if err = atomicPHPFile(filepath.Join(lifecycleStateRoot, "candidate-challenges.json"), manifest); err != nil { root.Close(); return fail(err) }
		file, err := root.OpenFile(name, os.O_CREATE|os.O_EXCL|os.O_WRONLY|syscall.O_NOFOLLOW, 0o600)
		if err != nil { root.Close(); return fail(err) }
		info, statErr := file.Stat()
		if statErr != nil { file.Close(); root.Remove(name); root.Close(); return fail(statErr) }
		owned = append(owned, ownedFile{root: root, name: name, info: info})
		if err = file.Chown(0, int(owner.Gid)); err == nil { err = file.Chmod(0o440) }
		if err != nil { file.Close(); return fail(err) }
	...
	if len(challenges) == 0 { return fail(ErrUnsupported) }
...
func recoverLifecycleChallenges() error {
	...
	if err != nil || !info.Mode().IsRegular() || info.Mode().Perm() != 0o600 || !rootOwnedFile(info) || info.Size() > linuxManagementMaximumFrame { return ErrInvalid }
	content, err := os.ReadFile(manifestPath)
	if err != nil { return err }
	var manifest *lifecycleChallengeManifest
	if decodeLifecyclePayload(content, &manifest) != nil { return ErrInvalid }
	if manifest == nil { return nil }
	if native.ValidateRequest(manifest.Render, manifest.Render.Desired.Engine.Edition) != nil || len(manifest.Challenges) > 32 { return ErrInvalid }
	for _, challenge := range manifest.Challenges {
		if !validSHA256(challenge.Secret) || challenge.Filename != ".panel-runtime-"+challenge.Secret[:24]+".php" { return ErrInvalid }
		found := false
		for _, app := range manifest.Render.Desired.Applications {
			if app.Ref != challenge.Application { continue }
			for _, site := range manifest.Render.Snapshot.Sites {
				if site.SiteRef != app.SiteRef { continue }
				found = true
				...
				if resolveErr != nil || resolved != base { return ErrInvalid }
				...
				if errors.Is(statErr, os.ErrNotExist) { root.Close(); continue }
				if statErr != nil || !file.Mode().IsRegular() || !rootOwnedFile(file) { root.Close(); return ErrConflict }
				removeErr := root.Remove(name)
				closeErr := root.Close()
				if removeErr != nil || closeErr != nil { return errors.Join(removeErr, closeErr) }
			}
		}
		if !found { return ErrInvalid }
	}
	return atomicPHPFile(manifestPath, []byte("null"))
}

func probeLifecycleHTTP(ctx context.Context, input lifecycleGenerationInput) (receipt ProbeReceipt, err error) {
	if _, err = renderLifecycleGeneration(ctx, input); err != nil { return receipt, err }
...
func executeLifecycleProbes(ctx context.Context, input lifecycleGenerationInput, challenges []lifecycleChallenge) (ProbeReceipt, error) {
	...
	for _, challenge := range challenges {
		if !validSHA256(challenge.Secret) || challenge.Filename != ".panel-runtime-"+challenge.Secret[:24]+".php" || challengeByApp[challenge.Application].Application != "" { return receipt, ErrInvalid }
		challengeByApp[challenge.Application] = challenge
	}
	listeners := map[webengine.ResourceRef]webengine.Listener{}
	for _, listener := range input.Render.Desired.Engine.Listeners { listeners[listener.Ref] = listener }
	...
	for _, binding := range input.Render.Desired.Bindings {
		...
		if binding.RoutingState != webengine.RoutingServe || binding.Relationship != webengine.BindingPrimary { continue }
		challenge, exists := challengeByApp[binding.ApplicationRef]
		if !exists || len(binding.Hostnames) == 0 { return receipt, ErrInvalid }
		for _, ref := range binding.ListenerRefs {
			listener, exists := listeners[ref]
			if !exists || !lifecycleLoopbackListener(listener) { return receipt, ErrUnsupported }
			for _, hostname := range binding.Hostnames {
				count++
				if count > 64 { return receipt, ErrInvalid }
				for nonceIndex := 0; nonceIndex < 2; nonceIndex++ {
					var nonceBytes [32]byte
					if _, err := rand.Read(nonceBytes[:]); err != nil { return receipt, err }
				...
				seenPHP = true
				if listener.TLSMode == webengine.TLSModeTLS { seenTLS = true } else { seenHTTP = true }
			}
		}
	}
	if count == 0 || !seenPHP { return receipt, ErrUnsupported }
	receipt.HTTP, receipt.HTTPS, receipt.TLS, receipt.PHP = seenHTTP, seenTLS, seenTLS, seenPHP
	receipt.EvidenceDigest = digestJSON(struct { Config string; Exchanges []lifecycleExchange }{input.ConfigDigest, evidence})
	receipt.ObservedAt = time.Now().UTC()
	return receipt, nil
}

func lifecycleLoopbackListener(listener webengine.Listener) bool {
	for _, address := range listener.Addresses { if address == "0.0.0.0" || address == "127.0.0.1" { return true } }
	return false
}
...
func lifecycleHTTPExchange(ctx context.Context, hostname string, listener webengine.Listener, challenge lifecycleChallenge, nonce string) (lifecycleExchange, error) {
	...
	if err != nil || parsed.String() != hostname || listener.Port == 0 { return result, ErrInvalid }
	ctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
	scheme := "http"
	if result.TLS { scheme = "https" }
	transport := &http.Transport{Proxy: nil, DisableKeepAlives: true, TLSClientConfig: &tls.Config{MinVersion: tls.VersionTLS12, ServerName: hostname}, DialContext: func(ctx context.Context, network, address string) (net.Conn, error) {
		if network != "tcp" || address != net.JoinHostPort(hostname, strconv.Itoa(int(listener.Port))) { return nil, ErrInvalid }
	...
	if err != nil { return result, err }
	if response == nil || response.Body == nil { return result, ErrAmbiguous }
	defer response.Body.Close()
	body, err := io.ReadAll(io.LimitReader(response.Body, 4097))
	if err != nil || len(body) > 4096 { return result, ErrAmbiguous }
	mac := hmac.New(sha256.New, []byte(challenge.Secret))
	_, _ = mac.Write([]byte(nonce))
	expected := hex.EncodeToString(mac.Sum(nil))+"\n"
	if response.StatusCode != http.StatusOK || !hmac.Equal(body, []byte(expected)) || !strings.HasPrefix(response.Header.Get("Content-Type"), "text/plain") { return result, fmt.Errorf("%w: candidate did not execute the bound PHP challenge", ErrAmbiguous) }
	if result.TLS {
		if response.TLS == nil || !response.TLS.HandshakeComplete || len(response.TLS.VerifiedChains) == 0 || len(response.TLS.PeerCertificates) == 0 { return result, ErrAmbiguous }
