...
type Capabilities struct{Inspect,Install,Convert,RefreshLicense,InstallPHP,Tune bool}
...
func New(repository Repository,catalog ArtifactCatalog,renderer TargetRenderer,executor Executor)(*Service,error){if repository==nil||catalog==nil||renderer==nil||executor==nil{return nil,ErrInvalid};capabilities:=Capabilities{Inspect:true,Install:true,Convert:true,RefreshLicense:true,InstallPHP:true,Tune:true};if provider,ok:=executor.(CapabilityProvider);ok{capabilities=provider.ManagementCapabilities()};return &Service{repository:repository,catalog:catalog,renderer:renderer,executor:executor,capabilities:capabilities,clock:time.Now},nil}
