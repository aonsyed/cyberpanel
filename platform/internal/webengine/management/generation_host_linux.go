...
func (host *LinuxLifecycleHost) coordinateConfiguration(ctx context.Context) error {
	if host.configurationRelease != nil { return nil }
...
func openLifecycleStore(edition webengine.Edition) (*fsstore.Store, error) {
	...
	if err != nil || !rootOwnedFile(info) { return nil, ErrInvalid }
...
func renderLifecycleGeneration(ctx context.Context, input lifecycleGenerationInput) (native.ConfigGeneration, error) {
	if input.Request.ExpectedGeneration == 0 || validateLifecycleEffect(input.Request, input.ConfigDigest) != nil { return native.ConfigGeneration{}, ErrInvalid }
	...
	default: return native.ConfigGeneration{}, ErrInvalid
	}
	generation, err := renderer.Render(ctx, input.Render)
	if err != nil || generation.ContentDigest != input.ConfigDigest { return native.ConfigGeneration{}, errors.Join(ErrConflict, err) }
...
func (host *LinuxLifecycleHost) handleGeneration(ctx context.Context, request LinuxManagementRequest) (any, error) {
	...
	case LinuxManagementSwitch:
		...
		if decodeLifecyclePayload(request.Payload, &input) != nil { return nil, ErrInvalid }
		return host.switchGeneration(ctx, input)
	case LinuxManagementConfirm, LinuxManagementRestore:
		var input SwitchReceipt
		if decodeLifecyclePayload(request.Payload, &input) != nil { return nil, ErrInvalid }
		if request.Operation == LinuxManagementRestore { return host.restoreGeneration(ctx, input) }
		return host.confirmGeneration(ctx, input)
	default:
		if host.switchPending() { return nil, ErrConflict }
		var input lifecycleGenerationInput
		if decodeLifecyclePayload(request.Payload, &input) != nil { return nil, ErrInvalid }
		if request.Operation == LinuxManagementActiveProbe { return host.probeActiveGeneration(ctx, input) }
		record, err := host.stageGeneration(ctx, input)
		if err != nil { return nil, err }
		if request.Operation == LinuxManagementStage { return record.Stage, nil }
		// Re-run observational checks: cached success is never fresh runtime proof.
		mode := "validate"
		if request.Operation == LinuxManagementShadow { mode = "probe" }
		observed, err := runLifecycleCandidate(ctx, input, mode)
		if err != nil {
			if mode == "validate" { return observed.Validation, err }
...
func (host *LinuxLifecycleHost) probeActiveGeneration(ctx context.Context, input lifecycleGenerationInput) (ProbeReceipt, error) {
	if _, err := renderLifecycleGeneration(ctx, input); err != nil { return ProbeReceipt{}, err }
	if err := host.adoptGeneration(ctx, input.Request.ExpectedGeneration); err != nil { return ProbeReceipt{}, err }
	edition, err := readLifecycleEdition()
	if err != nil || edition != input.Render.Desired.Engine.Edition { return ProbeReceipt{}, ErrConflict }
	store, err := openLifecycleStore(edition)
	if err != nil { return ProbeReceipt{}, err }
	defer store.Close()
	current, err := store.Current(ctx)
	if err != nil { return ProbeReceipt{}, err }
	if err = verifyLifecycleService(ctx, host.journal.Host.Plan); err != nil { return ProbeReceipt{}, err }
	probe, err := probeLifecycleHTTP(ctx, input)
	if err != nil { return probe, err }
	after, err := store.Current(ctx)
	if err != nil || after.Digest != current.Digest { return ProbeReceipt{}, ErrConflict }
...
func (host *LinuxLifecycleHost) stageGeneration(ctx context.Context, input lifecycleGenerationInput) (lifecycleGenerationRecord, error) {
	...
	if err != nil { return lifecycleGenerationRecord{}, err }
	if err = host.adoptGeneration(ctx, input.Request.ExpectedGeneration); err != nil { return lifecycleGenerationRecord{}, err }
	if !host.journal.Host.Active || host.journal.Host.Plan.Edition != generation.Edition { return lifecycleGenerationRecord{}, ErrUnsupported }
	if err = installedLifecyclePlan(ctx, host.journal.Host.Plan); err != nil { return lifecycleGenerationRecord{}, err }
	if input.Request.Fence <= host.journal.Host.Fence { return lifecycleGenerationRecord{}, ErrConflict }
	...
	if exists && digestJSON(record.Input) != digestJSON(input) { return record, ErrConflict }
	if !exists && len(host.journal.Generations) >= 32 {
		oldest := ""
		for digest, candidate := range host.journal.Generations {
			if digest == host.journal.Host.ConfigDigest || host.journal.Switch != nil && (digest == host.journal.Switch.Receipt.PreviousConfigDigest || digest == host.journal.Switch.Receipt.TargetConfigDigest) { continue }
			if oldest == "" || candidate.Stage.ObservedAt.Before(host.journal.Generations[oldest].Stage.ObservedAt) { oldest = digest }
		}
		if oldest == "" { return record, ErrConflict }
	...
	if err != nil { return record, err }
	if _, err = store.GenerationPath(ctx, staged); err != nil { return record, err }
	if !exists {
		record.Input = input
		...
		host.journal.Generations[input.ConfigDigest] = record
		if err = host.persist(); err != nil { delete(host.journal.Generations, input.ConfigDigest); return record, err }
	}
	return record, nil
}
...
func (host *LinuxLifecycleHost) switchGeneration(ctx context.Context, input SwitchRequest) (SwitchReceipt, error) {
	if host.journal.Switch != nil && host.journal.Switch.Request.EffectRequest.EffectID == input.EffectRequest.EffectID {
		if digestJSON(host.journal.Switch.Request) != digestJSON(input) { return SwitchReceipt{}, ErrConflict }
		if host.journal.Switch.State != "switched" && host.journal.Switch.State != "confirmed" && host.journal.Switch.State != "restored" { return host.journal.Switch.Receipt, ErrAmbiguous }
		return host.journal.Switch.Receipt, nil
	}
	if host.switchPending() || validateLifecycleEffect(input.EffectRequest, input.TargetConfigDigest) != nil || !validSHA256(input.PreviousConfigDigest) || !input.RollbackDeadline.After(host.now().Add(time.Minute)) || input.RollbackDeadline.After(host.now().Add(24*time.Hour)) { return SwitchReceipt{}, ErrInvalid }
	...
	edition, err := readLifecycleEdition()
	if err != nil || edition != input.Target || input.Previous != input.Target { return SwitchReceipt{}, ErrUnsupported }
	if err = host.adoptGeneration(ctx, input.EffectRequest.ExpectedGeneration); err != nil { return SwitchReceipt{}, err }
	if !host.journal.Host.Active || host.journal.Host.Plan.Edition != input.Previous || host.journal.Host.Fence >= input.EffectRequest.Fence { return SwitchReceipt{}, ErrConflict }
	...
	if !found || !previousFound || digestJSON(target.Input.Request) != digestJSON(input.EffectRequest) || target.Input.Render.Desired.Engine.Edition != input.Target || !target.Validation.Valid || !target.Probe.PHP || !validSHA256(target.Probe.EvidenceDigest) || target.Probe.ObservedAt.Before(host.now().Add(-5*time.Minute)) || previous.Input.Render.Desired.Engine.Edition != input.Previous { return SwitchReceipt{}, ErrConflict }
	store, err := openLifecycleStore(edition)
	if err != nil { return SwitchReceipt{}, err }
	defer store.Close()
	current, err := store.Current(ctx)
	if err != nil || current.Digest != input.PreviousConfigDigest { return SwitchReceipt{}, ErrConflict }
	if err = verifyLifecycleService(ctx, host.journal.Host.Plan); err != nil { return SwitchReceipt{}, err }
	var nonce [32]byte
	if _, err = rand.Read(nonce[:]); err != nil { return SwitchReceipt{}, err }
	now := host.now().UTC()
	...
	if err = host.persist(); err != nil { host.journal.Switch = previousSwitch; return receipt, err }
	go host.generationWatchdog(receipt)
	if err = store.SwapMaster(ctx, activation.Receipt{Edition: edition, Digest: input.TargetConfigDigest}); err == nil { err = runLifecycle(ctx, lifecycleBinaryPath, "-t") }
	if err == nil { err = runLifecycle(ctx, "/usr/bin/systemctl", "restart", lifecycleService) }
	if err == nil { err = verifyLifecycleService(ctx, host.journal.Host.Plan) }
	if err != nil {
	...
	if err = host.persist(); err != nil { return receipt, errors.Join(ErrAmbiguous, err) }
	return receipt, nil
}

func (host *LinuxLifecycleHost) checkSwitch(input SwitchReceipt) error {
	if host.journal.Switch == nil { return ErrNotFound }
	...
	if digestJSON(input) != digestJSON(expected) { return ErrConflict }
	return nil
}

func (host *LinuxLifecycleHost) confirmGeneration(ctx context.Context, input SwitchReceipt) (ProbeReceipt, error) {
	if err := host.checkSwitch(input); err != nil { return ProbeReceipt{}, err }
	record := host.journal.Switch
	if record.State == "confirmed" { return record.Probe, nil }
	if record.State != "switched" || !host.now().Before(input.RollbackDeadline) { return ProbeReceipt{}, ErrConflict }
	if host.journal.Host.Fence != input.Fence || host.journal.Host.ConfigDigest != input.TargetConfigDigest { return ProbeReceipt{}, ErrConflict }
	...
	if err != nil { return probe, err }
	defer store.Close()
	if err = verifyLifecycleService(ctx, host.journal.Host.Plan); err != nil { return probe, err }
	if err = store.Confirm(ctx, activation.Receipt{Edition: input.Target, Digest: input.TargetConfigDigest}); err != nil { return probe, err }
	...
	if err = host.persist(); err != nil { record.State = "switched"; record.Receipt.Confirmed = false; return probe, errors.Join(ErrAmbiguous, err) }
	return probe, nil
}
...
func (host *LinuxLifecycleHost) restoreGeneration(ctx context.Context, input SwitchReceipt) (ProbeReceipt, error) {
	if err := host.checkSwitch(input); err != nil { return ProbeReceipt{}, err }
	record := host.journal.Switch
	if record.State == "restored" { return record.Probe, nil }
	if record.State == "confirmed" { return ProbeReceipt{}, ErrConflict }
	if host.journal.Host.Fence > input.Fence { return ProbeReceipt{}, ErrConflict }
	record.State = "restoring"
	if err := host.persist(); err != nil { return ProbeReceipt{}, err }
	...
	if err = store.RestoreMaster(ctx, previous); err == nil { err = runLifecycle(ctx, lifecycleBinaryPath, "-t") }
	if err == nil { err = runLifecycle(ctx, "/usr/bin/systemctl", "restart", lifecycleService) }
	if err == nil { err = verifyLifecycleService(ctx, record.Previous.Plan) }
	if err != nil { return ProbeReceipt{}, errors.Join(ErrAmbiguous, err) }
	baseline, found := host.journal.Generations[input.PreviousConfigDigest]
	if !found { return ProbeReceipt{}, ErrAmbiguous }
	probe, err := probeLifecycleHTTP(ctx, baseline.Input)
	if err != nil { return probe, errors.Join(ErrAmbiguous, err) }
	if err = store.Confirm(ctx, previous); err != nil { return probe, err }
	...
	if err = host.persist(); err != nil { record.State = "restoring"; record.Receipt.Restored = false; return probe, errors.Join(ErrAmbiguous, err) }
	return probe, nil
}
...
func (host *LinuxLifecycleHost) generationWatchdog(receipt SwitchReceipt) {
	...
	if host.checkSwitch(receipt) != nil || !host.switchPending() { return }
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	_, _ = host.restoreGeneration(ctx, receipt)
