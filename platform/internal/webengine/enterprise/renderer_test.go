...
func TestRenderUsesEnterpriseNativeXMLNotApacheCompatibilityMode(t *testing.T) {
	...
	assertContainsAll(t, server,
		...
		"<configFile>$SERVER_ROOT/conf/"+native.VirtualHostDirectory(42, "site-a--binding-a", []byte(artifactContent(t, generation, native.ArtifactVirtualHost, "site-a--binding-a")))+"/vhconf.xml</configFile>",
		"<name>vh_site_b_binding_b</name>",
		"<configFile>$SERVER_ROOT/conf/"+native.VirtualHostDirectory(42, "site-b--binding-b", []byte(artifactContent(t, generation, native.ArtifactVirtualHost, "site-b--binding-b")))+"/vhconf.xml</configFile>",
