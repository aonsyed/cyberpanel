...
const ExecutorProtocolVersion uint16 = 1
...
func (envelope ExecutorEnvelope) Validate(now time.Time) error {
	if envelope.Version != ExecutorProtocolVersion || !validID(envelope.RequestID) || envelope.Operation == "" || len(envelope.Payload) > 64<<20 || envelope.Deadline.Before(now) || envelope.Deadline.After(now.Add(10*time.Minute)) { return fmt.Errorf("invalid executor envelope") }
	if envelope.SiteID != "" { if err := requireID("site", string(envelope.SiteID)); err != nil { return err } }
	if deploymentPayloadDigest(envelope.Payload) != envelope.PayloadHash { return ErrIntegrity }
	return nil
}
...
func (result ExecutorResult) Validate(request ExecutorEnvelope, now time.Time) error {
	if result.Version != ExecutorProtocolVersion || result.RequestID != request.RequestID || result.Operation != request.Operation || result.Receipt == "" || result.CompletedAt.After(now.Add(time.Minute)) || result.CompletedAt.Before(request.Deadline.Add(-10*time.Minute)) { return ErrIntegrity }
	if result.Succeeded == (result.ErrorCode != "") { return ErrIntegrity }
	if result.Succeeded && deploymentPayloadDigest(result.Payload) != result.PayloadHash { return ErrIntegrity }
	if len(result.Payload) > 64<<20 { return ErrLimitExceeded }
	return nil
}
...
type ExecutorTransport interface { Execute(context.Context, ExecutorEnvelope) (ExecutorResult, error) }
...
func (server ExecutorServer) Dispatch(ctx context.Context, envelope ExecutorEnvelope) (ExecutorResult, error) {
	if server.Authorizer == nil || server.Handler == nil { return ExecutorResult{}, errors.New("executor authorizer and handler are required") }
	now := time.Now().UTC(); if server.Now != nil { now = server.Now().UTC() }
	if err := envelope.Validate(now); err != nil { return ExecutorResult{}, err }
	if err := server.Authorizer.Authorize(ctx, envelope); err != nil { return ExecutorResult{}, ErrUnauthorized }
	result, err := server.Handler.Handle(ctx, envelope); if err != nil { return ExecutorResult{}, err }
	if err = result.Validate(envelope, now); err != nil { return ExecutorResult{}, err }
	return result, nil
}
