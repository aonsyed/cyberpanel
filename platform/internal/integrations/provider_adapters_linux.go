...
func (adapter *RemoteProviderAdapter) cloudflareVerify(ctx context.Context,binding ProviderBinding,material []byte)(string,error){if validateCloudflareEndpointPolicy(binding.Endpoint)!=nil{return "",ErrPolicyDenied};token,err:=cloudflareCredential(material);if err!=nil{return "",err};endpoint:=cloudflareAPIEndpoint+"/client/v4/user/tokens/verify";request,err:=http.NewRequestWithContext(ctx,http.MethodGet,endpoint,nil);if err!=nil{return "",err};request.Header.Set("Authorization","Bearer "+token);response,err:=cloudflareNoRedirectClient(adapter.Client).Do(request);if err!=nil{return "",providerNetworkError("cloudflare.verify",err)};defer response.Body.Close();raw,readErr:=io.ReadAll(io.LimitReader(response.Body,64<<10));if readErr!=nil||len(raw)>=64<<10{return "",ErrIntegrity};var envelope cloudflareEnvelope;if json.Unmarshal(raw,&envelope)!=nil{return "",ErrIntegrity};if response.StatusCode==429{return "",ErrRateLimited};if response.StatusCode==401||response.StatusCode==403||!envelope.Success||envelope.Result.Status!="active"||!validID(envelope.Result.ID){return "",ErrUnauthorized};return envelope.Result.ID,nil}

func (adapter *RemoteProviderAdapter) cloudflareRevoke(ctx context.Context,binding ProviderBinding,material []byte,tokenID string)error{if validateCloudflareEndpointPolicy(binding.Endpoint)!=nil{return ErrPolicyDenied};token,err:=cloudflareCredential(material);if err!=nil{return err};if !validID(tokenID){return ErrIntegrity};endpoint:=cloudflareAPIEndpoint+"/client/v4/user/tokens/"+url.PathEscape(tokenID);request,err:=http.NewRequestWithContext(ctx,http.MethodDelete,endpoint,nil);if err!=nil{return err};request.Header.Set("Authorization","Bearer "+token);response,err:=cloudflareNoRedirectClient(adapter.Client).Do(request);if err!=nil{return providerNetworkError("cloudflare.revoke",err)};defer response.Body.Close();raw,readErr:=io.ReadAll(io.LimitReader(response.Body,64<<10));if readErr!=nil||len(raw)>=64<<10{return ErrIntegrity};var envelope cloudflareEnvelope;if json.Unmarshal(raw,&envelope)!=nil{return ErrIntegrity};if response.StatusCode==429{return ErrRateLimited};if response.StatusCode==401||response.StatusCode==403{return ErrUnauthorized};if response.StatusCode<200||response.StatusCode>=300||!envelope.Success{return ErrUnavailable};return nil}
