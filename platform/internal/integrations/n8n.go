...
func (installation N8NInstallation) Validate() error { if !validID(string(installation.ID))||!validID(string(installation.TenantID))||!validID(installation.SiteID)||installation.Definition.Validate()!=nil||!validID(installation.DatabaseBindingRef)||!validID(string(installation.EncryptionKeyRef))||!validID(string(installation.OwnerCredentialRef))||!validID(installation.VolumeID)||!validID(installation.ResourceProfileID)||installation.WebReplicas==0||installation.WorkerReplicas==0||installation.WebhookReplicas==0||installation.PrivateServiceBinding==""||installation.PublicRouteBinding==""||installation.Generation==0||installation.CreatedAt.IsZero()||installation.UpdatedAt.IsZero(){return ErrInvalid};return nil }
...
type N8NReceipt struct { InstallationID ID `json:"installation_id"`; Action string `json:"action"`; ObservedGeneration uint64 `json:"observed_generation"`; ImageDigest string `json:"image_digest"`; WorkloadDigest string `json:"workload_digest"`; PrivateEndpoint string `json:"private_endpoint"`; HealthDigest string `json:"health_digest"`; Receipt string `json:"receipt"`; CompletedAt time.Time `json:"completed_at"` }
...
func (coordinator N8NCoordinator) Apply(ctx context.Context,installation N8NInstallation,action string)(N8NReceipt,error){if coordinator.Federation==nil||coordinator.Store==nil{return N8NReceipt{},ErrInvalid};if err:=installation.Validate();err!=nil{return N8NReceipt{},err};expected:=uint64(0);if action!="install"{persisted,err:=coordinator.Store.LoadN8NInstallation(ctx,installation.ID);if err!=nil{return N8NReceipt{},err};if persisted.Generation!=installation.Generation{return N8NReceipt{},ErrStaleGeneration};expected=persisted.Generation};intent:=N8NWorkloadIntent{InstallationID:installation.ID,Action:action,ExpectedGeneration:installation.Generation,ImageDigest:installation.Definition.ResolvedImageDigest,DatabaseBindingRef:installation.DatabaseBindingRef,SecretGrantRefs:[]string{string(installation.EncryptionKeyRef),string(installation.OwnerCredentialRef)},VolumeID:installation.VolumeID,ResourceProfileID:installation.ResourceProfileID,WebReplicas:installation.WebReplicas,WorkerReplicas:installation.WorkerReplicas,WebhookReplicas:installation.WebhookReplicas,InternalPort:5678,PrivateServiceBinding:installation.PrivateServiceBinding};if err:=intent.Validate();err!=nil{return N8NReceipt{},err};receipt,err:=coordinator.Federation.ApplyN8NIntent(ctx,intent);if err!=nil{return N8NReceipt{},err};if receipt.InstallationID!=installation.ID||receipt.Action!=action||receipt.ObservedGeneration<installation.Generation||receipt.ImageDigest!=installation.Definition.ResolvedImageDigest||!validDigest(receipt.WorkloadDigest)||!validDigest(receipt.HealthDigest)||receipt.Receipt==""||receipt.CompletedAt.IsZero(){return N8NReceipt{},fmt.Errorf("%w: n8n receipt",ErrIntegrity)};switch action{case "install","resume","scale","update":installation.State="active";case "suspend":installation.State="suspended";case "remove":installation.State="removed"};if expected>0{installation.Generation=expected+1};installation.UpdatedAt=receipt.CompletedAt;if err:=coordinator.Store.SaveN8NInstallation(ctx,installation,expected);err!=nil{return N8NReceipt{},err};return receipt,nil}
