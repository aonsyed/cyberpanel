...
func(policy *DefaultPolicy)ValidateExposure(_ context.Context,tenant ID,exposure Exposure)error{if policy==nil||!tenant.Valid()||exposure.TenantID!=tenant||!exposure.ID.Valid()||!exposure.WorkloadID.Valid()||exposure.PortName==""||exposure.ListenerRef==""||exposure.DomainBindingRef==""||exposure.Generation==^uint64(0){return ErrPolicy};if strings.ContainsAny(exposure.ListenerRef," \t\r\n")||strings.ContainsAny(exposure.DomainBindingRef,"/@ \t\r\n"){return ErrPolicy};for _,prefix:=range prefixesSorted(exposure.AllowedCIDRs){if !prefix.IsValid()||prefix.Addr().IsUnspecified()||prefix.Addr().IsMulticast(){return ErrPolicy}};return nil}
