...
export const pages: Record<string, PageDefinition> = {
  ...
  certificates: {
    id:"certificates",title:"Certificates",description:"ACME and guarded custom certificate generations, trust, deployment eligibility, renewal, and retirement.",resourceKind:"certificate.certificate",listOperation:"certificate.list",
    ...
    rowActions:[{id:"renew",label:"Renew",operation:"certificate.renew",mutating:true},{id:"deploy",label:"Deploy",operation:"certificate.deploy",mutating:true},{id:"revoke",label:"Revoke",operation:"certificate.revoke",mutating:true,tone:"critical",assurance:"mfa"}],
    globalActions:[
      {id:"material-import",label:"Import custom",operation:"certificate.material.import",mutating:true,assurance:"mfa",fields:[{key:"resource_id",label:"Certificate resource",type:"text",required:true},{key:"upload_handle",label:"One-use protected upload handle",type:"password",required:true,helper:"Use the opaque handle from a TLS-key secret enrollment bound to this resource and next generation. PEM is never submitted here."},{key:"display_label",label:"Display label",type:"text",required:true},{key:"dns_names",label:"DNS names",type:"textarea",helper:"One name per line. Supply DNS names, IP addresses, or both."},{key:"ip_addresses",label:"IP addresses",type:"textarea",helper:"One address per line."},{key:"expected_certificate_generation",label:"Current certificate generation",type:"number",required:true,defaultValue:0}]},
      {id:"material-self-signed",label:"Generate self-signed",operation:"certificate.material.self_signed",mutating:true,assurance:"mfa",tone:"warning",confirmation:"The result is explicitly untrusted, short-lived, and restricted to the selected development or recovery use.",fields:[{key:"resource_id",label:"Certificate resource",type:"text",required:true},{key:"display_label",label:"Display label",type:"text",required:true},{key:"mode",label:"Restricted use",type:"select",required:true,defaultValue:"development",options:[{label:"Development",value:"development"},{label:"Recovery",value:"recovery"}]},{key:"dns_names",label:"DNS names",type:"textarea",helper:"One name per line. Supply DNS names, IP addresses, or both."},{key:"ip_addresses",label:"IP addresses",type:"textarea",helper:"One address per line."},{key:"lifetime_seconds",label:"Lifetime in seconds",type:"number",required:true,defaultValue:86400},{key:"key_algorithm",label:"Key algorithm",type:"select",required:true,defaultValue:"ecdsa",options:[{label:"ECDSA",value:"ecdsa"},{label:"RSA",value:"rsa"}]},{key:"key_bits",label:"Key size",type:"number",required:true,defaultValue:256,helper:"ECDSA: 256, 384, or 521. RSA: 3072–8192."},{key:"expected_certificate_generation",label:"Current certificate generation",type:"number",required:true,defaultValue:0},{key:"acknowledge_no_public_replacement",label:"Acknowledge no public replacement",type:"boolean",required:true,defaultValue:false}]},
      {id:"material-list",label:"Custom materials",operation:"certificate.material.list",mutating:false,fields:[{key:"resource_id",label:"Certificate resource",type:"text",required:true}]},
      {id:"material-inspect",label:"Inspect custom",operation:"certificate.material.inspect",mutating:false,fields:[{key:"resource_id",label:"Certificate resource",type:"text",required:true},{key:"material_id",label:"Material ID",type:"text",required:true}]},
      {id:"material-eligibility",label:"Check deployment",operation:"certificate.material.deployment_eligibility",mutating:false,fields:[{key:"resource_id",label:"Certificate resource",type:"text",required:true},{key:"material_id",label:"Material ID",type:"text",required:true},{key:"consumer_kind",label:"Consumer kind",type:"select",required:true,options:[{label:"Web engine",value:"webengine"},{label:"Application",value:"application"},{label:"Panel",value:"panel"},{label:"Mail",value:"mail"},{label:"FTPS",value:"ftps"}]},{key:"consumer_id",label:"Consumer ID",type:"text",required:true},{key:"purpose",label:"Deployment purpose",type:"select",required:true,defaultValue:"public",options:[{label:"Public",value:"public"},{label:"Development",value:"development"},{label:"Recovery",value:"recovery"}]}]},
      {id:"material-retire",label:"Retire custom",operation:"certificate.material.retire",mutating:true,assurance:"mfa",tone:"critical",confirmation:"Retirement is metadata-only and is rejected while a consumer is still bound or deployment state is uncertain.",fields:[{key:"resource_id",label:"Certificate resource",type:"text",required:true},{key:"material_id",label:"Material ID",type:"text",required:true},{key:"expected_record_generation",label:"Current record generation",type:"number",required:true}]}
    ],emptyTitle:"No managed certificates",emptyBody:"Issue a public certificate or import a validated custom generation."
  },
  mail: {
